...
uses
  ...
  Vcl.ComCtrls,WPCTRRich, FireDAC.Stan.Intf, FireDAC.Stan.Option,
...
var
  ...
  viTexto : String;
  vlAttachmentFile, viCodigoPIX, viNome : String;

  procedure EncaminharBoletoEmail;
  var
    viTextoPadrao : String;
  begin
    ExecutaSimpleDSAux(' SELECT CAMINHO_ARQUIVO, CODIGO_PIX, PESSOA_ID, PESSOA_GRUPO_ID, BOLETA_ID FROM T_BOLETA '+
    ...
    begin
      ...
      begin
        ...
        viCodigoPIX      := FieldByName('CODIGO_PIX').AsString;

//        vgEmailConfig.TextoEmail.Text  := memTextoPadrao.Text;
        if viCodigoPIX <> '' then
        begin
         { vgEmailConfig.TextoEmail.Add(#13);
          vgEmailConfig.TextoEmail.Add('*** Código Pix (Copia Cola) ***');
          vgEmailConfig.TextoEmail.Add(viCodigoPIX);
          vgEmailConfig.TextoEmail.Add(#13);
          vgEmailConfig.TextoEmail.Add(#13);  }

          viTexto := viTextoPadrao + '<br />';
          viTexto := viTexto + '*** Código Pix (Copia Cola) ***';
          viTexto := viTexto + '<br />';
          viTexto := viTexto + viCodigoPIX;
          viTexto := viTexto + '<br />';
          viTexto := viTexto + '<br />';
        end;
