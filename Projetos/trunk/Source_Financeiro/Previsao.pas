...
uses
  ...
  FrameCalculadora, FireDAC.Stan.Intf, FireDAC.Stan.Option, FireDAC.Stan.Param,
...
type
  TfrmPrevisao = class(TForm)
    ...
    sqlFinanceiroCompromissoRESERVA_ID: TBCDField;
    ...
    sqlFinanceiroCompromissoTIPO_DESPESA_ID: TBCDField;
...
begin
  ...
  case vpOpcao of
    1 : begin
          ...
          else dxBarInfoFechamento.Caption := 'Livro Caixa Fechado até '+Mes[StrToInt(copy(viAnoMesFechamento,5,2))] +' de '+copy(viAnoMesFechamento,1,4);
        end;
  end;
end;
...
begin
  if Application.MessageBox(Pchar(' Saltar Lançamento Agendado?'+#13+#13+
                                  ' O Salto Cancela o Lançamento Selecionado!!!'), 'Confirmação', MB_YESNO) = IDNO then
    exit;
    ...
    begin
      ...
      begin
        viObservacao := viObservacao +
                        '  * Valores >> Orçamento '+FormatFloat('R$ ,#0.00', (sqlFinanceiroCompromissoVALOR_AGENDADO.AsCurrency+
