...
type
  TfrmCadRelatorioFinanceiro = class(TfrmCadastroAuxSimplificado)
    ...
    sqlRelatorioAnaliticoORDEM_GRUPO: TBCDField;
    sqlRelatorioSomatorioGrupo: TI9Query;
    ...
    sqlRelatorioResumidoBALANCETE_GRUPO_ID: TBCDField;
...
begin
  inherited;
  if Application.MessageBox('Excluir Modelo de Relatório?', 'Pergunta', mb_IconQuestion + mb_YesNo) = idNo then
    exit;
...
begin
  inherited;
  if Application.MessageBox('Salvar Configurações de Relatório?', 'Pergunta', mb_IconQuestion + mb_YesNo) = idNo then
    exit;

  viNome := '';
  repeat
    if not InputQuery('Nome do Modelo do Relarório', 'Nome:', viNome) then
     exit;
  until viNome <> '';
