...
begin
  ...
  if (ClientAncestral.FieldByName('TIPO_PETICIONADOR').AsString = 'S') or
    ...
    begin
      Application.MessageBox('Digite um Nome para Identificação no sistema', 'Atenção', mb_ok + MB_ICONWARNING);
...
begin
  ...
  begin
    Application.MessageBox('Selecione um tipo para esta pessoa!', 'Atenção', mb_ok + MB_ICONWARNING);
    edtTipoPessoa.SetFocus;
    abort;
  end;

  if pnlFuncao.Visible then
    VerificarPreenchimentolCX_db(lcxPessoaFuncao.Text, 'Função', lcxPessoaFuncao);
