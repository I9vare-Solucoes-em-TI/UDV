...
uses
  ...
  ad4Live, IdAntiFreezeBase, cxPC,
  cxCurrencyEdit, dxScreenTip, DbxDevartInterBase, dxSkinsCore, ad3Spell,
  frxExportRTF, IdHTTP, dxSkinOffice2007Black, dxSkinOffice2007Blue,
  dxSkinOffice2007Green, dxSkinOffice2007Pink, dxSkinOffice2007Silver,
  FundoTipo, frxExportBaseDialog, (*Geral.Model.Email, *)mxExport, FireDAC.Stan.Intf, FireDAC.Stan.Option, FireDAC.Stan.Param, FireDAC.Stan.Error, FireDAC.DatS,
...
type
  ...
  TEmailConfig = Record
    HostSmtp, HostPop3, UserName, PassWord, AssuntoPadrao, EmailEnvio, EmailDestino, EmailCopiaOculto, Tabela, Porta, CodPix : String;
...
type
  ...
  private
    ...
    procedure ConfigConnection(
      const vpCon: TI9Connection;
      const vpProtocolo: string;
      const vpPorta: Integer;
      const vpServidor: string;
      const vpCompressao: Boolean;
      const vpBaseDados: string;
  ...
  public
    ...
    procedure EnviarEmailAntigo(vBodyHtml : String; vEmailCC: string; vAttachment: String; vAnexo: TStrings);
    ...
    procedure EnviarEmailNovo(vBodyHtml : String; vEmailCC: string; vAttachment: String; vAnexo: TStrings;
    ...
    procedure Suporte(vpTipo : string = '1');
...
procedure TdtmControles.ConfigConnection(
  const vpCon: TI9Connection;
  const vpProtocolo: string;
  const vpPorta: Integer;
  const vpServidor: string;
  ...
  const vpSenha: string);
begin
  if not vpProtocolo.Trim.IsEmpty then
    vpCon.Params.Add('Protocol=' + Cripto.CriptoHexToText(vpProtocolo));

  if vpPorta > 0 then
    vpCon.Params.Add('Port='+  Cripto.CriptoHexToText(vpPorta.ToString));

  if not vpServidor.Trim.IsEmpty then
    vpCon.Params.Add('Server=' + Cripto.CriptoHexToText(vpServidor));

  if vpCompressao then
    vpCon.Params.Add('WireCompression=true');

  vpCon.Params.Database := Cripto.CriptoHexToText(vpBaseDados);
  vpCon.Params.UserName := Cripto.CriptoHexToText(vpUsuario);
  vpCon.Params.Password := Cripto.CriptoHexToText(vpSenha);
end;
...
begin
  // DB.Commit(TD); { TODO 5 -oBRUNO : VERIFICAR }
end;
...
procedure TdtmControles.EnviarEmailAntigo(vBodyHtml : String; vEmailCC: string; vAttachment: String; vAnexo: TStrings);
...
var
  ...
  viAttachmentFile,
  viCondicao : String;
  viConfigEmailId: Integer;
  viEmailEnvio: TEmailEnvio;
  viEnvioConfirmado: Boolean;
...
//  viAttachmentFile := 'c:\Temp\EmailRascunho.pdf';
  ...
  try
    ...
    dtmLookup.LerDadadosConfigEmail('T_EMAIL_CONFIG');

{    if Assigned(vgEmailConfig.TextoEmail) then
    begin
      mxStringsExport1.Strings := vgEmailConfig.TextoEmail;
      if Assigned(vgEmailConfig.TextoRodape) then
        mxStringsExport1.Strings.Add(vgEmailConfig.TextoRodape.Text);
    ...
    viTextoEmail := TStringList.Create;
    viTextoEmail.Add(vBodyHtml);

    if Assigned(vgEmailConfig.TextoEmail) then
    begin
      mxStringsExport1.Strings := viTextoEmail;
//      if Assigned(vgEmailConfig.TextoRodape) then
  //      mxStringsExport1.Strings.Add(vgEmailConfig.TextoRodape.Text);
    ...
    FreeAndNil(viTextoEmail);

    {$REGION 'Enviar E-mail'}
    viDestinatario := EmptyStr;
    viDestinatario := vgEmailConfig.EmailDestino;

    if not viDestinatario.Trim.IsEmpty then
    begin
      viEmailEnvio := TEmailEnvio.Create;
      if not vpEncaminhaAnexo then
        viAttachmentFile := '';
      try
        try
          viEmailEnvio.EnviarEmail(
            { vpEmailDestinatario } viDestinatario,
        ...
        FreeAndNil(viEmailEnvio);
      end;
    end;
    {$ENDREGION}

    viTexto := Trim(Rotinas.RetornaTextoArquivo('c:\Temp\Body.txt'));
  finally
...
begin
  // DB.ExecuteDirect(Qry); { TODO 5 -oBRUNO : VERIFICAR }
end;
...
begin
  // DB.Rollback(TD); { TODO 5 -oBRUNO : VERIFICAR }
end;
...
begin
  ...
  begin
    ...
    //DB.StartTransaction(TD); { TODO 5 -oBRUNO : VERIFICAR }
  end;
end;

procedure TdtmControles.Suporte(vpTipo : string = '1');
var
  viUrl : string;
begin
  if vpTipo = '1' then
       viUrl := 'http://tri7.mysuite2.com.br/clientlegume.php?param=hd_chat_gc_cad_chatdep&cdu=&inf=&sl=tri7&lf=&ca=&cr=&idm=&cdu=1&redirect=http://tri7.mysuite2.com.br/empresas/tri7/central.php'
  else viUrl := 'http://www.tri7.com.br';
  ShellExecute(Application.Handle,'open',PWideChar(viUrl),nil,nil,SW_SHOW);
end;
...
{  if (vgChaveSecao = 'Grupo@I9vare@Tri7') and not(viDesenvolvedor) then
  ...
  (* REMOVER ESTE CÓDIGO
  DB.Params.Clear;
  DB.Params.Values['GetDriverFunc']      := 'getSQLDriverInterBase';
  DB.Params.Values['LibraryName']        := 'dbexpida40.dll';
  DB.Params.Values['VendorLib']          := 'fbclient.dll';
  ...
  DB.Params.Values['DevartInterBase TransIsolation'] := 'ReadCommited';
  DB.Params.Values['ProductName']                    := 'DevartInterBase';
  DB.Params.Values['DriverPackageLoader']            := 'TDBXDynalinkDriverLoader,DBXCommonDriver210.bpl';
  DB.Params.Values['MetaDataPackageLoader']          := 'TDBXDevartInterBaseMetaDataCommandFactory,DbxDevartInterBaseDriver210.bpl';
  DB.Params.Values['DriverUnit']                     := 'DbxDevartInterBase';
...
//   FormatSettings.Shortdateformat := 'dd/mm/yyyy';
  ...
  try
    try
      viArquivoIni := TIniFile.Create(ExtractFilePath(Application.ExeName) + 'Config.ini');

      viSecao:= 'Geral';

      viProtocolo := viArquivoIni.ReadString(viSecao, 'Protocolo', '1F16101C09');
      viPorta := viArquivoIni.ReadInteger(viSecao, 'Porta', 3051);
      viServidor := viArquivoIni.ReadString(viSecao, 'Servidor', '');
      viCompressao := viArquivoIni.ReadString(viSecao, 'Compressao', 'N') = 'S';
      viBaseDados := viArquivoIni.ReadString(viSecao, 'BaseDados', '');
      viUsuario := viArquivoIni.ReadString(viSecao, 'Usuario', '');
      viSenha := viArquivoIni.ReadString(viSecao, 'Senha', '');
      ...
      begin
        Application.MessageBox(
          Pchar('Não foi possível conectar no banco de Dados!'+#13+
...
function TdtmControles.NucleoNome: string;
begin
  Result := dtmControles.GetStr('SELECT DESCRICAO FROM T_CEDENTE WHERE PRINCIPAL = '+ QuotedStr('S'));
end;
...
begin
  ...
  DBH.Params.Values['GetDriverFunc']      := 'getSQLDriverInterBase';
  DBH.Params.Values['LibraryName']        := 'dbexpida40.dll';
  DBH.Params.Values['VendorLib']          := 'fbclient.dll';
  ...
  DBH.Params.Values['DevartInterBase TransIsolation'] := 'ReadCommited';
  DBH.Params.Values['ProductName']                    := 'DevartInterBase';
  DBH.Params.Values['DriverPackageLoader']            := 'TDBXDynalinkDriverLoader,DBXCommonDriver210.bpl';
  DBH.Params.Values['MetaDataPackageLoader']          := 'TDBXDevartInterBaseMetaDataCommandFactory,DbxDevartInterBaseDriver210.bpl';
  DBH.Params.Values['DriverUnit']                     := 'DbxDevartInterBase';

  //Novos
  DBH.Params.Values['CommitRetain']       := 'False';
  DBH.Params.Values['RoleName']           := '';
  ...
  DBH.Params.Values['Prepared']           := 'True';
