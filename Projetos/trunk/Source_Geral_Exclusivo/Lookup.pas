...
type
  ...
  TdtmLookup = class(TDataModule)
    ...
    sqlCedenteTodos: TI9Query;
    dtsCedenteTodos: TDataSource;
    sqlCedenteTodosDESCRICAO: TStringField;
    sqlCedenteTodosCEDENTE_ID: TBCDField;
    sqlCedenteAtivos: TI9Query;
    ...
    sqlCedenteAtivosCEDENTE_ID: TBCDField;
    ...
    sqlCompromissoMensalVALOR_ATUAL: TBCDField;
    ...
    sqlGrupoTodos: TI9Query;
    sqlGrupoTodosDESCRICAO: TStringField;
    sqlGrupoTodosPESSOA_GRUPO_ID: TBCDField;
    sqlGrupoAtivo: TI9Query;
    ...
    sqlReservasAtivas: TI9Query;
    sqlReservasAtivasDESCRICAO: TStringField;
    sqlReservasAtivasRESERVA_ID: TBCDField;
    dtsReservasAtivas: TDataSource;
    Lista_ReservasAtivas: TcxEditRepositoryLookupComboBoxItem;
    sqlReservasAtivasBALANCETE_GRUPO_ID: TBCDField;
    sqlCabecalhoNOME: TStringField;
    sqlCabecalhoRAZAO_SOCIAL: TStringField;
    sqlCabecalhoCNPJ_CPF: TStringField;
    sqlCabecalhoEND_CIDADE: TStringField;
    sqlCabecalhoEND_UF: TStringField;
    sqlCabecalhoCALC_CPNJ: TStringField;
    ...
    Combo_TipoCusto: TcxEditRepositoryImageComboBoxItem;
    procedure DataModuleCreate(Sender: TObject);
    procedure DataModuleDestroy(Sender: TObject);
    procedure sqlCabecalhoCalcFields(DataSet: TDataSet);
  private
  ...
  public
    ...
    procedure MarcarDesmarcarClientCompromissos(vpClientDataSet : TClientDataSet; vpMarcar : Boolean);
    ...
    procedure DefinirPessoaGrupoPrincipal(vpPessoaID, vpPessoaGrupoId, vpCampo : String);
    procedure DefinirPessoaGrupoFatura(vpPessoaGrupoId, vpTipo, vpBoletoEmail : String);
    procedure VerificarEmailGrupo;
    ...
    procedure CarregarAno(vpIcxComboBox: TcxImageComboBox);
  end;
...
var
  ...
  viDiferenca, viCompromisoId, viAnoMesFinal, viQtdGravar, viProximoPeriodo, viPeriodoInicial, viSituacao : String;
  ...
  begin
    ...
    ExecutaSqlAuxiliar(' SELECT SUM(VALOR_ATUAL) AS VALOR, COUNT(COMPROMISSO_VENCIDO_ID) AS QTD, SITUACAO '+
                       ' FROM T_COMPROMISSO_VENCIDO '+
                       ' WHERE CONTABIL_CONTA_ID = '+ CurrToStr(vgCompromissoVencido.ContabilContaId)+
                       '   AND PESSOA_ID = '+ CurrToStr(vgCompromissoVencido.PessoaId)+
                       '   AND ANO_MES_REFERENCIA = '+QuotedStr(vgCompromissoVencido.PeriodoInicial)+
                       '   AND ((SITUACAO = '+QuotedStr('2')+ ') or (SITUACAO = '+QuotedStr('3')+'))'+
                       ' GROUP BY ANO_MES_REFERENCIA, SITUACAO',0);
    ...
    begin
      ...
      viSituacao     := dtmControles.sqlAuxiliar.FieldByName('SITUACAO').AsString;

      if viQtdLimitada then
      begin
        if (viSituacao = '3') or (viValorPago >= vgCompromissoVencido.Valor) then
        begin
  ...
  begin
    ExecutaSqlAuxiliar(' SELECT CONTABIL_CONTA_ID, COMPROMISSO_AGENDADO_ID, VALOR_ATUAL, VALOR_ANTERIOR, SITUACAO_GERAL, SITUACAO_LOCAL '+
                       ...
                       ' WHERE CONTABIL_CONTA_ID = '+ CurrToStr(vgCompromissoVencido.ContabilContaId)+
                       '   AND PESSOA_ID = '+ CurrToStr(vgCompromissoVencido.PessoaId)+
                       '   AND NOT LANCAMENTO_COMPROMISSO_ID IS NULL',0);
    if not dtmControles.sqlAuxiliar.IsEmpty then
    begin
      ...
      begin
        if vgCompromissoVencido.MostrarMensagem then
          Application.MessageBox(Pchar('Este compromisso já se encontra agendado com a Situação de Inativo!!!'+#13+#13+
      ...
      begin
        if vgCompromissoVencido.MostrarMensagem then
          Application.MessageBox('Já existe este compromisso agendado!!!', 'Atenção', MB_OK + MB_ICONWARNING);
        Result := False;
        exit;
      end
      else
      begin
        if vgCompromissoVencido.MostrarMensagem then
          Application.MessageBox('Compromisso Inativo. Verifique no Gerenciador de Compromissos!!!', 'Atenção', MB_OK + MB_ICONWARNING);
        Result := False;
      end;
    end
    else
...
begin
  ...
  begin
    repeat
      if vgCompromissoVencido.LancamentoCompromissoId = '0' then
        vgCompromissoVencido.LancamentoCompromissoId := '';

      dtmLookupContabil.VerificarProvimentoMes(vgCompromissoVencido.LancamentoCompromissoId,
                                                vgCompromissoVencido.PeriodoInicial);
...
procedure TdtmLookup.CarregarAno(vpIcxComboBox: TcxImageComboBox);
var
  viAno, viAnoInicial : Integer;
begin
  viAnoInicial := 2015;
  viAno := StrToInt(Copy(DateToStr(Date), 7,4));
  vpIcxComboBox.Properties.Items.Clear;
  repeat
    Inc(viAnoInicial);
    dtmLookup.CarregarIcxComboBox(IntToStr(viAnoInicial), IntToStr(viAnoInicial), vpIcxComboBox);
...
begin
  ...
  sqlCabecalho.ParamByName('CEDENTE_ID').AsBCD         := vgCedenteAtivo;
...
begin
  ...
  sqlGrupoAtivo.Close;
  sqlGrupoAtivo.ParamByName('CEDENTE_ID').AsBCD := vgCedenteAtivo;
  sqlGrupoAtivo.Open;

  sqlGrupoTodos.Close;
  sqlGrupoTodos.ParamByName('CEDENTE_ID').AsBCD := vgCedenteAtivo;
  sqlGrupoTodos.Open;

  sqlBalanceteGrupo.Close;
  sqlBalanceteGrupo.ParamByName('CEDENTE_ID').AsBCD := vgCedenteAtivo;
  sqlBalanceteGrupo.Open;

  sqlGrupoContabilTodos.Close;
  sqlGrupoContabilTodos.ParamByName('CEDENTE_ID').AsBCD := vgCedenteAtivo;
  sqlGrupoContabilTodos.Open;

  sqlGrupoContabilAtivos.Close;
  sqlGrupoContabilAtivos.ParamByName('CEDENTE_ID').AsBCD := vgCedenteAtivo;
  sqlGrupoContabilAtivos.Open;

  sqlGrupoContabilEntradas.Close;
  sqlGrupoContabilEntradas.ParamByName('CEDENTE_ID').AsBCD := vgCedenteAtivo;
  sqlGrupoContabilEntradas.Open;

  sqlGrupoContabilSaidas.Close;
  sqlGrupoContabilSaidas.ParamByName('CEDENTE_ID').AsBCD := vgCedenteAtivo;
  sqlGrupoContabilSaidas.Open;

  sqlCentroCustoTodos.Close;
  sqlCentroCustoTodos.ParamByName('CEDENTE_ID').AsBCD := vgCedenteAtivo;
  sqlCentroCustoTodos.Open;

  sqlCentroCustoAtivo.Close;
  sqlCentroCustoAtivo.ParamByName('CEDENTE_ID').AsBCD := vgCedenteAtivo;
  sqlCentroCustoAtivo.Open;

  sqlPlanoContasAtivo.Close;
  sqlPlanoContasAtivo.ParamByName('CEDENTE_ID').AsBCD := vgCedenteAtivo;
  sqlPlanoContasAtivo.Open;

  sqlPlanoContasTodos.Close;
  sqlPlanoContasTodos.ParamByName('CEDENTE_ID').AsBCD := vgCedenteAtivo;
  sqlPlanoContasTodos.Open;

  sqlPlanoContasReceitas.Close;
  sqlPlanoContasReceitas.ParamByName('CEDENTE_ID').AsBCD := vgCedenteAtivo;
  sqlPlanoContasReceitas.Open;

  sqlCompromissoMensal.Close;
  sqlCompromissoMensal.ParamByName('CEDENTE_ID').AsBCD := vgCedenteAtivo;
  sqlCompromissoMensal.Open;

  sqlPessoas.Close;
  sqlPessoas.ParamByName('CEDENTE_ID').AsBCD := vgCedenteAtivo;
  sqlPessoas.Open;

  sqlCompReceitaTodos.Close;
  sqlCompReceitaTodos.ParamByName('CEDENTE_ID').AsBCD := vgCedenteAtivo;
  sqlCompReceitaTodos.Open;

  sqlPlanoContasDespesasTodos.Close;
  sqlPlanoContasDespesasTodos.ParamByName('CEDENTE_ID').AsBCD := vgCedenteAtivo;
  sqlPlanoContasDespesasTodos.Open;

  sqlPlanoContasDespesasAtivo.Close;
  sqlPlanoContasDespesasAtivo.ParamByName('CEDENTE_ID').AsBCD := vgCedenteAtivo;
  sqlPlanoContasDespesasAtivo.Open;

  sqlCompSaidaTodos.Close;
  sqlCompSaidaTodos.ParamByName('CEDENTE_ID').AsBCD := vgCedenteAtivo;
  sqlCompSaidaTodos.Open;

  sqlReservasAtivas.Close;
  sqlReservasAtivas.Params.ParamByName('CEDENTE_ID').AsBCD := vgCedenteAtivo;
...
procedure TdtmLookup.DefinirPessoaGrupoFatura(vpPessoaGrupoId, vpTipo,
      vpBoletoEmail : String);
begin
  ExecutaSqlAuxiliar(' UPDATE T_PESSOA SET FATURA_TIPO = '+ QuotedStr(vpTipo)+
                     '                    ,BOLETO_EMAIL='+ QuotedStr(vpBoletoEmail)+
                     ' WHERE PESSOA_GRUPO_ID = '+vpPessoaGrupoId,1);

  ExecutaSqlAuxiliar(' UPDATE T_PESSOA_GRUPO SET FATURA_TIPO = '+ QuotedStr(vpTipo)+
                     '                          ,BOLETO_EMAIL='+ QuotedStr(vpBoletoEmail)+
                     ' WHERE PESSOA_GRUPO_ID = '+vpPessoaGrupoId,1);
end;

procedure TdtmLookup.DefinirPessoaGrupoPrincipal(vpPessoaID, vpPessoaGrupoId, vpCampo : String);
begin
  ExecutaSqlAuxiliar(' UPDATE T_PESSOA SET '+vpCampo+' = '+QuotedStr('N')+
                     ' WHERE PESSOA_GRUPO_ID = '+vpPessoaGrupoId,1);

  ExecutaSqlAuxiliar(' UPDATE T_PESSOA SET '+vpCampo+' = '+QuotedStr('S')+
                     ' WHERE PESSOA_ID = '+vpPessoaID,1);
end;
...
//  ExportGridToExcel('C:\TEMP\RELTEMP.XLS', vpGrid);
  ExportGridToExcel(SaveDialog1.FileName, vpGrid);

//  AbrirPlanilha();
end;

procedure TdtmLookup.FiltrarGrupoContasBalancete(vpSimpleDataSet : TI9Query; vpBalanceteId : Integer;
           vpFiltrar : Boolean);
begin
...
begin
  if vpTirarBarra = 'S' then
    ...
    if vpTirarBarra = '-' then
      result := copy(vpAnoMes,1,4) +'-'+ copy(vpAnoMes,5,4)
  else
  begin
...
procedure TdtmLookup.MarcarDesmarcarClientCompromissos(
  vpClientDataSet: TClientDataSet; vpMarcar: Boolean);
begin
  ...
  begin
    ...
    vpClientDataSet.FieldByName('SELECIONADO').AsBoolean := vpMarcar;
...
procedure TdtmLookup.MarcarDesmarcarCompromissos(vpI9Query : TI9Query; vpMarcar : Boolean);
begin
...
procedure TdtmLookup.sqlCabecalhoCalcFields(DataSet: TDataSet);
begin
  sqlCabecalhoCALC_CPNJ.AsString := Rotinas.FormataCNPJ(sqlCabecalhoCNPJ_CPF.AsString);
end;
...
procedure TdtmLookup.VerificarEmailGrupo;
var
  viFlag : Boolean;

  procedure DesativarExcluirGrupo(vpID : String; vpRetirarPessoaGrupo : Boolean = False);
  var
    viQtdBoleto : Integer;
  begin
    if vpRetirarPessoaGrupo then
      ExecutaSqlAuxiliar('UPDATE T_PESSOA SET PESSOA_GRUPO_ID = NULL WHERE PESSOA_GRUPO_ID = '+vpID,1);

    viQtdBoleto := dtmControles.GetInt(' SELECT COUNT(BOLETA_ID) FROM T_BOLETA WHERE PESSOA_GRUPO_ID = '+ vpID);
    if viQtdBoleto = 0 then
         ExecutaSqlAuxiliar('DELETE FROM T_PESSOA_GRUPO WHERE PESSOA_GRUPO_ID = '+vpID,1)
    else ExecutaSqlAuxiliar('UPDATE T_PESSOA_GRUPO SET SITUACAO = ''I'' WHERE PESSOA_GRUPO_ID = '+vpID,1)
  end;

  procedure DefinirEmailGrupoPadrao;
  var
    viPessoa : string;
  begin
    dtmControles.sqlAuxiliar.First;
    viPessoa := dtmControles.sqlAuxiliar.FieldByName('PESSOA_ID').AsString;
    ExecutaSqlAuxiliar(' UPDATE T_PESSOA SET PRINCIPAL = ''S'', EMAIL_GRUPO_PADRAO = ''S'''+
                       ' WHERE PESSOA_ID = '+viPessoa,1);
  end;

begin
  ExecutaSqlAuxiliar(' UPDATE T_PESSOA SET PESSOA_TERCEIRO_ID = NULL, EMAIL_TERCEIRO = '+QuotedStr('N')+
                     ' WHERE PESSOA_ID IN ( '+
                     ' SELECT PESSOA_ID '+
                     ' FROM T_PESSOA '+
                     ' WHERE NOT PESSOA_GRUPO_ID IS NULL '+
                     '   AND NOT PESSOA_TERCEIRO_ID IS NULL)',1);

  ExecutaSimpleDSAux(' SELECT PESSOA_GRUPO_ID '+
                     ' FROM T_PESSOA_GRUPO '+
                     ' WHERE SITUACAO = '+QuotedStr('A'),0);
  with dtmControles.SimpleAuxiliar do
  begin
    First;
    while not Eof do
    begin
      ExecutaSqlAuxiliar(' SELECT PESSOA_ID, EMAIL_GRUPO_PADRAO, PRINCIPAL, PESSOA_GRUPO_ID '+
                         ' FROM T_PESSOA '+
                         ' WHERE PESSOA_GRUPO_ID = '+FieldByName('PESSOA_GRUPO_ID').AsString+
                         '  ORDER BY GRAU DESC' ,0);

      with dtmControles.sqlAuxiliar do
      begin
        if IsEmpty then
          DesativarExcluirGrupo(dtmControles.SimpleAuxiliar.FieldByName('PESSOA_GRUPO_ID').AsString)
        else
          if RecordCount = 1 then
            DesativarExcluirGrupo(FieldByName('PESSOA_GRUPO_ID').AsString, True)
        else
        begin
          First;
          viFlag := False;
          while not Eof do
          begin
            viFlag := FieldByName('EMAIL_GRUPO_PADRAO').AsString = 'S';
            if viFlag then
              Break;
            Next;
          end;
...
begin
  ...
  vgDadosImagem.LocalImagem := GetCurrentDir;
  CriarDiretorio(vgDadosImagem.LocalImagem);
...
begin
  ...
  begin
    ...
    dtmControles.SimpleAuxiliar.post;
