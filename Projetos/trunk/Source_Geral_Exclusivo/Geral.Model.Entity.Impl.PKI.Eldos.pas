 ...
 * @version    1.0.0.0
 * @unit       Geral.Model.Entity.Impl.PKI.Eldos.pas
 * @author     I9vare
...
type
  ...
  public
      ...
      const vpOffsetX: Integer;
      const vpOffsetY: Integer): Boolean;
  ...
  public
    ...
    function BuscarDataAssinaturaArquivo(
      const vpCaminhoArquivo: string): TDateTime;
  ...
  TSubject = class(TInterfacedObject, IX500Principal)
  private
    FEldosPKCS: TElX509Certificate;
    FCommonName: string;
  ...
  public
    ...
    function Provider: TElX509Certificate;
    function Subject:IX500Principal;
...
function TX509Certificate.Subject: IX500Principal;
begin
  Result:= FSubject;
end;
  ...
  try
    try
      ...
      try
        ...
        Sig.WidgetProps.OffsetX        := vpOffsetX; //Posição borda esqueda da página
        Sig.WidgetProps.Offsety        := Document.PageInfos[0].Height - (vpOffsety + 50); //Pisição borda inferior da página
        if Sig.WidgetProps.Offsety < 0 then
          Sig.WidgetProps.Offsety      := 5;
        ...
        CertStorage.Clear;
        CertStorage.Add(viEldosPKCS, True);
...
function TSubject.CommonName: string;
begin
  Result:= FCommonName;
end;

constructor TSubject.Create(const vpEdlsoPKCS: TElX509Certificate);
begin
  inherited Create;
  FEldosPKCS := vpEdlsoPKCS;

  FCommonName := vpEdlsoPKCS.SubjectName.CommonName;
end;

class function TSubject.New(
  const vpEdlsoPKCS: TElX509Certificate): IX500Principal;
begin
  Result := Create(
    vpEdlsoPKCS);
...
function TAssinaturaArquivoCADES.Assinar(
  ...
  const vpX509Certificate: IX509Certificate): string;
var
  viArquivo: TFileStream;
  viCAdESProcessor : TElCAdESSignatureProcessor;
  viCMS: TElSignedCMSMessage;
  viSig: TElCMSSignature;
  viNovoArquivo: TFileStream;
  viProviderEldosPKCS: IProviderEldosPKCS;
  viEldosPKCS: TElX509Certificate;
begin
  viArquivo := nil;
  viCAdESProcessor := nil;
  viCMS := nil;
  viNovoArquivo := nil;
  ...
  try
    ...
    viCMS := TElSignedCMSMessage.Create(nil);

    viCMS.CreateNew(viArquivo, 0, viArquivo.Size);
    viSig := viCMS.Signatures[viCMS.AddSignature()];

    viCAdESProcessor := TElCAdESSignatureProcessor.Create();
    viCAdESProcessor.Signature := viSig;
    viCAdESProcessor.ForceSigningCertificateV2 := True;
    viCAdESProcessor.IgnoreChainValidationErrors := True;
    viCAdESProcessor.OfflineMode := False;

    viCAdESProcessor.CreateBES(viEldosPKCS);

    Result :=  ExtractFilePath(vpCaminhoArquivo) + ExtractFileName(vpCaminhoArquivo) + '.p7s';

    viNovoArquivo := TFileStream.Create(
      Result,
      fmCreate);

    viCMS.Save(viNovoArquivo);
  finally
    FreeAndNil(viArquivo);
    FreeAndNil(viCAdESProcessor);
    FreeAndNil(viCMS);
    FreeAndNil(viNovoArquivo);
  end;
end;

function TAssinaturaArquivoCADES.BuscarDataAssinaturaArquivo(const vpCaminhoArquivo: string): TDateTime;
var
  viCms: TElSignedCMSMessage;
  viCAdESProcessor : TElCAdESSignatureProcessor;  
  viArquivo: TFileStream;
begin      
  ...
  try
    ...
    viCms := TElSignedCMSMessage.Create(nil);
    viCms.Open(viArquivo, nil);

    if not (viCms.SignatureCount > 0) then
      raise Exception.Create('Arquivo com mais de uma assinatura.');

    if viCms.SignatureCount = 0 then
      raise Exception.Create('Arquivo não assinado.');

    viCAdESProcessor := TElCAdESSignatureProcessor.Create(viCms.Signatures[0]);

    Result := viCAdESProcessor.ClaimedSigningTime;
  finally
    FreeAndNil(viArquivo);
    FreeAndNil(viCAdESProcessor);
  end;
