...
var
  ...
  procedure  AtualizarConfiguracoes(vpGrid : TcxGridDBTableView; vpForm : String);
...
begin
  ...
  begin
    with dtmControles.SimpleAuxiliar do
    begin
      // Verifica a Ordem dos Registros;
      for i := 0 to vpGrid.ItemCount - 1 do
      begin
        if Locate('CAMPO', vpGrid.Items[i].Caption,[loCaseInsensitive]) then
        begin
          if (FieldByName('ID').AsInteger <> vpGrid.Items[i].Index) then
          begin
            Edit;
            FieldByName('ID').AsInteger := vpGrid.Items[i].Index;
            ApplyUpdates(-1);
          end;
        end;
      end;
...
begin
  ...
  begin
    ...
    begin
      ...
      for i := 0 to vpGrid.ItemCount - 1 do
      begin
        if ClientGrid.Locate('CAMPO',vpGrid.Items[i].Caption,[loCaseInsensitive]) then
        begin
          if (ClientGridID.AsInteger <> vpGrid.Items[i].Index) then
          begin
            ClientGrid.Edit;
            ClientGridID.AsInteger := vpGrid.Items[i].Index;
            ClientGrid.Post;
          end;
        end
        else
