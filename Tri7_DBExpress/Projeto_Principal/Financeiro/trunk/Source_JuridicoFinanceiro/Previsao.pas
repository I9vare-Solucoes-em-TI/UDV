...
begin
  ...
  begin
    Application.MessageBox('Alteração não permitida. Existe boleto vinculado!!!','Aviso', MB_OK + MB_ICONINFORMATION);
    exit;
  end;
  ...
  if Application.MessageBox(Pchar('Confirma a alteração da Data do Registro Selecionado, com vencimento em '+
...
begin
  ...
  if Application.MessageBox(Pchar('Confirma a alteração do Valor do Registro Selecionado, com vencimento em '+
                                    sqlFinanceiroCompromissoDATA_VENCIMENTO.AsString+'?'+viMensagem),'Pergunta', mb_IconQuestion + mb_YesNo) = idNo then
    exit;
...
begin
  ...
  begin
    Application.MessageBox('Cancelamento não permitido. Existem itens já pago neste Faturamento!!!','Aviso', MB_OK + MB_ICONINFORMATION);
    Exit;
  end;

  if Application.MessageBox(Pchar(' Cancelar Faturamento?'+#13+#13+
                                  ' Todos os itens vinculado ao Faturamento serão disponibilizados em "Financeiro Ximenes"!!!'), 'Confirmação', MB_YESNO) = IDNO then
    exit;
...
begin
  if Application.MessageBox('Retirar Item dos Lançamentos Favoritos?', 'Confirmação', MB_YESNO) = IDNO then
    exit;
...
begin
  ...
  dtmLookupContabil.PreencherSaldoLabel(' Previsão de Saldo  '+ tbcPrevisao.Tabs[tbcPrevisao.TabIndex].Caption+' R$ ',viValorPrevisao, dxPrevisaoSaldo, True ,'2');
end;
...
begin
  ...
  begin
    Application.MessageBox('Sem Permissão pra Registrar!!!','Aviso', MB_OK + MB_ICONINFORMATION);
    Exit;
  end;
...
begin
  if Application.MessageBox(Pchar('Finalizar Orçamento. Confirma?'+#13+
                                  'O Saldo Disponível do Orçamento é de '+FormatFloat('R$ ,#0.00', sqlFinanceiroCompromissocalc_saida.AsCurrency)),'Pergunta', mb_IconQuestion + mb_YesNo) = idNo then
    exit;
...
begin
  try
    ...
    case vgTipoForm[1] of
      '3' : Begin
              Caption := 'Controle Financeiro - Previsão';
...
begin
  if sqlFinanceiroCompromissoUTILIZA_BOLETO.AsString = 'N' then begin
    Application.MessageBox('Cliente não utiliza geração de boleto!','Aviso', MB_OK + MB_ICONINFORMATION);
    Exit;
  end;

  if sqlFinanceiroCompromissoBOLETA_ID.AsInteger > 0 then begin
    Application.MessageBox('Boleta já foi gerada!','Aviso', MB_OK + MB_ICONINFORMATION);
    Exit;
  end;

  if Application.MessageBox('Deseja gerar boleto?','Confirmação', MB_YESNO + MB_ICONQUESTION) = IDNO then
    Exit;
  ...
  if viBancoId = 0 then begin
    Application.MessageBox('Informe a conta bancária do caixa financeiro! ','Falha', MB_OK + MB_ICONWARNING);
    Exit;
  end;
...
begin
  ...
  begin
    Application.MessageBox(Pchar('Lançamento aguardando ativação.'+#13+#13+
...
begin
  ...
  begin
    Application.MessageBox(Pchar('Relatório de Previsão não configurado!!!'), 'Atenção', MB_OK + MB_ICONWARNING);
    exit;
  end;
...
begin
  ...
  if tbcPagamentos.TabIndex = 1 then
       viMsg := ' Saltar Lançamento Agendado?'+#13+#13+' O salto cancela o(s) lançamento(s) selecionado(s)!!!'
  else viMsg := ' Devolver Item(s) ao Sistema de Origem?';

  if Application.MessageBox(Pchar(viMsg), 'Confirmação', MB_YESNO) = IDNO then
    exit;
...
var
  ...
  {$REGION 'VerificarVencimento e Orçamento'}
  procedure VerificarVencimento;
  begin
    if sqlFinanceiroCompromissoATIVADO.AsString = '0' then
    begin
      sqlFinanceiroCompromissocalc_Observacao.AsString := 'AGUARDANDO ATIVAÇÃO DO AGENDAMENTO!!!';
      Exit;
    end;

    if sqlFinanceiroCompromissoOPERACAO.AsString = 'O' then
    begin
      viGrupo := 3; {ORÇAMENTOS}
      viObservacao := '  ORÇAMENTO '+ sqlFinanceiroCompromissoHISTORICO.AsString +
                      ' * Valor Orçado '+FormatFloat('R$ ,#0.00', (sqlFinanceiroCompromissoVALOR_AGENDADO.AsCurrency+
    ...
    begin
      ...
      if sqlFinanceiroCompromissoDATA_VENCIMENTO.AsDateTime = vgDataAtualTipoDate then
        ...
        begin
          ...
          sqlFinanceiroCompromissocalc_Observacao.AsString := 'Vencido há '+ IntToStr(viNumeroDias)+' dia(s)...'+ sqlFinanceiroCompromissocalc_Observacao.AsString;
        end;
...
{     if sqlFinanceiroCompromissoDATA_HISTORICO.AsString <> '' then
        sqlFinanceiroCompromissocalc_Observacao.AsString := sqlFinanceiroCompromissocalc_Observacao.AsString + ' *** '+
                                                            'Histórico de Alteração de Vencimento = '+ sqlFinanceiroCompromissoDATA_HISTORICO.AsString;}
    end;
  end;
  {$ENDREGION}
...
var
  ...
  begin
    ...
    // Armazena Condição para Previsão do Saldo
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          Application.MessageBox(Pchar('Boleto já registrado!!!'+#13+#13+
...
begin
  ...
  begin
    ...
    begin
      ...
      vgItensVinculados.Titulo           := 'Itens do Orçamento - '+sqlFinanceiroCompromissoHISTORICO.AsString;
...
begin
  if sqlFinanceiroCompromissoBOLETA_ID.AsInteger = 0 then begin
    Application.MessageBox('Arquivo não foi gerado!','Informação', MB_OK + MB_ICONINFORMATION);
    Exit;
  end;
  ...
  if not FileExists(vgCaminhoArquivo) then begin
    Application.MessageBox('Caminho do arquivo não encontrado!','Informação', MB_OK + MB_ICONINFORMATION);
    Exit;
  end;
