...
begin
  ...
  if (sqlLivroCaixaINTERNO.AsString = 'S') then
    if Application.MessageBox('Item não pode ser excluído.', 'Confirmação', MB_YESNO) = IDNO then
      exit;

  vgDadosCadastro.SomentePesquisa := False;

  if sqlLivroCaixaCONCILIACAO_REGISTRADO_ID.AsString <> '' then
    vgDadosLivroCaixa.MensagemBloqueado := 'Item ja Conciliado. Alteração de alguns campos não permitido!!!'
  else
    if sqlLivroCaixaIDENTIFICADOR_TIPO.AsString = 'XF' then
      vgDadosLivroCaixa.MensagemBloqueado := 'ITEM AUTOMÁTICO DO FINANCEIRO XIMENES!!!'
  else
    if sqlLivroCaixaAUTOMATICO.AsString = 'S' then
       vgDadosLivroCaixa.MensagemBloqueado := 'LANÇAMENTO INTERNO!!!';
...
var
  ...
  begin
    ...
    begin
      Application.MessageBox(Pchar('Item de Agendamento e/ou Faturamento não pode ser excluído.'+#13+#13+ 'Estorne e/ou Cancele o Item!!!'), 'Atenção', MB_OK + MB_ICONWARNING);
      abort;
    end;

    if (sqlLivroCaixaAUTOMATICO.AsString = 'S') then
    begin
      Application.MessageBox(Pchar('Item de Lançamento Interno.'+#13+#13+ 'Exclusão não permitida!!!'), 'Atenção', MB_OK + MB_ICONWARNING);
      abort;
    end;

    if sqlLivroCaixaCALC_CONCILIADO.AsString = 'S' then
    begin
      Application.MessageBox(Pchar('Item ja Conciliado.'+#13+#13+ 'Exclusão não permitida!!!'), 'Atenção', MB_OK + MB_ICONWARNING);
      abort;
    end;
  end;
...
begin
  ...
  if vgPertmissaoAlteracao then
    Application.MessageBox('Permissão Ativada!!!', 'Informação', mb_Ok + MB_ICONINFORMATION);
end;
...
begin
  ...
  if vgPermissaoInterno then
    Application.MessageBox('Permissão Lançamento Interno Ativada!!!', 'Informação', mb_Ok + MB_ICONINFORMATION);
end;
...
var
  ...
  begin
    ...
    begin
      Application.MessageBox('Agrupamento não permitido. Os Registros devem estar na mesma data!!!', 'Atenção', mb_Ok + MB_ICONEXCLAMATION);
      abort;
    end;

    if (viOperacao <> viOperacaoAux) then
    begin
      Application.MessageBox('Agrupamento não permitido. Os Registros devem ser do mesmo tipo de operação!!!', 'Atenção', mb_Ok + MB_ICONEXCLAMATION);
      abort;
    end;

    if VarToStr(cxGridPesquisa.DataController.Values[ARowInfo.RecordIndex, cxGridPesquisaIndiceImagem.index]) <> '' then
    begin
      Application.MessageBox('Agrupamento não permitido. Existe registro selecionado já agrupado!!!', 'Atenção', mb_Ok + MB_ICONEXCLAMATION);
      abort;
    end;
    ...
    begin
      ...
      begin
        Application.MessageBox('Agrupamento não permitido. Existem comprovantes digitalizados para + de 1 registro!!!', 'Atenção', mb_Ok + MB_ICONEXCLAMATION);
        abort;
      end;
...
begin
  ...
  begin
    ...
    repeat
      if not InputQuery('Descrição do Agrupamento', 'Descrição', viDescricaoAgrupador) then
        exit;
  ...
  begin
    ...
    begin
      ...
      if sqlLivroCaixaVALOR_OUTRA_DEDUCAO.AsCurrency > 0 then
        PreencherObservacao(' - Outras Deduções '+ FormatFloat(',#0.00', sqlLivroCaixaVALOR_OUTRA_DEDUCAO.AsCurrency));
...
begin
  ...
  begin
    if sqlLivroCaixaCALC_OBSERVACAO.AsString = '' then
         sqlLivroCaixaCALC_OBSERVACAO.AsString := 'Registro Contabil de Distribuição de Lucro'
    else sqlLivroCaixaCALC_OBSERVACAO.AsString := sqlLivroCaixaCALC_OBSERVACAO.AsString + ' - '+
                                                  'Registro Contabil de Distribuição de Lucro';
  end;
