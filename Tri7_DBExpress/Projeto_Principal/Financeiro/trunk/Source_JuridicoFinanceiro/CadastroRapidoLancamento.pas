...
var
  ...
  begin
    if vpEdit.Text = '' then
      ...
      begin
        Application.MessageBox('Valor negativo não permitido!!!', 'Atenção', mb_Ok + MB_ICONEXCLAMATION);
        vpEdit.Text := '';
        vpEdit.SetFocus;
        Abort;
      end;
...
begin
  ...
  begin
    ...
    begin
      Application.MessageBox('Valor do Desconto está maior que o Valor Faturado!!!', 'Atenção', mb_Ok + MB_ICONEXCLAMATION);
...
begin
  ...
  begin
    Application.MessageBox('Histórico não preenchido !!!', 'Atenção', mb_Ok + MB_ICONEXCLAMATION);
    exit;
  end;

  dtmLookupContabil.GravarHistorico(edtHistorico.Text, IntToStr(lcxCompromisso.EditValue));
  Application.MessageBox('Histórico padrão salvo neste Compromisso !!!', 'Atenção', mb_Ok + MB_ICONEXCLAMATION);
end;
...
begin
  ...
  Application.MessageBox('Compromisso padrão salvo neste Favorecido !!!', 'Atenção', mb_Ok + MB_ICONEXCLAMATION);
end;
...
var
  ...
  begin
    ...
    begin
      ...
      lblInformaca.Caption        := 'Lançamento CONCILIAÇÃO ';
    end
    else
    begin
  ...
  begin
    ...
    begin
      ...
      if vgDadosLivroCaixa.Bloqueado then
           pnlAlerta.Caption := 'ALTERAÇÃO NÃO PERMITIDA. '+vgDadosLivroCaixa.MensagemBloqueado
...
var
  ...
  begin
    ...
    begin
      if Application.MessageBox('Data de Registro inferior a 30 dias. Solicitar Permissão?', 'Confirmação', MB_YESNO) = IDNO then
        Abort;
    ...
    begin
      Application.MessageBox(Pchar('Data do Registro não pode ser maior que a Data Atual!!!'), 'Aviso', MB_OK + MB_DEFBUTTON1 + MB_ICONINFORMATION);
      Abort;
    end;
    ...
    begin
      ...
      VerificarPreenchimentoLCX_TX(lcxConta.Text, 'Caixa Lançamento', lcxConta);
    end;
...
begin
  ...
  begin
    if Application.MessageBox('Atualizar Valor do Agendamento?', 'Confirmação', MB_YESNO) = IDNO then
      Exit;
