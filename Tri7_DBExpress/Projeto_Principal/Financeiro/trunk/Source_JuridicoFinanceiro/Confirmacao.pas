...
begin
  ...
  begin
    ...
    begin
      ...
      VerificarPreenchimentoEDT_TX(edtDescricaoAgrupada.Text, 'Histórico do Fechamento', edtDescricaoAgrupada);
    end
    else
    begin
      viMsg := 'Confirma Registro dos Itens?';
      VerificarPreenchimentoEDT_TX(edtDescricaoAgrupada.Text, 'Descrição Agrupado', edtDescricaoAgrupada);
    end;


    if pnlFechamento.Visible then
    begin
      VerificarPreenchimentoICX_TX(icxNovaEspecie.Text, 'Espécie', icxNovaEspecie);
...
begin
  ...
  if vgDadosConfirmacao.Fechamento then
  begin
    chxAgrupado.Caption := 'Informe um Histórico para o Fechamento?';
    chxAgrupado.Checked := True;
    lblInformacaoLancamento.Caption := 'Confirmação de Fechamento/Faturamento do(s) Item(s) - Operação '+icxOperacao.Text;
  end
  else lblInformacaoLancamento.Caption := 'Confirmação de Registro do(s) Item(s) - Operação '+icxOperacao.Text;
