...
procedure TfrmGerenciaBoletoRemessa.AlterarDatadeVencimentoClick(
  Sender: TObject);
begin
  if Application.MessageBox('Alterar Data de Vencimento do Título Selecionado?','Confirmação', MB_YESNO + MB_ICONQUESTION) = IDNO then
    Exit;
...
begin
  ...
  if not vgRenovarTitulo.Renovar then
    if Application.MessageBox(PChar('Gerar '+vlInfoBoleto+'?'),'Confirmação', MB_YESNO + MB_ICONQUESTION) = IDNO then
      Exit;
...
procedure TfrmGerenciaBoletoRemessa.CancelarTodosBoletosClienteSelecioando1Click(
  Sender: TObject);
begin
  if Application.MessageBox(PChar('Deseja cancelar todos os boletos do CLIENTE Selecionado?'),'Confirmação', MB_YESNO + MB_ICONQUESTION) = IDNO
  then
    Exit;
...
begin
  if Application.MessageBox('Limpar cálculo de Taxas?', 'Confirmação', MB_YESNO) = IDNO then
    exit;
...
var
  ...
  begin
    ...
    if sqlPesqCompromissoENDERECO_CIDADE_ID.AsInteger > 0  then begin
      ...
      Application.MessageBox(Pchar('Cliente '+ sqlPesqCompromissoNome.AsString+' NÃO possui cidade CADASTRADA!'),'Aviso', MB_OK + MB_ICONINFORMATION);
      Abort;
    end;
...
procedure TfrmGerenciaBoletoRemessa.mniReenviarBoletoClick(
  Sender: TObject);
begin
  if Application.MessageBox(Pchar('Adicionar na Lista de Envio por email?'),'Confirmação', MB_YESNO) = IDNO then
    Exit;
  ...
  begin
    ...
    begin
      ...
      if sqlCompromissoVALOR_OUTRA_DEDUCAO.AsCurrency > 0 then
        PreencherObservacao(' - Outras Deduções '+ FormatFloat(',#0.00', sqlCompromissoVALOR_OUTRA_DEDUCAO.AsCurrency));
...
procedure TfrmGerenciaBoletoRemessa.ValidarDataBoleto;
  ...
  begin
    ...
    begin
      Application.MessageBox('Data de Geração está inferior a Data Atual!','Informar', MB_OK + MB_ICONINFORMATION);
      Abort;
    end;
  end;
