...
uses
  ...
  cxCurrencyEdit, cxCheckBox, cxCalendar, Gauges, cxPC, Menus,
  FrameEditor, cxMaskEdit, cxSplitter, cxMemo, cxDBEdit, cxGridLevel,
  ...
  cxGroupBox, ExtCtrls, cxRadioGroup, DBClient, SimpleDS, WPCTRRich,
  ComCtrls, WPRTEPaint, WPRTEDefs, cxScheduler, cxSchedulerStorage,
  cxSchedulerCustomControls, cxSchedulerCustomResourceView,
  cxSchedulerDayView, cxSchedulerDateNavigator, cxSchedulerWeekView,
  cxSchedulerDBStorage, cxSchedulerTimeGridView, cxSchedulerUtils,
  cxSchedulerYearView, dxSkinsCore, dxSkinBlack, dxSkinBlue, dxSkinCaramel,
  ...
  dxSkinscxPCPainter, cxSchedulerHolidays, cxSchedulerGanttView, cxLookAndFeels,
...
type
  ...
  private
    vgValorAuditoria, vgOperacaoAuditoria, vgObsAuditoria: string;
    vpCriouTwain: boolean;
    vpData: TDateTime;
...
begin
  ...
  if (vpPermissao_AcessarAgenda[6] = '1') or (vgLogin = 'I937') then //Tem Permissão
