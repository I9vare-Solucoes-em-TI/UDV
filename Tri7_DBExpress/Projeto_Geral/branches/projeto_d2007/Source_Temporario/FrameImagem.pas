...
uses
  ...
  WPRTEDefs, WPCTRMemo, WPCTRRich, cxLookupEdit, cxDBLookupEdit,
  ...
  FrameEditorSimples, cxGraphics, cxSpinEdit,
  TWNPRO4Lib_TLB, PrintPRO3_TLB, NOTEXP70Lib_TLB,
  ImagXpr7_TLB, dxBar, cxClasses, ExtDlgs, cxProgressBar,
  AdvGlowButton, cxPC, cxCheckBox, FrameAssinaturaDigital, DelphiTwain,
  dxSkinsCore, dxSkinOffice2007Blue, dxSkinscxPCPainter, dxSkinBlue,
  dxSkinBlack, dxSkinCaramel, dxSkinCoffee, dxSkinDarkSide, dxSkinGlassOceans,
  dxSkiniMaginary, dxSkinLilian, dxSkinLiquidSky, dxSkinLondonLiquidSky,
  dxSkinMcSkin, dxSkinMoneyTwins, dxSkinOffice2007Black, dxSkinOffice2007Green,
  dxSkinOffice2007Pink, dxSkinOffice2007Silver, dxSkinPumpkin, dxSkinSilver,
  dxSkinStardust, dxSkinSummer2008, dxSkinsDefaultPainters, dxSkinValentine,
  dxSkinXmas2008Blue, cxLookAndFeels, dxSkinBlueprint, dxSkinDarkRoom,
  ...
  cxPCdxBarPopupMenu, dxSkinDevExpressDarkStyle;

type
  TImagem = Record
    Valor1, Valor2, Valor3, Valor4, Valor5, Valor6, Valor7, Valor8, Valor9, Valor10,
    Pagina, Folha, TipoImpressao, SeloAgrupador : string;
  end;
...
type
  ...
  public
    ...
    procedure ValidarImagem;
    procedure SetEstiloCarimbo(vpTipo : Integer);

  end;

var
  vgImagemImpressa : Boolean;

implementation

uses Controles, Rotinas, MostrarImagem, ControleVariaveis;
...
begin
  ...
  begin
    ...
    else AtualizaImagem(vg_ArqruivoCarimbo);
...
procedure TfmeImagem.btnExcluirClick(Sender: TObject);
begin
if Application.MessageBox('Confirma exclusão desta imagem?', 'Atenção', MB_YESNO + MB_ICONQUESTION) = ID_YES then
  begin
    if Xpress1.Pages = 1 then
    begin
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        Application.ProcessMessages;
        dtmControles.CriarImagem(fmeEditorSimples1.wptTexto, vg_DiretorioAuxiliar + '\Carimbo.tif');
        pnlCarimbo.Visible := False;
      end
      else
      begin
...
procedure TfmeImagem.ValidarImagem;
begin
  if vg_Imagem_Alterada then
    if Application.MessageBox('Imagem alterada e não salva! Deseja continuar mesmo assim?', 'Atenção', MB_YESNO + MB_ICONWARNING) = IDNO then
      Abort;
end;
...
//    RedefinirPadraoFonte(fmeEditorSimples1.wptTexto);
  ...
  begin
    if (strtoint(vg_Imagem.Folha) > 1) and (vg_CabecalhoDiferente) then
    begin
    ...
    if Trim(fmeEditorSimples1.wptTexto.Text) <> '' then
    begin
      dtmControles.CriarImagem(fmeEditorSimples1.wptTexto, vg_DiretorioAuxiliar + '\cabecalho.bmp',2);
  ...
  begin
    if (strtoint(vg_Imagem.Folha) = NPagina) and (vg_RodapeDiferenteUtlPag) then
    begin
    ...
    if Trim(fmeEditorSimples1.wptTexto.Text) <> '' then
    begin
      dtmControles.CriarImagem(fmeEditorSimples1.wptTexto, vg_DiretorioAuxiliar + '\rodape.bmp',2);
...
begin
  ...
  begin
    Inc(viPagAux);
    vg_Imagem.Pagina  := IntToStr(viPagAux);
...
begin
  ...
  begin
    ...
    viExtArquivo := GetExtArquivo(OpenPictureDialog.FileName);
    if UpperCase(viExtArquivo) = 'PDF' then
    begin
      viNomeArquivo := 'c:\Temp\ImgTemp.tiff';
      dtmControles.ConverterPdfToTiff(OpenPictureDialog.FileName, viNomeArquivo)
    end
    else
      viNomeArquivo := OpenPictureDialog.FileName;
...
begin
  NPagina      := Xpress1.Pages; //Número de Paginas
  DiretorioPDF := ExtractFilePath(Application.ExeName) + '\PDF';
  ...
  Xp := TImagXpress.Create(Xpress1);
  Xp.SaveFileName := DiretorioPDF + '\' + vg_NumeroMatricula + '.PDF';
  xp.SaveMultiPage := True;
...
begin
  try
    ...
    begin
      ...
      vg_NomeArquivo := vg_CaminhoDiretorio + icxGrupo.EditValue + '_' + sqlIndexacaoTipoSIGLA.AsString + '_' + RetZeros(edtNumero.Text, 6) + Letra + '.spd';
      btnIndexar.Visible := True;
    end;
