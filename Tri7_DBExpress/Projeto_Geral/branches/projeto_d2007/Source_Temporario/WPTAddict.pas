(*************************************************************
  Addict 3,  (c) 1996-2005, Addictive Software
  Contact: support@addictivesoftware.com
  *************************************************************
  Parser class for WPTools 4 and 5 -  http:/www.wptools.de
  *************************************************************
  WPTools 5 is a powerful suite of components to do word processing,
...
{$ENDIF}
  ad3Spell, ad3SpellBase, ad3ParserBase, ad3Ignore, ad3LiveBase,
  ad3Thesaurus, ad3ThesaurusBase, ad3Configuration;
...
type
  ...
  protected
    ...
    var atr : Cardinal;   var Abort : Boolean);  {$ELSE}
  ...
  protected
    ...
    FLiveMenuOptions: TSpellPopupOptions;
    FAddictSpell: TAddictSpell3Base;
    FAddictThesaurus: TThesaurus3Base;
    FCreatedSpell: TAddictSpell3Base;
    FCreatedThesaurus: TThesaurus3Base;
  protected
    ...
    procedure WriteAddictSpell(AddictSpell: TAddictSpell3Base); virtual;
    procedure WriteAddictThesaurus(AddictThesaurus: TThesaurus3Base); virtual;
    ...
    procedure UpdateInstances; virtual;
  ...
  public
    ...
    property AddictSpell: TAddictSpell3Base read FAddictSpell write WriteAddictSpell;
    property AddictThesaurus: TThesaurus3Base read FAddictThesaurus write WriteAddictThesaurus;
...
procedure CheckWPTools(AddictSpell: TAddictSpell3Base; Control: TWPCustomRtfEdit; CheckType: TCheckType);
procedure LookupWPTools(AddictThesaurus: TThesaurus3Base; Control: TWPCustomRtfEdit);
...
begin
  ...
  else FEdit.TextCursor.SelectedTextAttr.SetCAFSStyle(0, cafsMisSpelled);
end;
{$ELSE}
procedure TWPToolsControlParser.IgnoreWordAttr(MarkCorrect: Boolean);
var
...
begin
  if FEdit.TextCursor.active_posinpar<0 then FEdit.TextCursor.active_posinpar := 0;
...
procedure TWPTAddictInstance.WriteAddictSpell(AddictSpell: TAddictSpell3Base);
begin
...
begin
  ...
  begin
    ...
    begin
      TWPCustomRtfEdit(FLiveControl).SetSelPosLen(Pos, 0{$IFDEF WPTOOLS5},true{$ENDIF});
    end;
  end;
...
begin
  ...
  begin
    ...
    else if (AComponent is TAddictSpell3Base) and (TAddictSpell3Base(AComponent) = FAddictSpell) then
    begin
      FDeleting := True;
      AddictSpell := nil; // calls propwrite
      FDeleting := False;
    end
    else if (AComponent is TThesaurus3Base) and (TThesaurus3Base(AComponent) = FAddictThesaurus) then
    begin
      FDeleting := True;
...
procedure TWPTAddict.WriteAddictSpell(AddictSpell: TAddictSpell3Base);
begin
  if (FAddictSpell <> AddictSpell) then
  begin
    if (Assigned(FAddictSpell)) then
...
procedure TWPTAddict.WriteAddictThesaurus(AddictThesaurus: TThesaurus3Base);
begin
...
procedure TWPTAddict.UpdateInstances;
var
  Index: Integer;
  Recheck: Boolean;
begin
    ...
    case Mode of
      wpStartSpellCheck,
        ...
        begin
          ...
          begin
            FCreatedSpell := TAddictSpell3.Create(Control);
      ...
      wpStartThesuarus:
        begin
          ...
          begin
            FCreatedThesaurus := TThesaurus3.Create(Control);
