object frmControleSelos: TfrmControleSelos
  Left = 155
  Top = 210
  Caption = 'Alterar data dos selos'
  ClientHeight = 464
  ClientWidth = 1015
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poMainFormCenter
  ShowHint = True
  OnClose = FormClose
  OnCreate = FormCreate
  OnKeyPress = FormKeyPress
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 1015
    Height = 464
    Align = alClient
    TabOrder = 0
    object cxGroupBox1: TcxGroupBox
      Left = 1
      Top = 1
      Align = alTop
      Caption = 'Pesquisar'
      ParentBackground = False
      ParentColor = False
      ParentFont = False
      Style.StyleController = dtmControles.cxEditStyleGroup
      TabOrder = 0
      Height = 80
      Width = 1013
      object btnPesquisa: TcxButton
        Left = 402
        Top = 16
        Width = 106
        Height = 26
        Cursor = crHandPoint
        Caption = 'Pesquisar'
        Colors.Default = clWhite
        OptionsImage.Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFF}
        TabOrder = 4
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
        OnClick = btnPesquisaClick
      end
      object Panel3: TPanel
        Left = 840
        Top = 18
        Width = 171
        Height = 60
        Align = alRight
        BevelOuter = bvNone
        Color = 14807280
        ParentBackground = False
        TabOrder = 0
        object btnFechar: TcxButton
          Left = 114
          Top = 4
          Width = 48
          Height = 48
          Cursor = crHandPoint
          Caption = 'Sair'
          Colors.Default = clWhite
          OptionsImage.Glyph.Data = {
            
            0606060606061005000000000000000000000000001F11060606}
          OptionsImage.Layout = blGlyphTop
          ParentShowHint = False
          ShowHint = True
          TabOrder = 0
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          OnClick = btnFecharClick
        end
        object btnDesmarcar: TcxButton
          Left = 4
          Top = 30
          Width = 106
          Height = 19
          Cursor = crHandPoint
          Caption = 'Desmarcar Todos'
          Colors.Default = clBlue
          Colors.Normal = 15513994
          Colors.Hot = 8421440
          Colors.Pressed = clGreen
          Colors.Disabled = clBtnFace
          Enabled = False
          LookAndFeel.NativeStyle = False
          LookAndFeel.SkinName = 'Office2007Pink'
          TabOrder = 1
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          OnClick = btnDesmarcarClick
        end
        object btnMarcar: TcxButton
          Left = 4
          Top = 10
          Width = 106
          Height = 19
          Cursor = crHandPoint
          Caption = 'Marcar Todos'
          Colors.Default = clBlue
          Colors.Normal = 15513994
          Colors.Hot = 8421440
          Colors.Pressed = clGreen
          Colors.Disabled = clBtnFace
          Enabled = False
          LookAndFeel.NativeStyle = False
          LookAndFeel.SkinName = 'Office2007Green'
          TabOrder = 2
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          OnClick = btnMarcarClick
        end
      end
      object cxGroupBox2: TcxGroupBox
        Left = 5
        Top = 13
        Caption = ' Per'#237'odo '
        ParentFont = False
        Style.StyleController = dtmControles.cxEditStyleGroup
        TabOrder = 1
        Height = 61
        Width = 194
        object edtDtIni: TcxDateEdit
          Left = 10
          Top = 26
          ParentFont = False
          Style.StyleController = dtmControles.cxEsc_EditsNormal
          TabOrder = 0
          Width = 80
        end
        object edtDtFim: TcxDateEdit
          Left = 102
          Top = 26
          ParentFont = False
          Style.StyleController = dtmControles.cxEsc_EditsNormal
          TabOrder = 1
          Width = 80
        end
        object cxLabel2: TcxLabel
          Left = 90
          Top = 27
          Caption = 'a'
          ParentFont = False
          Style.StyleController = dtmControles.cxStyleLabel
          Transparent = True
        end
      end
      object btnExportar: TcxButton
        Left = 402
        Top = 47
        Width = 106
        Height = 25
        Cursor = crArrow
        Hint = 'Exportar para XLS'
        Caption = 'Exportar'
        Colors.Default = clWhite
        OptionsImage.Glyph.Data = {
          
          401D0404040404040404}
        ParentShowHint = False
        ShowHint = True
        TabOrder = 5
        Visible = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
        OnClick = btnExportarClick
      end
      object cxGroupBox3: TcxGroupBox
        Left = 519
        Top = 13
        Caption = 'Alterar data dos selos selecionados'
        ParentFont = False
        Style.StyleController = dtmControles.cxEditStyleGroup
        TabOrder = 6
        Height = 61
        Width = 250
        object edtDataSelo: TcxDateEdit
          Left = 11
          Top = 18
          EditValue = '31/12/2018'
          ParentFont = False
          Properties.DateButtons = [btnClear, btnNow, btnToday]
          Properties.Kind = ckDateTime
          Properties.UseNullString = True
          Style.StyleController = dtmControles.cxEsc_EditsNormal
          TabOrder = 0
          Width = 126
        end
        object btnConfirmar: TcxButton
          Left = 143
          Top = 16
          Width = 95
          Height = 22
          HelpType = htKeyword
          Caption = 'Confirmar'
          Colors.Default = clWhite
          LookAndFeel.Kind = lfUltraFlat
          LookAndFeel.NativeStyle = False
          LookAndFeel.SkinName = ''
          OptionsImage.Glyph.Data = {
            
            FFFFFFFFF5F5F5EFF0EF}
          TabOrder = 1
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          OnClick = btnConfirmarClick
        end
        object cbxDesfazer: TcxCheckBox
          Left = 11
          Top = 39
          Caption = 'Desfazer altera'#231#227'o de data'
          ParentFont = False
          Style.StyleController = dtmControles.cxEsc_EditsNormal
          TabOrder = 2
          Width = 200
        end
      end
      object cxGroupBox4: TcxGroupBox
        Left = 205
        Top = 13
        Caption = 'Data Limite'
        ParentFont = False
        Style.StyleController = dtmControles.cxEditStyleGroup
        TabOrder = 2
        Height = 61
        Width = 100
        object edtDataLimite: TcxDateEdit
          Left = 10
          Top = 26
          EditValue = 43465d
          ParentFont = False
          Style.StyleController = dtmControles.cxEsc_EditsNormal
          TabOrder = 0
          Width = 80
        end
      end
      object rdgSistema: TcxRadioGroup
        Left = 311
        Top = 13
        Caption = 'Sistema'
        ParentFont = False
        Properties.Items = <
          item
            Caption = 'RI'
            Value = '2'
          end
          item
            Caption = 'Protesto'
            Value = '5'
          end>
        Properties.OnChange = rdgSistemaPropertiesChange
        ItemIndex = 0
        Style.StyleController = dtmControles.cxEditStyleGroup
        TabOrder = 3
        Height = 61
        Width = 82
      end
    end
    object gridExportar: TcxGrid
      Left = 1
      Top = 81
      Width = 1013
      Height = 382
      Align = alClient
      TabOrder = 1
      ExplicitLeft = 0
      object gridExportarTable: TcxGridDBTableView
        Navigator.Buttons.CustomButtons = <>
        Navigator.Buttons.PriorPage.Visible = False
        Navigator.Buttons.NextPage.Visible = False
        Navigator.Buttons.Insert.Visible = False
        Navigator.Buttons.Delete.Visible = False
        Navigator.Buttons.Edit.Visible = False
        Navigator.Buttons.Post.Visible = False
        Navigator.Buttons.Cancel.Visible = False
        Navigator.Buttons.Refresh.Visible = False
        Navigator.Buttons.SaveBookmark.Visible = False
        Navigator.Buttons.GotoBookmark.Visible = False
        Navigator.Buttons.Filter.Visible = False
        Navigator.Visible = True
        OnCustomDrawCell = gridExportarTableCustomDrawCell
        DataController.DataModeController.GridMode = True
        DataController.DataSource = dtsSelos
        DataController.Summary.DefaultGroupSummaryItems = <>
        DataController.Summary.FooterSummaryItems = <
          item
            Format = ',#0.00'
            Kind = skSum
            FieldName = 'VALOR_EMOLUMENTO'
            Column = cxGridDBEmolumento
          end
          item
            Format = ',#0.00'
            Kind = skSum
            FieldName = 'VALOR_TAXA_JUDICIARIA'
            Column = cxGridDBTaxa
          end
          item
            Format = ',#0.00'
            Kind = skSum
            FieldName = 'VALOR_FUNDESP'
            Column = cxGridDBFundesp
          end
          item
            Format = ',#0'
            Kind = skCount
            FieldName = 'NUMERO_SELO'
            Column = cxGridDBTipoSelo
          end>
        DataController.Summary.SummaryGroups = <>
        OptionsBehavior.ColumnHeaderHints = False
        OptionsCustomize.ColumnGrouping = False
        OptionsData.CancelOnExit = False
        OptionsData.Deleting = False
        OptionsData.DeletingConfirmation = False
        OptionsData.Inserting = False
        OptionsView.ColumnAutoWidth = True
        OptionsView.Footer = True
        OptionsView.GroupByBox = False
        OptionsView.GroupFooters = gfAlwaysVisible
        Styles.StyleSheet = dtmControles.GridTableViewStyleSheetDevExpress
        object gridExportarTableColumn3: TcxGridDBColumn
          DataBinding.FieldName = 'DATA_AUX'
          Visible = False
        end
        object gridExportarTableColumn4: TcxGridDBColumn
          DataBinding.FieldName = 'DATA_MODIFICADA'
          Visible = False
        end
        object gridExportarTableColumn1: TcxGridDBColumn
          Caption = 'Selecionar'
          DataBinding.FieldName = 'CALC_SELECIONADO'
          PropertiesClassName = 'TcxCheckBoxProperties'
          Properties.ValueGrayed = 'False'
          Width = 28
        end
        object cxGridDBServentia: TcxGridDBColumn
          Caption = 'Serventia'
          DataBinding.FieldName = 'TIPO_CARTORIO'
          PropertiesClassName = 'TcxImageComboBoxProperties'
          Properties.Items = <
            item
              Description = 'Tabelionato de Notas'
              ImageIndex = 0
              Value = '1'
            end
            item
              Description = 'Registro de Im'#243'veis'
              Value = '2'
            end
            item
              Description = 'Registro Civil'
              Value = '3'
            end
            item
              Description = 'RTD'
              Value = '4'
            end
            item
              Description = 'Protesto'
              Value = '5'
            end>
          Visible = False
          GroupIndex = 0
          Options.Editing = False
          Width = 86
        end
        object cxGridDBTableData: TcxGridDBColumn
          Caption = 'Data Selo'
          DataBinding.FieldName = 'DATA'
          PropertiesClassName = 'TcxDateEditProperties'
          Properties.SaveTime = False
          Options.Editing = False
          Width = 65
        end
        object cxGridDBProtocolo: TcxGridDBColumn
          Caption = 'Protocolo/Pedido'
          DataBinding.FieldName = 'CALC_PROTOCOLO'
          PropertiesClassName = 'TcxTextEditProperties'
          Properties.Alignment.Horz = taCenter
          Options.Editing = False
          Width = 97
        end
        object cxGridDBTipoSelo: TcxGridDBColumn
          Caption = 'Natureza do Selo (Ato)'
          DataBinding.FieldName = 'TIPO_SELO'
          PropertiesClassName = 'TcxTextEditProperties'
          Properties.Alignment.Horz = taLeftJustify
          Options.Editing = False
          Width = 219
        end
        object cxGridDBTableApresentante: TcxGridDBColumn
          Caption = 'Nome Civil'
          DataBinding.FieldName = 'APRESENTANTE'
          Options.Editing = False
          Width = 201
        end
        object cxGridDBTableNumeroSelo: TcxGridDBColumn
          Caption = 'Numero Selo'
          DataBinding.FieldName = 'NUMERO_SELO'
          Options.Editing = False
          Width = 133
        end
        object cxGridDBEmolumento: TcxGridDBColumn
          Caption = 'Emolumentos'
          DataBinding.FieldName = 'VALOR_EMOLUMENTO'
          PropertiesClassName = 'TcxCurrencyEditProperties'
          Options.Editing = False
          Width = 93
        end
        object cxGridDBTaxa: TcxGridDBColumn
          Caption = 'Taxa Jud.'
          DataBinding.FieldName = 'VALOR_TAXA_JUDICIARIA'
          PropertiesClassName = 'TcxCurrencyEditProperties'
          Options.Editing = False
          Width = 70
        end
        object cxGridDBFundesp: TcxGridDBColumn
          Caption = 'Fundos'
          DataBinding.FieldName = 'VALOR_FUNDESP'
          PropertiesClassName = 'TcxCurrencyEditProperties'
          Options.Editing = False
          Width = 88
        end
        object gridExportarTableColumn2: TcxGridDBColumn
          DataBinding.FieldName = 'DATA_EXPORTACAO'
          Visible = False
          Options.Editing = False
        end
      end
      object cxGridLevel2: TcxGridLevel
        GridView = gridExportarTable
      end
    end
  end
  object sqlSelos: TI9Query
    Aggregates = <>
    Connection = dtmControles.DB
    SQL.Strings = (
      'SELECT CAST(SL.DATA AS DATE) AS DATA,'#13#10'       SL.CAMPO_ID,'#13#10'    ' +
      '   SL.TABELA,'#13#10'       SG.DESCRICAO_COMPLETA AS TIPO_SELO,'#13#10'     ' +
      '  SL.APRESENTANTE,'#13#10'       SL.DESCRICAO,'#13#10'       LPAD(CAST(SL.NU' +
      'MERO AS INT), 6, '#39'0'#39') AS NUMERO_SELO,'#13#10'       SO.SIGLA AS LOTE_S' +
      'ELO,'#13#10'       SL.VALOR_EMOLUMENTO,'#13#10'       SL.VALOR_TAXA_JUDICIAR' +
      'IA,'#13#10'       SL.VALOR_FUNDESP,'#13#10'       SG.TIPO_CARTORIO,'#13#10'       ' +
      'SL.DATA_EXPORTACAO,'#13#10'       SL.SELO_LIVRO_ID,'#13#10'       SL.DATA_AU' +
      'X,'#13#10'       SL.DATA_MODIFICADA'#13#10'FROM G_SELO_LIVRO SL'#13#10'  LEFT JOIN' +
      ' G_SELO_LOTE SO ON'#13#10'  SL.SELO_LOTE_ID = SO.SELO_LOTE_ID'#13#10'  LEFT ' +
      'JOIN G_SELO_GRUPO SG ON'#13#10'  SO.SELO_GRUPO_ID = SG.SELO_GRUPO_ID'#13#10 +
      'WHERE NOT SG.TIPO_CARTORIO IS NULL'#13#10'  AND SL.SELO_LIVRO_ID = 0')
    ParamData = <>
    Filter = 'CALC_SELECIONADO = TRUE'
    Params = <>
    Left = 460
    Top = 157
    object sqlSelosCAMPO_ID: TBCDField
      FieldName = 'CAMPO_ID'
      Precision = 15
      Size = 2
    end
    object sqlSelosTABELA: TStringField
      FieldName = 'TABELA'
      Size = 30
    end
    object sqlSelosTIPO_SELO: TStringField
      FieldName = 'TIPO_SELO'
      Size = 260
    end
    object sqlSelosAPRESENTANTE: TStringField
      FieldName = 'APRESENTANTE'
      Size = 120
    end
    object sqlSelosDESCRICAO: TStringField
      FieldName = 'DESCRICAO'
      Size = 120
    end
    object sqlSelosNUMERO_SELO: TStringField
      FieldName = 'NUMERO_SELO'
      Size = 6
    end
    object sqlSelosLOTE_SELO: TStringField
      FieldName = 'LOTE_SELO'
      Size = 30
    end
    object sqlSelosVALOR_EMOLUMENTO: TBCDField
      FieldName = 'VALOR_EMOLUMENTO'
      Precision = 15
      Size = 3
    end
    object sqlSelosVALOR_TAXA_JUDICIARIA: TBCDField
      FieldName = 'VALOR_TAXA_JUDICIARIA'
      Precision = 15
      Size = 3
    end
    object sqlSelosCALC_SELECIONADO: TBooleanField
      FieldKind = fkInternalCalc
      FieldName = 'CALC_SELECIONADO'
    end
    object sqlSelosVALOR_FUNDESP: TBCDField
      FieldName = 'VALOR_FUNDESP'
      Precision = 15
      Size = 3
    end
    object sqlSelosTIPO_CARTORIO: TStringField
      FieldName = 'TIPO_CARTORIO'
      Size = 3
    end
    object sqlSelosCALC_PROTOCOLO: TStringField
      DisplayWidth = 10
      FieldKind = fkInternalCalc
      FieldName = 'CALC_PROTOCOLO'
      Size = 10
    end
    object sqlSelosDATA: TDateField
      FieldName = 'DATA'
    end
    object sqlSelosDATA_EXPORTACAO: TSQLTimeStampField
      FieldName = 'DATA_EXPORTACAO'
    end
    object sqlSelosSELO_LIVRO_ID: TBCDField
      FieldName = 'SELO_LIVRO_ID'
      Required = True
      Precision = 20
      Size = 2
    end
    object sqlSelosDATA_AUX: TSQLTimeStampField
      FieldName = 'DATA_AUX'
    end
    object sqlSelosDATA_MODIFICADA: TStringField
      FieldName = 'DATA_MODIFICADA'
      Size = 1
    end
  end
  object dtsSelos: TDataSource
    DataSet = sqlSelos
    Left = 525
    Top = 153
  end
end
