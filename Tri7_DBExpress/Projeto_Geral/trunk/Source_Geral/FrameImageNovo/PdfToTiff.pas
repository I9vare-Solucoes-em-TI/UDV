...
uses
  ...
  gtCstPDFDoc,
  gtExPDFDoc, gtExProPDFDoc, gtPDFDoc, iemview, imageenview, imageenio,
  imageenproc,
  Graphics, Dialogs, hyiedefs, dxBarExtItems;

function ConverterPdfToTiff(vpOrigem, vpDestino: string;
  vpProgressBar: TdxBarProgressItem): Integer;
procedure ConverterToTiff(vpOrigem, vpDestino: string;
  vpProgressBar: TdxBarProgressItem);
...
var
  Bmp: TBitmap;
  Output_DPI, Desktop_DPI, viPaginas, i: Integer;
...
begin
  ...
  try
    ...
    begin
      ...
      gtPDFDocument.RenderToDC(Bmp.Canvas.Handle, // Handle of the bitmap canvas
...
procedure ParticionarPDF(vpArquivo: String; vpTamanhoPDF: Integer);
Const
  PaperWidth: array [0 .. 11] of Integer = (2380, 1684, 1190, 842, 595, 421,
    297, 501, 612, 612, 1224, 792);
  PaperHeight: array [0 .. 11] of Integer = (3368, 2380, 1684, 1190, 842, 595,
    421, 709, 792, 1008, 792, 1224);
  PDFCompression: array [0 .. 3] of TIOPDFCompression = (ioPDF_RLE,
    ioPDF_G3FAX2D, ioPDF_G4FAX, ioPDF_JPEG);

var
  ImageEnViewPDF: TImageEnView;
  i, viIdx, viSize, viPaginas: Integer;
  str: TMemoryStream;
  viArquivo, viArquivoTiff: String;
begin
  ...
  viArquivo := Copy(vpArquivo, 1, Pos('.PDF', AnsiUpperCase(vpArquivo))
    - 1) + '_';
  ...
  ImageEnViewPDF.IO.CreatePDFFile(viArquivo + FormatFloat('000', viIdx)
    + '.pdf');
