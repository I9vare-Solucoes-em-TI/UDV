...
type
  ...
  private
    ...
    procedure AtualizarPessoaTabelionato;
  public
    { Public declarations }
...
uses Cadastro, Controles, Pesquisa, Rotinas, Lookup, CapturarBiometria,
  RotinasFingkey, CapturaFoto, HoraExtra;
...
procedure TfrmUsuario.AtualizarPessoaTabelionato;
begin
...
begin
  inherited;
  pnlInformação.Caption := 'Nenhuma Grupo e/ou Usuário Selecionado';
  ...
  if rdbGrupo.Checked then
       pnlInformação.Caption := '    Permissão do Grupo '+AnsiUpperCase(lcbGruposUsuario.Text) + ' - '+MaiusculoMinusculo(tbcSistemas.Tabs[tbcSistemas.TabIndex].Caption)
  else pnlInformação.Caption := '    Permissão Individual do Usuário '+AnsiUpperCase(lcbGruposUsuario.Text) + ' - '+MaiusculoMinusculo(tbcSistemas.Tabs[tbcSistemas.TabIndex].Caption);
end;
