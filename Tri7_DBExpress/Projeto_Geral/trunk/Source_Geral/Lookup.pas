...
type
  ...
  TdtmLookup = class(TDataModule)
    ...
    sqlG_TB_Orgao_Expedidor: TSimpleDataSet;
    ...
    sqlTipoCartaoQTD_PARCELA: TFMTBCDField;
    ...
    sqlG_TB_DocumentoTipoPrincipal: TSimpleDataSet;
    ...
    sqlG_TB_DocumentoTiposOutros: TSimpleDataSet;
    ...
    sqlUsuariosTodos: TSimpleDataSet;
    sqlUsuariosTodosUSUARIO_ID: TFMTBCDField;
...
var
  ...
  var
    ...
    begin
      ...
      if CharInSet(vpEspecie[1],['D', 'C','O']) then
        ...
        case vpEspecie[1] of
          ...
          'T' : viSql := viSql + ' AND (((SITUACAO = ''9'') OR (((SITUACAO = ''4'') OR (SITUACAO = ''10'')) and (ESPECIE_PAGAMENTO ='+QuotedStr(vpEspecie)+')))'+
                                 ' OR (((SITUACAO = ''10'') OR (SITUACAO = ''11'')) AND (ESPECIE_PAGAMENTO = '+ QuotedStr('')+')))';
...
{$ENDREGION}
begin
  viValoresFundos := GetValoresFundos(vpValorEmolumento, vpQuantidade);
  try
    Result := TCalculadoraSeloFundos.Get(viValoresFundos, GetFundoTipos);
  finally
    viValoresFundos.Free;
  end;
end;
...
begin
  Result := TConfiguradorFundo.RemoverExcludentes(GetFundoTipos, vpEmolumento);
end;
