...
begin
  ...
  begin
    ...
    Begin
      ...
      frmBematech.Caption := 'Aplicativo de teste usando a API de comunicação e o driver de spooler';
    ...
    Begin
      ...
      frmBematech.Caption := 'Test application using the API of comunication and spooler driver';
  ...
  Begin
    ...
    Begin
        ...
        case retorno of
          0 : // imp. desligada ou cabo desc. na LPT
              if ( ComboBox1.ItemIndex = 0 ) then
                ...
                ThreadMemo.Text := 'Off-line ou Fim de papel';
        32 : // pouco papel e off-line na LPT
              if ( ComboBox1.ItemIndex = 0 ) then
                ThreadMemo.Text :='Pouco papel e off-line'
              else if ( (ComboBox1.ItemIndex = 1) or (ComboBox1.ItemIndex = 2) ) then// fim de papel na serial
                ThreadMemo.Text := 'Off-line ou Fim de papel'
              else //usb
      ...
      case retorno of
        0 : // imp. desligada ou cabo desc. na LPT
            if ( ComboBox1.ItemIndex = 0 ) then
              ThreadMemo.Text := 'Off or detached handle'
            else // off-line na serial
              ThreadMemo.Text := 'Off-line or end of paper';
        32 : // pouco papel e off-line na LPT
            if ( ComboBox1.ItemIndex = 0 ) then
              ThreadMemo.Text := 'Low paper and off-line'
            else // fim de papel na serial
              ThreadMemo.Text := 'Off-line or end of paper';
        4  : // pouco papel e off-line na serial
            ThreadMemo.Text := 'Low paper and off-line';
        40 : // fim de papel na LPT
            ThreadMemo.Text := 'End of paper';
        5,  48 : // 5 = pouco papel serial e 48 na LPT
            ThreadMemo.Text := 'Low paper and on-line';
        79 : // off-line na LPT
            ThreadMemo.Text := 'Off-line';
        9, 128 : // 9 = head-up na serial e 128 na LPT
            ThreadMemo.Text := 'Head Up';
        24, 144 : // 24 = on-line na serial e 144 na LPT
            ThreadMemo.Text := 'Printer ok'; // 24 (COM) e 144 (LPT)
        Else //Se não for nenhum dos status acima
          ThreadMemo.Text := 'Unknown status';
      end;
    End;
  end
end;
