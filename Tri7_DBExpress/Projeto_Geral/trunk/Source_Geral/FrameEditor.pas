...
uses
 Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms,
  ...
  dxSkinOffice2007Silver, dxSkinsDefaultPainters, WPOBJ_Image;
...
var
  ...
  begin
    if pos('/', viCaminho) > 0 then
      Result := '/'
    else
      Result := '\';
  end;
begin
  viCaminho     := dtmControles.BuscarConfig('PRINCIPAL','DICIONARIO','LOCAL_DICIONARIO','S');
  viNomeArquivo := dtmControles.BuscarConfig('PRINCIPAL','DICIONARIO','NOME_ARQUIVO_DICIONARIO','S');

  dtmControles.AddictSpell.ConfigDefaultMain.Clear;
  dtmControles.AddictSpell.ConfigDefaultMain.Add('brazport.adm');

  dtmControles.AddictSpell.ConfigFilename := viCaminho + getBarra + viNomeArquivo + '-spell.cfg';
  dtmControles.AddictSpell.SuggestionsLearningDict := viCaminho + getBarra + viNomeArquivo + '_sp.adl';

  dtmControles.AddictSpell.SuggestionsLearning := True;
  dtmControles.AddictSpell.ConfigDictionaryDir.Clear;
  dtmControles.AddictSpell.ConfigDictionaryDir.Add(viCaminho);
  dtmControles.AddictSpell.ConfigDefaultActiveCustom := viNomeArquivo + '.adu';

  WPAddict.AddictSpell := dtmControles.AddictSpell;
  ...
  wptTexto.CPPosition := 1; // force ruler update!
