  ...
  case vgId of
   ...
   7, 12, 20: begin
         ...
         begin
           ...
           begin
             DataSetAncestral.CommandText := DataSetAncestral.CommandText +  ' FROM C_MINUTA ';
             vlSigla_Sistema := 'C';
           end;
...
begin
  ...
  dtmControles.SimpleAuxiliar.SaveToFile(ExtractFilePath(Application.ExeName) + vlSigla_Sistema + '_XML_MINUTAS.XML', dfXMLUTF8);
...
begin
  ...
  sqlImportar.DataSet.CommandText := ' SELECT * FROM ' + vlSigla_Sistema + '_MINUTA ';
  ...
  begin
    try
      sqlImportar.Insert;
      sqlImportar.FieldByName('MINUTA_ID').AsInteger   := sqlXmlAtualizador.FieldByName('MINUTA_ID').AsInteger;
