...
uses
  ...
  Dialogs, CadBasico, DB, DBClient, SqlExpr, ComCtrls, StdCtrls, cxButtons,
  ExtCtrls, cxEdit, WPCTRRich, WPRTEDefs, WPCTRMemo, cxGridLevel,
  cxGridDBTableView, cxGrid, cxSplitter, cxDBEdit, cxContainer, cxLabel,
  cxImageComboBox, Menus, cxDropDownEdit, FrameEditor, cxGroupBox,
  SimpleDS, cxGraphics, cxLookAndFeels, cxLookAndFeelPainters, dxSkinsCore,
  dxSkinBlack, dxSkinCaramel, dxSkinCoffee, dxSkinLiquidSky,
  dxSkinLondonLiquidSky, dxSkinMcSkin, dxSkinOffice2007Black, FMTBcd,
  ...
  cxClasses, cxGridCustomView, cxTextEdit, cxMaskEdit, cxCheckBox, Provider,
  ActnList;
...
uses Controles, Rotinas, ControleVariaveis, 
...
begin
  ...
  begin
    ...
    if (viTipoValor[1] in ['F','C','L','0']) then
    begin
    ...
    // Campos Separadores
    if (viTipoValor[1] in ['F', 'P']) or (icxTipoQualificacao.Enabled) then
