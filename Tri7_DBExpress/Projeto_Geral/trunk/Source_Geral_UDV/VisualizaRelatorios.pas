...
uses
  ...
  frxExportTXT, ComCtrls, Controles, DBClient, SimpleDS, frxRichEditor,
  cxLookAndFeelPainters, cxButtons, Printers, cxGraphics, cxLookAndFeels,
  frxDesgn, frxRich, frxEditMemo, dxSkinsCore, dxSkinBlack, dxSkinBlue,
...
var
  ...
  procedure ShowPreview(Nome : String; Report: TfrxReport; WindowState:TWindowState = wsNormal);
  procedure ExibirRelatorio(frxReport : TfrxReport; ID : String; ExecProcedure : TExecuteProcedure = nil;
               VisualizarImpressao: Boolean = True; vpImpressora : String = ''; vpSolicitarImpressora : Boolean = False);
  ...
  function ExecutarFuncoesRelatorio(NameFuncao: String; var Parametros: Variant): Variant;
...
begin
  ...
  frmVisualizaRelatorios.frxPreview.ZoomMode := zmPageWidth;
...
begin
  ...
  begin
    Sql := 'SELECT RELATORIO, INTERNO FROM G_CONFIG_RELATORIO ' +
    ...
    else frxReport.PrintOptions.Printer := printer.Printers[Printer.PrinterIndex];
    ...
    begin
      ...
      GravarRelatorio(ID);
         ...
         if VisualizarImpressao then
            ShowPreview('Preview',frxReport, wsMaximized)
         else
         begin
...
procedure TfrmVisualizaRelatorios.spbPreview100PercentClick(
  Sender: TObject);
begin
  frxPreview.Zoom := 1;
  UpdateZoom;
...
procedure TfrmVisualizaRelatorios.frxPreviewPageChanged(
  Sender: TfrxPreview; PageNo: Integer);
begin
  StatusBar.Panels[0].Text := 'Páginas : ' + IntToStr(frxPreview.PageNo) + ' de ' +
                              IntToStr(frxPreview.PageCount);
end;
...
procedure CarregaRelatorio(ID, Relatorio: String;
  ...
  begin
    ...
    if vgVersao2 <> 0 then
       ...
       begin
         ...
         begin
           if Application.MessageBox('Deseja gravar essa alterção?', 'Atenção', MB_YESNO + MB_ICONQUESTION) = IDYES then
           begin
             Sql := 'UPDATE G_CONFIG_RELATORIO SET ' +
                  '       RELATORIO = :RELATORIO ';
             if FileExists(Historico) then
             begin
               if Application.MessageBox('Deseja gravar o histórico dessa alterção?', 'Atenção', MB_YESNO + MB_ICONQUESTION) = IDYES then
               begin
...
procedure CriarFuncoesRelatorio(frRelatorio: TfrxReport);
begin
  frRelatorio.AddFunction('function BuscarConfig(vpGrupo, vpSecao, vpNome : string; vpTipoValor: char): Variant', 'I937', 'Retorna o valor dos parâmetros de configuração do sistema');
  frRelatorio.AddFunction('function ExecSQL(Qry: string):Boolean', 'I937', 'Executa o SQL passado como parâmetro');
  frRelatorio.AddFunction('function GetStr(Qry: string): string', 'I937', 'Retorna o valor do SELECT passado no formato de STRING');
  frRelatorio.AddFunction('function RetornaNumerico(Valor : String) : String', 'I937', 'Retorna apenas os valores numericos no formato STRING');
  frRelatorio.AddFunction('function QuotedStr(Valor : String) : String', 'I937', 'Retorna uma STRING com delimitadores');
  frRelatorio.AddFunction('function TrocaVirgPPto(Valor: string): String', 'I937', 'Troca as "," por "."');
  frRelatorio.AddFunction('function ValorExtenso(Valor: Double; TipoExtenso : Integer = 3): String', 'I937', 'Retorna o valor por extenso');
  frRelatorio.AddFunction('procedure RetornaValores(Valor: String)', 'I937', 'Retorna valores internos para a aplicação');
  frRelatorio.AddFunction('procedure MaiusculoMinusculo(Valor: String)', 'I937', 'Retorna Nome em Maiusculo/Minusculo');
  frRelatorio.AddFunction('procedure RetornaCPFCNPJFormatado(Valor: String)', 'I937', 'Retorna CPF/CNPJ Formatado');
end;


function ExecutarFuncoesRelatorio(NameFuncao: String; var Parametros: Variant): Variant;
var TipoExtenco: Integer;
begin
 Result := Null;
 ...
 if AnsiSameText(NameFuncao, 'ExecSQL') then
    Result := dtmControles.ExecSQL(Parametros[0])
 else
 if AnsiSameText(NameFuncao, 'GetStr') then
    Result := dtmControles.GetStr(Parametros[0])
 else
 if AnsiSameText(NameFuncao, 'RetornaNumerico') then
    Result := RetornaNumerico(Parametros[0])
 else
 if AnsiSameText(NameFuncao, 'QuotedStr') then
    Result := QuotedStr(Parametros[0])
 else
 if AnsiSameText(NameFuncao, 'TrocaVirgPPto') then
    Result := TrocaVirgPPto(Parametros[0])
 else
 if AnsiSameText(NameFuncao, 'MaiusculoMinusculo') then
    Result := MaiusculoMinusculo(Parametros[0])
 else
 if AnsiSameText(NameFuncao, 'RetornaCPFCNPJFormatado') then
    Result := RetornaCPFCNPJFormatado(Parametros[0])
 else
 if AnsiSameText(NameFuncao, 'ValorExtenso') then
 begin
   TipoExtenco := Parametros[1];
   Result := MaiusculoMinusculo(Trim(ValorExtenso(Parametros[0], '', '', TipoExtenco)));
 end
 else
 if AnsiSameText(NameFuncao, 'RetornaValores') then
...
begin
  ...
  begin
    if  AnsiUpperCase(TfrxRichView(Sender).TagStr) = 'EDITAR' then
      with TfrxRichEditorForm.Create(frmVisualizaRelatorios) do
      begin
