...
type
  ...
  private
    ...
    constructor Create(
      ...
      const vpConnection: TCRSQLConnection); reintroduce;
  ...
  public
    ...
    class function New(
      const vpEmailConfigID: Integer;
      const vpConnection: TCRSQLConnection): IFactory<IEnvioEmail>;
  end;
...
constructor TEnvioEmailFactory.Create(
  const vpEmailConfigID: Integer;
  const vpConnection: TCRSQLConnection);
begin
  inherited Create;
...
{$ENDREGION}
begin
  viQuery := TSimpleDataSet.Create(
    nil);

  Result := viQuery;
  viQuery.Connection := FConnection;

  viQuery.DataSet.CommandText := {$REGION 'Comando SQL SELECT'}
    'SELECT ' +
