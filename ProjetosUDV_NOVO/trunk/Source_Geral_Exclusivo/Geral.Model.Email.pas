...
var
  viQuery: TSimpleDataSet;
begin
  viQuery := nil;
  try
    viQuery := TSimpleDataSet.Create(nil);
    viQuery.Connection := FConnection;
    FEmailConfig := viQuery;

    viQuery.DataSet.CommandText := {$REGION 'SQL'}
      'SELECT ' +
    ...
    viQuery.DataSet.ParamByName('P_EMAIL_CONFIG_ID').AsInteger := FEmailConfigID;
...
{    viQuery.DataSet.CommandText := 'SELECT EMAIL_CONFIG_ID '+
                        'FROM G_EMAIL_CONFIG '+
 ...
 {   if Result > 0 then
  ...
  {  viQuery.DataSet.CommandText := 'SELECT FIRST 1 EMAIL_CONFIG_ID '+
                        'FROM G_EMAIL_CONFIG '+
                        'WHERE PADRAO = ''S'' ';
    viQuery.Open;
