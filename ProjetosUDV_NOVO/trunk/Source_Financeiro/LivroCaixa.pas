...
uses
  ...
  FN.Model.GED, Geral.Model.GED, frxRich;
...
type
  ...
  TfrmLivroCaixa = class(TForm)
    ...
    sqlLivroCaixaASSOCIADO_ID: TFMTBCDField;
    sqlLivroCaixaCOMPROMISSO_VENCIDO_ID: TFMTBCDField;
    sqlLivroCaixaDATA_LANCAMENTO: TSQLTimeStampField;
    sqlLivroCaixaIDENTIFICACAO_TIPO: TStringField;
    sqlLivroCaixaRESERVA_ID: TFMTBCDField;
    ...
    sqlLivroCaixaTIPO_DESPESA_ID: TFMTBCDField;
    ...
    sqlCaixaSimplificado: TSimpleDataSet;
    sqlCaixaSimplificadoCAIXA_ID: TFMTBCDField;
    ...
    sqlCaixaSimplificadoVALOR: TFMTBCDField;
    ...
    sqlLivroCaixaIDENTIFICACAO_ID: TFMTBCDField;
    ...
    sqlReciboIDENTIFICACAO_ID: TFMTBCDField;
    ...
    sqlCaixaSimplificadoORDEM: TFMTBCDField;
    ...
    sqlCaixaSimplificadoCAIXA_TRANSFERENCIA_ID: TFMTBCDField;
    cxGridDBTableView1Column9: TcxGridDBColumn;
    cxGridDBTableView1Column10: TcxGridDBColumn;
    sqlCaixaSimplificadoIDENTIFICACAO_ID: TFMTBCDField;
    ...
    sqlTransfProvisao: TSimpleDataSet;
    dtsTransfProvisao: TDataSource;
    sqlTransfProvisaoLIVRO_CAIXA_ID: TFMTBCDField;
    ...
    sqlTransfProvisaoRESERVA_ID: TFMTBCDField;
    sqlTransfProvisaoVALOR: TFMTBCDField;
  ...
  private
    ...
    procedure ExcluirItem(vpSimpleDataSet : TSimpleDataSet);
...
begin
  ...
  begin
    Application.MessageBox(Pchar('Os registros devem estar ordenados por data.'+#13+#13+
                                 'Pressione no Título do Campo data para ordenar!!!'), 'Atenção', MB_OK + MB_ICONWARNING);
    exit;
  end;
  ...
  begin
    ...
    Application.MessageBox(Pchar('Data não pode ser alterada!!!'), 'Atenção', MB_OK + MB_ICONWARNING);
    exit;
  end;
...
begin
  ...
  dtmLookupContabil.CarregarSaldoVisualizacao(dxBarStaticSaldoAnterior,   'Saldo Mês Anterior R$ ', vlSaldoMesAnterior);
end;
...
begin
  ...
  begin
    Application.MessageBox(Pchar('Alteração não permitida. Livro Caixa Fechado!!!'), 'Atenção', MB_OK + MB_ICONWARNING);
    exit;
  end;
  ...
  begin
    Application.MessageBox(Pchar('Item não pode ser alterado!!!'), 'Atenção', MB_OK + MB_ICONWARNING);
    exit;
  end;
  ...
  begin
    // Verifica se Não é Agendamento Parcial de Agendamento;
    ...
    begin
      ...
      begin
        Application.MessageBox(Pchar('Item não pode ser alterado.'+ #13+#13+
                                     'Estorne e Lance Novamente!!!'),'Atenção', mb_Ok + MB_ICONEXCLAMATION);
        exit;
      end;
    end;
    ...
    begin
      Application.MessageBox(Pchar('Item só pode ser alterado pelo Gerenciador de Receitas!!!'), 'Atenção', MB_OK + MB_ICONWARNING);
      exit;
    end;
...
begin
  if pgcVisualizacao.ActivePageIndex = 0 then
    ...
    Application.MessageBox(Pchar('Exclusão somente pela Visualização Detalhada!!!'), 'Atenção', MB_OK + MB_ICONWARNING);
end;
...
begin
  Application.MessageBox(Pchar('Alteração somente pela visualização detalhada!!!'), 'Atenção', MB_OK + MB_ICONWARNING);
...
begin
  EmitirReciboComprovante('R','Emissão de Recibo');
end;
...
var
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          Application.MessageBox(Pchar('Item(s) Selecionado(S) devem ter o "Tipo Doc = '+ vpMsg+'"!!!'), 'Atenção', mb_Ok + MB_ICONEXCLAMATION);
          abort;
        end;
        ...
        begin
          ...
          begin
            Application.MessageBox('Item(s) com numero de recibo/comp. diferente".!!!', 'Atenção', mb_Ok + MB_ICONEXCLAMATION);
            abort;
          end;
        end;
      end;
  ...
  begin
    ...
    begin
      Application.MessageBox(Pchar('Comprovante/Recibo não Configurado!!!'), 'Atenção', MB_OK + MB_ICONWARNING);
      abort;
    end;
  end;
...
begin
  ...
  sqlRecibo.DataSet.ParamByName('IDENTIFICACAO_ID').AsInteger := viReciboCompId;
...
procedure TfrmLivroCaixa.ExcluirItem(vpSimpleDataSet: TSimpleDataSet);
var
  viMensagem, viCaixaTransferenciaID, viSql : String;
begin
  if vpSimpleDataSet.FieldByName('SITUACAO').AsString = 'F' then
  begin
    Application.MessageBox(Pchar('Exclusão não permitida. Livro Caixa Fechado!!!'), 'Atenção', MB_OK + MB_ICONWARNING);
    exit;
  end;

  if (sqlLivroCaixaLIVRO_FINANCEIRO_ID.AsInteger > 0) or (sqlLivroCaixaINTERNO.AsString = 'S') then
  begin
    Application.MessageBox(Pchar('Item de Agendamento não pode ser excluído.'+#13+#13+ 'Estorne e/ou Cancele o Item!!!'), 'Atenção', MB_OK + MB_ICONWARNING);
    exit;
  end;

  if (sqlLivroCaixaCOMPROMISSO_VENCIDO_ID.AsInteger > 0) then
  begin
    Application.MessageBox(Pchar('Item só pode ser excluído e/ou estornado pelo Gerenciador de Receitas!!!'), 'Atenção', MB_OK + MB_ICONWARNING);
    exit;
  end;
...
begin
  if sqlLivroCaixaSITUACAO.AsString = 'F' then
  begin
    Application.MessageBox(Pchar('Estorno não permitido. Livro Caixa Fechado!!!'), 'Atenção', MB_OK + MB_ICONWARNING);
    exit;
  end;
...
begin
  if Application.MessageBox('Definir Item nos Lançamentos Favoritos?', 'Confirmação', MB_YESNO) = IDNO then
    exit;
...
begin
  ...
  if viRelatorio = '' then
  begin
    Application.MessageBox(Pchar('Relatório simplificado não configurado!!!'), 'Atenção', MB_OK + MB_ICONWARNING);
    exit;
  end;
  dtmLookup.CarregarCabecalho(1);
...
var
  ...
  begin
    ...
    sqlTransfProvisao.DataSet.CommandText := viSql;
...
begin
  ...
  visql := ' SELECT SUM(LC.VALOR) AS VALOR, LC.CAIXA_ID, LC.OPERACAO, LC.DATA_PAGAMENTO, LC.SITUACAO, DOCUMENTO_NUMERO, DOC_TIPO,'+
           ...
           '       WHEN (OPERACAO = ''TD'') THEN ''Saída - '' || LC.HISTORICO '+
  ...
  sqlCaixaSimplificado.DataSet.CommandText := viSql;
...
procedure TfrmLivroCaixa.rdbPesquisaPersonalizadaClick(Sender: TObject);
  ...
  begin
    // Verificar Provisão Itens Duplicados (Reserva no Livro Caixa e Reserva no Lancamento_compromisso)
//    vlListaProvisao := '';
...
begin
  if Application.MessageBox('Resetar/Refazer Campos da Grid?', 'Confirmação', MB_YESNO) = IDNO then
    exit;

  ExecutaSqlAuxiliar(' DELETE FROM J_GRID WHERE FORM = '+QuotedStr('LIVRO_CAIXA'),1);
  Application.MessageBox('Reabra o Formulário para ativar as configurações da Grid!!!', 'Informação', mb_Ok + MB_ICONEXCLAMATION);
...
begin
  ...
  else lblCancelarComprovante.Caption := 'Cancelar Emissão do Recibo';
