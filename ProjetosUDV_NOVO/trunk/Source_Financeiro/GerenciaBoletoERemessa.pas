...
type
  TfrmGerenciaBoletoRemessa = class(TForm)
    ...
    sqlCompromissoLIVRO_REMUNERACAO_ID: TFMTBCDField;
    sqlCompromissoFINANCEIRO_REMUNERACAO_ID: TFMTBCDField;
    ...
    sqlPesqCompromissoBOLETA_ID: TFMTBCDField;
    ...
    sqlPesqCompromissoCONTABIL_CONTA_ID: TFMTBCDField;
    ...
    sqlPesqCompromissoCAIXA_ID: TFMTBCDField;
    ...
    sqlPesqCompromissoLIVRO_FINANCEIRO_ID: TFMTBCDField;
...
begin
  ...
  if Application.MessageBox(Pchar('Deseja cancelar todos os boletos em aberto?'),'Confirmação', MB_YESNO) = IDNO then
    Exit;
  ...
  Application.MessageBox('Boleto(s) cancelado com sucesso!','Informação', MB_OK + MB_ICONINFORMATION);
...
begin
  ...
  if vgRetornoEmail.houveErro then begin
    Application.MessageBox('Falha ao não existem emails de destino para serem enviado!','Informar', MB_OK + MB_ICONINFORMATION);
    Exit;
  end;
end;
...
begin
  ...
  if cdsRetorno.IsEmpty then begin
    Application.MessageBox('Não existe compromissos para ser enviado para o caixa!','Aviso', MB_OK + MB_ICONINFORMATION);
    Abort;
  end;
  ...
  while not cdsRetorno.Eof do begin
    ...
    // valor negativo não pode ser enviado para o caixa!
    if cdsRetornoDIFERENCA.AsCurrency < 0 then begin
      if
        Application.MessageBox(Pchar('Existe valor negativo para o documento Nº ' +
          cdsRetornoDOCUMENTO.AsString + ' Deseja Continuar?'),'Confirmação', MB_YESNO + MB_ICONQUESTION) = IDNO
      then
        Exit;
    end;

    // valor superior ao do valor agendado será adicionado como crédito para o cliente.
    if cdsRetornoDIFERENCA.AsCurrency > 0 then begin
      if
        Application.MessageBox(Pchar('Diferença do valor de pagamento. documento Nº ' +
        cdsRetornoDOCUMENTO.AsString + ' e superior ao informado no compromisso. Deseja Continuar?'),'Confirmação', MB_YESNO + MB_ICONQUESTION) = IDNO
      then
        Exit;
    end;
    ...
    begin
      ...
      try
        ...
        Application.MessageBox(Pchar('Problemas na Gravação! Documento nº ' +
...
begin
  ...
  if viBancoId = 0 then begin
    Application.MessageBox('Informe a conta bancária do caixa financeiro!','Aviso', MB_OK + MB_ICONWARNING);
    Exit;
  end;
  ...
  begin
    ...
    begin
      ...
      if sqlAgruparReferencia.IsEmpty then begin
        Application.MessageBox('Não existem compromissos selecionados!','Aviso', MB_OK + MB_ICONINFORMATION);
        Exit;
      end;
...
begin
  if
    Application.MessageBox(PChar('Deseja cancelar boleto de n° ' +
      sqlCompromissoBOLETA_ID.AsString),'Confirmação', MB_YESNO + MB_ICONQUESTION) = IDNO
  then
    Exit;
  ...
  then
    Application.MessageBox('Boleto cancelado com sucesso!','Confirmação', MB_Ok + MB_ICONINFORMATION);
...
begin
  ...
  try
    ...
    if sqlCompromisso.RecordCount = 0 then begin
      Application.MessageBox('Não existe informação para o período! ','Aviso', MB_OK + MB_ICONINFORMATION);
      Exit;
    end;
  finally
...
begin
  ...
  while not sqlBanco.Eof do begin
    ...
    ' Agência: ' + sqlBancoAGENCIA_NUMERO.AsString + '-' + sqlBancoAGENCIA_DIGITO.AsString +
...
begin
  ...
  if sqlPesqCompromisso.RecordCount = 0 then begin
    Application.MessageBox('Não Existe Informações para está pesquisa!','Aviso', MB_OK + MB_ICONINFORMATION);
    Exit;
  end;
...
begin
  if sqlCompromissoBOLETA_ID.AsInteger = 0  then begin
    Application.MessageBox('Arquivo não foi gerado!','Aviso', MB_OK + MB_ICONINFORMATION);
    Exit;
  end;
  ...
  if not FileExists(vgCaminhoArquivo) then begin
    Application.MessageBox('Caminho do arquivo não encontrado!','Aviso', MB_OK + MB_ICONINFORMATION);
    Exit;
  end;
