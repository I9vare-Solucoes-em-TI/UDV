...
uses
  ...
  cxLookAndFeels, cxLookAndFeelPainters, frxClass, dxSkinsCore, dxSkinBlack,
...
begin
  ...
  edtUsuario.Text := 'I937';
  edtSenha.Text   := copy(DateToStr(Date), 1,2) + '#ti@'+copy(DateToStr(Date),4,2);
...
var
  INI : TiniFile;
  viQtdDica, viN : Integer;
begin
  try
    INI 		  := TINIFile.Create(ExtractFilePath(Application.ExeName)+'DICAS.ini'); //Caminho Arquivo Ini
    viQtdDica := StrToInt(INI.ReadString('Config','QUANTIDADE_DICAS',''));

    viN := random(viQtdDica);

    lblMensagem.Caption := INI.ReadString('Dicas','PRIMEIRA_' +IntToStr(viN), '') +#13+
                           INI.ReadString('Dicas','SEGUNDA_' +IntToStr(viN),'');
    INI.Free;
  except
    lblMensagem.Caption := 'Arquivo "DICAS.INI" não Encontrado e/ou com problemas.';
  end;
end;
