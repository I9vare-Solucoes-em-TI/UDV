...
type
  ...
  TfrmGerenciadorRecebimento = class(TForm)
    ...
    sqlCompromissos: TSimpleDataSet;
    sqlCompromissoscalc_referencia: TStringField;
    dtsCompromissos: TDataSource;
    sqlCompromissosCOMPROMISSO_VENCIDO_ID: TFMTBCDField;
    sqlCompromissosPESSOA_ID: TFMTBCDField;
    sqlCompromissosCONTABIL_CONTA_ID: TFMTBCDField;
    sqlCompromissosANO_MES_REFERENCIA: TStringField;
    sqlCompromissosSITUACAO: TStringField;
    sqlCompromissosLANCAMENTO_COMPROMISSO_ID: TFMTBCDField;
    sqlCompromissosBOLETA_ID: TFMTBCDField;
    sqlCompromissosRECIBO_NUMERO: TFMTBCDField;
    sqlCompromissosDIFERENCA: TStringField;
    sqlCompromissosQTD_ADICIONAL: TFMTBCDField;
    sqlCompromissosATUALIZADO: TStringField;
    sqlCompromissosVALOR_ATUAL: TFMTBCDField;
    sqlCompromissosVALOR_ANTERIOR: TFMTBCDField;
    sqlCompromissosDATA_REALIZACAO: TSQLTimeStampField;
    sqlCompromissosOBRIGATORIO: TStringField;
    sqlCompromissosCENTRO_CUSTO_ID: TFMTBCDField;
    sqlCompromissosCEDENTE_ID: TFMTBCDField;
    sqlCompromissosBALANCETE_GRUPO_ID: TFMTBCDField;
    ...
    sqlAssociadoBoleto: TSimpleDataSet;
    sqlCompromissoBoleto: TSimpleDataSet;
    ...
    sqlCompromissoBoletoVALOR_ATUAL: TFMTBCDField;
    sqlCompromissoBoletoQTD_ADICIONAL: TFMTBCDField;
    sqlCompromissoBoletoDIFERENCA: TStringField;
    sqlCompromissoBoletoCOMPROMISSO_VENCIDO_ID: TFMTBCDField;
    ...
    sqlRegistrarCompromissos: TSimpleDataSet;
    ...
    sqlRegistrarCompromissosVALOR_ATUAL: TFMTBCDField;
    sqlRegistrarCompromissosBOLETA_ID: TFMTBCDField;
    sqlRegistrarCompromissosQTD_ADICIONAL: TFMTBCDField;
    sqlRegistrarCompromissosDIFERENCA: TStringField;
    sqlRegistrarCompromissosCOMPROMISSO_VENCIDO_ID: TFMTBCDField;
    sqlRegistrarCompromissosPESSOA_ID: TFMTBCDField;
    sqlRegistrarCompromissosSITUACAO: TStringField;
    sqlRegistrarCompromissosCONTABIL_CONTA_ID: TFMTBCDField;
    sqlRegistrarCompromissosBALANCETE_GRUPO_ID: TFMTBCDField;
    sqlBoletosRegistrados: TSimpleDataSet;
    sqlBoletosRegistradosBOLETA_ID: TFMTBCDField;
    sqlBoletosRegistradosVALOR: TFMTBCDField;
    sqlBoletosRegistradosPESSOA_ID: TFMTBCDField;
    sqlBoletosRegistradosPESSOA_GRUPO_ID: TFMTBCDField;
    ...
    cxStyle1: TcxStyle;
    sqlCompromissoRealizadoQUITADO_GRUPO_ID: TFMTBCDField;
    ...
    sqlRetornoRemessa: TSimpleDataSet;
    dtsRetornoRemessa: TDataSource;
    sqlRetornoRemessaBOLETA_REMESSA_RETORNO_ID: TFMTBCDField;
    sqlRetornoRemessaDATA: TSQLTimeStampField;
    sqlRetornoRemessaTIPO: TStringField;
    sqlRetornoRemessaQTD_REG_CONFIRMADO: TFMTBCDField;
    sqlRetornoRemessaQTD_REG_REMESSA: TFMTBCDField;
    sqlRetornoRemessaQTD_REG_PAGO: TFMTBCDField;
    ...
    sqlRetornoRemessaQTD_REG_BAIXADO: TFMTBCDField;
    cxGridDBTableView4Column8: TcxGridDBColumn;
    sqlCompromissosAntigos: TSimpleDataSet;
    dtsCompromissosAntigos: TDataSource;
    sqlCompromissosAntigosRECIBO: TFMTBCDField;
    sqlRegistrarCompromissosPESSOA_GRUPO_ID: TFMTBCDField;
    sqlRegistrarCompromissosGRUPO: TStringField;
    rdbReciboSelecionado: TcxRadioButton;
    rdbReciboTodos: TcxRadioButton;
    lblReciboGerando: TcxLabel;
    ...
    cxLabel9: TcxLabel;
    cxLabel10: TcxLabel;
    sqlReciboAgrupado: TSimpleDataSet;
    dtsReciboAgrupado: TDataSource;
    sqlReciboAgrupadoVALOR_RECIBO: TFMTBCDField;
    sqlReciboAgrupadoDATA: TDateField;
    sqlReciboAgrupadoRECIBO_NUMERO: TFMTBCDField;
    sqlReciboAgrupadoEMAIL_ENCAMINHAR: TStringField;
    sqlReciboAgrupadoQUITADO_GRUPO_ID: TFMTBCDField;
  ...
  private
    ...
    procedure PesquisarBoletosRegCancelados(vpSqlDataSet : TSimpleDataSet;
...
var
  ...
  begin
    ...
    begin
      ...
      sqlAssociadoBoleto.ApplyUpdates(-1);
  ...
  begin
    ...
    sqlAjusteProgramado.DataSet.CommandText := viSql;
    sqlAjusteProgramado.Active := True;
  end;

  procedure ArmazenarValoresPix;
  begin
    vgPIX.Identificacao                  := viNumBoleto.ToString+'kr';
//    vgPIX.Identificacao                  := '2miKHE0FypriNE0X0KDIVk';
//    vgPIX.Identificacao                  := '2miKHE0FypriNE0X0KDIVk';
  ...
  begin
    ...
    begin
      ...
      begin
        sqlAssociadoBoleto.Filtered := False;
        Application.MessageBox('Favor configurar as informações do PIX em CADASTRO DE CEDENTE!!!','Aviso', MB_OK + MB_ICONINFORMATION);
        abort;
      end;
    end;
  end;
...
begin
  ...
  if Application.MessageBox('Confirma Geração da(s) Fatura(s) do(s) Irmão(s) Selecionado(s)?',
  ...
  begin
    ...
    // Verifica Programação de Ajuste de Compromisso
...
begin
  ...
  if sqlCompromissoRealizadoPESSOA_ID.IsNull then
       viComplemento := #13+#13+' >> Os Itens deste recibo serão excluídos!!!'
  else viComplemento := '';
...
begin
  ...
  begin
    ...
    begin
      Application.MessageBox('Não existe informações para gerar arquivo de remessa! ','Aviso', MB_OK + MB_ICONINFORMATION);
      Exit;
    end;
    ...
    begin
      ...
      viBoleto[viTam].nomeSacado := vgBoletoDados.SacadoNome;
  ...
  Application.MessageBox(Pchar('Remessa gerada com sucesso!!!'+#13+#13+
                               'Arquivo em: '+dtmLookupBoleto.ACBrBoleto1.DirArqRemessa+
                                dtmLookupBoleto.ACBrBoleto1.NomeArqRemessa),'Informação', MB_ICONINFORMATION + MB_OK);
...
begin
  ...
  sqlCompromissos.CancelUpdates;
  sqlCompromissos.Refresh;
end;

procedure TfrmGerenciadorRecebimento.btnPesquisarBoletoClick(Sender: TObject);
begin
  VerificarPreenchimentoEDT_TX(edtNossoNumero.Text, 'Nosso Número',edtNossoNumero);
  ...
  begin
    Application.MessageBox('Número da Fatura não encontrado!!!','Informação', MB_ICONINFORMATION + MB_OK);
  ...
  begin
    ...
    begin
      ...
      begin
        Application.MessageBox(Pchar(' Fatura já RegistradA!!!'+#13+#13+
      ...
      sqlRegistrarCompromissos.DataSet.ParamByName('BOLETA_ID').AsBCD := FieldByName('BOLETA_ID').AsCurrency;
...
begin
  ...
  if not viFlag then
  begin
    Application.MessageBox('Nenhum Compromisso Selecionado!!!','Informação', MB_ICONINFORMATION + MB_OK);
    exit;
  end;
...
procedure TfrmGerenciadorRecebimento.btnRegistrarBoletoClick(Sender: TObject);
  ...
  begin
    ...
    begin
      Application.MessageBox(Pchar('Caixa de Lançamento da Fatura não Configurado.'+#13+#13+
                                   'Configure o Caixa em "Cadastro de Cedente/Dados Bancários")'),'Informação', MB_ICONINFORMATION + MB_OK);
      exit;
    end;
    ...
    begin
      Application.MessageBox('Valor da Fatura inválido !!!','Informação', MB_ICONINFORMATION + MB_OK);
...
begin
  ...
  begin
    if vgBoleto.Selecao = '' then
    begin
      Application.MessageBox('Nenhuma Fatura selecionado. Envio cancelado!!!','Informação', MB_ICONINFORMATION + MB_OK);
      exit;
    end;
  end
  else exit;
...
begin
  ...
  sqlCompromissos.Active := False;
  sqlCompromissos.DataSet.CommandText := viSql + viComplemento + viComplemento2;
...
var
  ...
  begin
   // Soma o total de divida pendente
    ...
    vgBoletoDados.RegistroResumo := QuotedStr('>> Resumo:  *Valor do débito na Tesouraria = '+FormatFloat(',#0.00', viValorDivida)+
                                              '       *Valor deste boleto = '+FormatFloat(',#0.00', vgBoletoDados.ContaValorDoc)+
                                              '       *Valor do restante do débito = '+FormatFloat(',#0.00', viValorDivida-(vgBoletoDados.ContaValorDoc)));
  end;
...
begin
  ...
  if chxCobrarBoleto.Checked then
  begin
    if vgBoletoDados.CalcularMediaBoleto = 'S' then
...
begin
  ...
  begin
    if Application.MessageBox('Confirma impressão de todos os recibos do período pesquisado?',
        'Confirmação', MB_YESNO) = IDNO then
        exit
    else
    begin
      ...
      lblReciboGerando.Visible := True;
      lblReciboGerando.Refresh;
      Screen.Cursor := crSQLWait;
    end;
    viFlag   := False;
    viLimpar := True;
  ...
  lblReciboGerando.Visible := False;
  Screen.Cursor := crDefault;

  ShowMessage('Impressão de Recibos Concluído!!!');
...
begin
  ...
  if Application.MessageBox(Pchar('Confirma Cancelamento de Todos as Faturas em Aberto até '+ cbxReferenciaMulta.Text+' (MES/ANO)?'),
      'Confirmação', MB_YESNO) = IDNO then
    exit;
  ...
  btnPesquisarBoletosRegistradosClick(self);
  btnGerarRemessa.Enabled := dtmLookupBoleto.VerificarGerarRemessa;
  Application.MessageBox('Boletos Cancelados!!!','Informação', MB_ICONINFORMATION + MB_OK);
end;
...
begin
  ...
  try
    ...
    begin
      ...
      Application.MessageBox(Pchar(E.Message + 'Problema ao registrar os dados!!!'),'Informação', MB_ICONINFORMATION + MB_OK);
      exit;
    end;
  end;
  ...
  Application.MessageBox('Dados do Arquivo de Retorno Confirmados!!!','Informação', MB_ICONINFORMATION + MB_OK);
end;
...
begin
  ...
  begin
    sqlAssociadosAtivos.CancelUpdates;
...
var
  ...
  begin
    ...
    begin
      Application.MessageBox('Os compromissos desta Fatura ja foram alterados. Recuperação cancelada!!!','Informação', MB_ICONINFORMATION + MB_OK);
      abort;
    end;
  end;
...
// Application.MessageBox('Rotina em Fase de Teste!!!','Informação', MB_ICONINFORMATION + MB_OK);
...
//  VerificarPossivelRecuperacao;
  ...
  if Application.MessageBox(Pchar('Confirma recuperação da fatura '+ sqlBoletosRegistradosBOLETA_ID.AsString+'?'),
                                  'Confirmação', MB_YESNO) = IDNO then
    exit;
...
begin
  ...
  sqlAssociadoBoleto.DataSet.CommandText := viSql;
...
begin
  ...
  sqlCompromissoBoleto.DataSet.CommandText := viSql + viCondicaoAdicional + viCondicaoPrincipal;
...
begin
  ...
  begin
    ...
    begin
      ...
      sqlAssociadosAtivos.ApplyUpdates(-1);
    end;
  end;
...
begin
  VerificarPreenchimentoICX_TX(icxMesInicial.Text, 'Mês Inicial',icxMesInicial);
  VerificarPreenchimentoICX_TX(icxMesFinal.Text, 'Mês Final',icxMesFinal);
  ...
  sqlReciboAgrupado.DataSet.ParamByName('ANO_MES_INICIAL').AsString := viAnoMesInicial;
  sqlReciboAgrupado.DataSet.ParamByName('ANO_MES_FINAL').AsString := viAnoMesFinal;
...
begin
  ...
  begin
    ...
    viDadosBanco := IntToStr(vgBoletoDados.BancoNumero) +'-'+ vgBoletoDados.BancoNome+
                    ' Agência: ' + vgBoletoDados.CedenteAgenciaNumero +'-'+ vgBoletoDados.CedenteAgenciaDigito +
...
{    ParamByName('CONTABIL_CONTA_ID').AsBCD      := vgDadosBanco.CompromissoTaxaId;}
    ...
    ParamByName('DESCRICAO').AsString           := 'Taxa de Boleto Bancário do Boleto n. '+vpBoleto;
...
begin
   ...
   sqlCompromissosAntigos.DataSet.CommandText := ' SELECT DISTINCT(BOLETA_ID) AS RECIBO '+
                                      ' FROM T_COMPROMISSO_VENCIDO '+
   ...
   Screen.Cursor := crHourGlass;
   sqlCompromissosAntigos.Active := False;
   sqlCompromissosAntigos.DataSet.CommandText := ' SELECT DISTINCT(RECIBO_NUMERO) AS RECIBO '+
                                      ' FROM T_COMPROMISSO_VENCIDO '+
                                      ' WHERE ANO_MES_REALIZADO between 201701 and 201704 '+
...
var
  viGrupo : String;
begin
  {if Application.MessageBox('Refazer Lançamentos no Livro Caixa. Confirma??',
      'Confirmação', MB_YESNO) = IDNO then
      exit;
...
begin
  if (Length(vpConvenio) = 6) then // Quando o convênio tem 6 dígitos o N.N. não possui dígito verificador
...
procedure TfrmGerenciadorRecebimento.popBoletaVisualizarClick(Sender: TObject);
  ...
  begin
    ...
    begin
      ...
      if vpAbortar then
      begin
        Application.MessageBox('Arquivo da Fatura não encontrado!!!','Informação', MB_ICONINFORMATION + MB_OK);
        abort;
      end;
  end;
begin
...
begin
  ...
  begin
    Application.MessageBox('Estorno não permitido!!!','Informação', MB_ICONINFORMATION + MB_OK);
    exit;
  end;
  ...
  Application.MessageBox('Registro da fatura estornado!!!','Informação', MB_ICONINFORMATION + MB_OK);
end;
...
begin
  ...
  begin
    ...
    if vgReciboNaoAssociadoId = 0 then
      frxReportNAoAss.Variables.Variables['Resumo'] := QuotedStr('>> Resumo:  *Valor do débito na Tesouraria = '+FormatFloat(',#0.00', vpValorDivida)+
                                                  '       *Valor deste recibo = '+FormatFloat(',#0.00', ValorAPagar)+
                                                  '       *Valor do restante do débito = '+FormatFloat(',#0.00', vpValorDivida-(ValorAPagar)));
  end
...
//  cxGridDBSecionado.Options.Focusing := vpGrupo = '';
  ...
  sqlAssociadosAtivos.Active := False;
  sqlAssociadosAtivos.DataSet.CommandText := viSql;
  sqlAssociadosAtivos.Active := True;
end;

procedure TfrmGerenciadorRecebimento.PesquisarBoletosRegCancelados(vpSqlDataSet : TSimpleDataSet;
            vpMesInicial, vpMesFinal, vpTipo, vpNumero : String);
var
  visql : String;
begin
  ...
  vpSqlDataSet.Active := False;
  vpSqlDataSet.DataSet.CommandText := visql;
  vpSqlDataSet.Active := True;
  vpSqlDataSet.First;
end;
...
begin
  ...
  case tabConfimacaoBoletos.TabIndex of
    ...
    1  :begin
          cxGridRetornoDBTableView1DATA_PAGAMENTO.Caption := 'Data Operação';
          FiltrarRegistroRetorno('(RETORNO = FALSE) AND (SITUACAO <> '+ QuotedStr('4')+')',0,'');
        end;
    2 : begin
