...
begin
  ...
  Incio}
    if not FileExists('fbclient.dll') then
      CopyFile(PChar(IncludeTrailingPathDelimiter(vgVERSAO) + 'fbclient.dll')  , PChar('fbclient.dll')  , false);
  {Fim}
  {$endif VER180}

  vgVERSAO  := IncludeTrailingPathDelimiter(vgVERSAO) + NomeExe + '.exe';
  ...
  begin
    ...
    begin
      ...
      WinExec(PAnsichar('Update ' + NomeExe), 0); //Executa o update.exe
