...
uses
  ...
  FlCtrlEx,{$WARN UNIT_PLATFORM OFF} Vcl.FileCtrl,{$WARN UNIT_PLATFORM ON}
  cxListView, ParamListbox, ColListb, FrameImagem, FramePDF, cxSplitter,
  ImagXpr7_TLB, cxDBLabel, FireDAC.Stan.Intf, FireDAC.Stan.Option,
...
type
  TfrmLegislacao = class(TForm)
    ...
    pnlOpcaoDigitalizar: TPanel;
    rdbDigitalizacaoPretoBranco: TcxRadioButton;
    rdbDigitalizacaoColorida: TcxRadioButton;
    ...
    procedure pagArquivoAnexoPageChanging(Sender: TObject; NewPage: TcxTabSheet;
      var AllowChange: Boolean);
    procedure pagPrincipalPageChanging(Sender: TObject; NewPage: TcxTabSheet;
      var AllowChange: Boolean);
    procedure btnImprimirProtocoloClick(Sender: TObject);
    procedure rdbDigitalizacaoPretoBrancoClick(Sender: TObject);
    procedure rdbDigitalizacaoColoridaClick(Sender: TObject);
  private
  ...
  begin
    ...
    begin
      Application.MessageBox(
        'O campo Esfera/Nível é de preenchimento obrigatório!',
        'Atenção!',
    ...
    begin
      Application.MessageBox(
        'O campo Origem é de preenchimento obrigatório!',
        'Atenção!',
        MB_OK + MB_ICONWARNING
      );
    ...
    begin
      Application.MessageBox(
        'O campo Natureza/Área é de preenchimento obrigatório!',
        'Atenção!',
        MB_OK + MB_ICONWARNING
      );
    ...
    begin
      if
        Application.MessageBox(
          'É obrigatório anexar pelo menos um arquivo!' + #13#10 +
          'Deseja anexar agora?',
          'Atenção!',
...
begin
  ...
  begin
    Application.MessageBox(
      'Existe um registro em edição, salve ou cancele antes de efetuar a pesquisa!',
      'Aviso!',
      MB_OK + MB_ICONWARNING
    );
...
begin
  ...
  begin
    Application.MessageBox(
      'Parâmetro no SISTEMA DO CAIXA: OUTROS/PROVIMENTO/LOCAL_ANEXOS_PROVIMENTO não configurado!!!',
      'ATENÇÃO!!!',
  ...
  begin
    ...
    begin
      ...
      begin
        Application.MessageBox(
          'Já existe um arquivo com este nome anexado ao provimento!',
          'Atenção!',
          MB_OK + MB_ICONWARNING
        );
...
begin
  if not fmeImagem1.ValidarImagem then
    Abort;
...
procedure TfrmLegislacao.pagArquivoAnexoPageChanging(Sender: TObject;
  NewPage: TcxTabSheet; var AllowChange: Boolean);
begin
  AllowChange := fmeImagem1.ValidarImagem;
end;

procedure TfrmLegislacao.pagPrincipalPageChanging(Sender: TObject;
  NewPage: TcxTabSheet; var AllowChange: Boolean);
begin
  AllowChange := fmeImagem1.ValidarImagem;
end;

procedure TfrmLegislacao.rdbDigitalizacaoColoridaClick(Sender: TObject);
begin
  fmeImagem1.ImagemColorida(True);
end;

procedure TfrmLegislacao.rdbDigitalizacaoPretoBrancoClick(Sender: TObject);
begin
  fmeImagem1.ImagemColorida(False);
end;
