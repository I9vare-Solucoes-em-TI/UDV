...
type
  TfrmCadSeloLote = class(TfrmCadBasico)
    ...
    sqlSeloLivroCAMPO_ITEM_ID: TBCDField;
    sqlSeloSituacaoSELO_SITUACAO_ID: TBCDField;
    sqlSeloSituacaoSISTEMA_ID: TBCDField;
    cdsSelosCODIGO_EXPORTACAO: TBCDField;
    sqlSeloLivroVALIDADOR: TStringField;
    ClientAncestralNUMERO: TBCDField;
    grdBasicaDBTableView1Column1: TcxGridDBColumn;
    ClientAncestralCODIGO_SELO_PRENOTACAO: TStringField;
...
uses Lookup, Controles, Rotinas, Atualizacao, RegistrarSelo, System.Types, System.StrUtils,
  ADM.View.Movimentacao.SelosTJ_GO.DevolucaoAtos.NovaDevolucao,
  ADM.View.Movimentacao.SelosTJ_GO.DevolucaoAtos.HistoricoDevolucao,
  MensagemUtils;
...
var
  viCondicao, viCodigo, viSql : string;
...
begin
  ...
  viSql := ' SELECT '+
           ...
           ' LEFT JOIN G_SELO_GRUPO G_SG ON G_SG.SELO_GRUPO_ID = G_ST.SELO_GRUPO_ID ';

  if rdbAtivos.Checked then
       viSql := viSql + ' WHERE G_ST.SITUACAO <> '+QuotedStr('F')
  else viSql := viSql + ' WHERE G_ST.SITUACAO = '+QuotedStr('F');

  if Trim(edtCodigo.Text) <> '' then
  begin
    viCodigo := dtmControles.GetStr(' SELECT SELO_GRUPO_ID FROM G_SELO_GRUPO WHERE NUMERO = '+ edtCodigo.Text);

    if Trim(viCodigo) = '' then
    begin
      TMensagemUtils.ExibirMensagemAtencao('Código do grupo de selos informado não existe!');
      edtCodigo.SetFocus;
      Exit;
    end;

    viCondicao := viCondicao + ' AND G_ST.SELO_GRUPO_ID = '+viCodigo;
  end;

  if Trim(edtNF.Text) <> '' then
    viCondicao := viCondicao + ' AND G_ST.NOTA_FISCAL = '+QuotedStr(edtNF.Text);

  viSql := viSql + viCondicao + ' ORDER BY G_ST.SELO_GRUPO_ID, G_ST.DATA_LOTE ';
...
procedure TfrmCadSeloLote.pgcControleChange(Sender: TObject);
  ...
  begin
    Result := IntToStr(dtmControles.GetInt(' SELECT COUNT(SELO_LIVRO_ID) FROM G_SELO_LIVRO '+
                                            ' WHERE SELO_LOTE_ID = '+ IntToStr(ClientAncestralSELO_LOTE_ID.AsInteger) +
...
begin
  ...
  begin
    ...
    begin
      ...
      lblUsuario.Caption     := ' Usuário   = '+ IntToStr(vgSeloUsuario);
    end
    else
    begin
      lblReservado.Caption   := ' Devolvido = '+ IntToStr(vgSeloReservado);
      vgSeloUsuario := StrToInt(PegarSeloDisponivel('7'));
      lblUsuario.Caption     := ' Perdido   = '+ IntToStr(vgSeloUsuario);
    end;
    ...
    (*
       Remover este bloqueio e permitir que selos digitais possam ser apenas inutilizados
      ...
      begin
        ...
        begin
          ...
          begin
            btnRegistrarSelo.Enabled := False;
            Break;
          end;
        end;
      end;
...
begin
  ...
  if vgQtdCaracterSelo = 6 then
    ...
    sqlSeloLivroCALC_NUMERO.AsString   := dtmControles.FormatarMascaraSelo(
      sqlSeloLivroSIGLA.AsString, sqlSeloLivroNUMERO.AsInteger, sqlSeloLivroVALIDADOR.AsString);
end;
...
begin
  ...
  sqlSeloscal_numero_selo.AsString := dtmControles.FormatarMascaraSelo(
    sqlSelosSIGLA.AsString, sqlSelosNUMERO.AsInteger, sqlSeloLivroVALIDADOR.AsString);
end;
...
begin
  ...
  chxFechamentoLote.Checked := True;
  btnFecharLoteClick(Self);
end;
...
begin
  ...
  begin
    if ClientAncestralSELO_LOTE_ID.AsString <> '' then
      viFechar := dtmControles.GetInt(' SELECT COUNT(*) FROM G_SELO_LIVRO '+
                                      ' WHERE SELO_SITUACAO_ID IN (1,5,6) '+
    ...
    if not viFlag then
    begin
      ShowMessage('Este Lote não pode ser Fechado!!!');
      exit;
    end;
  ...
  chxFechamentoLote.Checked := False;
end;
...
var
  ...
  begin
    viSql := ' SELECT ' + sLineBreak +
             '       SO.NOTA_FISCAL, ' + sLineBreak +
             '       SL.SELO_LIVRO_ID, ' + sLineBreak +
             '       SL.NUMERO_AGRUPADOR, ' + sLineBreak +
             '       SL.CODIGO_EXPORTACAO, ' + sLineBreak +
             '       SL.SIGLA, ' + sLineBreak +
             '       SL.NUMERO, ' + sLineBreak +
             '       SG.NUMERO AS TIPO_ATO, ' + sLineBreak +
             '       SL.APRESENTANTE, ' + sLineBreak +
             '       U.NOME_COMPLETO, ' + sLineBreak +
             '       SL.DATA, ' + sLineBreak +
             '       SL.IP_MAQUINA, ' + sLineBreak +
             '       SL.VALOR_TOTAL, ' + sLineBreak +
             '       SL.VALOR_EMOLUMENTO, ' + sLineBreak +
             '       SL.VALOR_TAXA_JUDICIARIA, ' + sLineBreak +
             '       SL.VALOR_FUNDESP, ' + sLineBreak +
             '       SL.SELO_LIVRO_ID, ' + sLineBreak +
             '       SG.SELO_GRUPO_ID, ' + sLineBreak +
             '       SL.SELO_SITUACAO_ID, ' + sLineBreak +
             '       SL.DESCRICAO, ' + sLineBreak +
             '       SL.TABELA, ' + sLineBreak +
             '       SL.CAMPO_ID, ' + sLineBreak +
             '       SG.TIPO_CARTORIO, ' + sLineBreak +
             '       Sl.PROTOCOLO ' + sLineBreak +
             ' FROM G_SELO_LIVRO SL ' + sLineBreak +
             ' LEFT JOIN G_SELO_LOTE SO ON ' + sLineBreak +
             '  SL.SELO_LOTE_ID = SO.SELO_LOTE_ID ' + sLineBreak +
             ' LEFT JOIN G_SELO_GRUPO SG ON ' + sLineBreak +
             '  SO.SELO_GRUPO_ID = SG.SELO_GRUPO_ID ' + sLineBreak +
             ' LEFT JOIN G_USUARIO U ON ' + sLineBreak +
             '   SL.USUARIO_ID = U.USUARIO_ID ' + sLineBreak +
  ...
  begin
    try
      ...
      begin
        ...
        if (Trim(viSeloSigla) = '')
        or (Trim(viSeloNumero) = '') then
        begin
          Application.MessageBox(PChar('Número do selo na lista é inválido! Selo: ' + viSeloLista), 'Aviso', MB_OK + MB_ICONWARNING);
          Exit;
        end;
        ...
        sqlSelos.SQL.Text :=
          viSql + sLineBreak +
          ' AND SL.SIGLA = ' + QuotedStr(viSeloSigla) + sLineBreak +
          ' AND SL.NUMERO = ' + viSeloNumero + sLineBreak +
      ...
      on e: Exception do
        ShowMessage(e.Message + sLineBreak + mmoLista.Lines.Strings[j]);
    end;
...
begin
  inherited;
  ClientAncestralCALC_NUMERO_INI.AsString    := dtmControles.FormatarMascaraSelo('', ClientAncestralNUMERO_INICIAL.AsInteger, '');
  FormatFloat('000000000', ClientAncestralNUMERO_INICIAL.AsInteger);
  ClientAncestralCALC_NUMERO_FIM.AsString    := dtmControles.FormatarMascaraSelo('', ClientAncestralNUMERO_FINAL.AsInteger, '');
  FormatFloat('000000000', ClientAncestralNUMERO_INICIAL.AsInteger);
end;
  ...
  repeat
    ...
    begin
      try
        ...
        viProtocolo := '';
        Application.MessageBox('Numero inválido. Digite apenas número.', 'Atenção', MB_OK + MB_ICONWARNING);
      end;
    end
    else abort;
  until viProtocolo <> '';
...
begin
  ...
  begin
    Application.MessageBox('Número de Selo não encontrado!', 'Atenção', MB_OK + MB_ICONWARNING);
    edtNumeroSelo.SetFocus;
  end;
end;
...
begin
  inherited;
  ClientAncestral.Active := False;
  grdBasicaDBTableView1.DataController.DataModeController.GridMode := False;
  DataSetAncestral.FetchOptions.Mode := fmAll;
  ClientAncestral.PacketRecords := -1;
  PesquisarLoteSelos;
end;
...
begin
  inherited;
  ClientAncestral.Active := False;
  grdBasicaDBTableView1.DataController.DataModeController.GridMode := True;
  DataSetAncestral.FetchOptions.Mode       := fmOnDemand;
  DataSetAncestral.FetchOptions.RowsetSize := 50;
  ClientAncestral.PacketRecords            := 50;
  PesquisarLoteSelos;
end;
...
begin
  ...
  begin
    if ClientAncestralNUMERO.AsInteger = StrToIntDef(viCodServentia, 0) then
    begin
      Application.MessageBox('Não é permitido alterar lotes de selos digitais!', 'Atenção', MB_OK + MB_ICONWARNING);
      Exit;
    end;
  end;
...
begin
  ...
  begin
    if ClientAncestralNUMERO.AsInteger = StrToIntDef(viCodServentia, 0) then
    begin
      Application.MessageBox('Não é permitido excluir lotes de selos digitais!', 'Atenção', MB_OK + MB_ICONWARNING);
      Exit;
    end;
  end;
