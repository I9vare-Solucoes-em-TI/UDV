...
uses
  I9Query, Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls,
  Forms, Dialogs, CadAuxiliar, Menus, FMTBcd, cxDropDownEdit, cxLookupEdit,
  ...
  StdCtrls, cxButtons, ExtCtrls, Vcl.ComCtrls, cxGraphics, cxLookAndFeels,
  cxLookAndFeelPainters, dxCore, cxDateUtils, DbxDevartInterBase, SimpleDS,
  FireDAC.Stan.Intf, FireDAC.Stan.Option, FireDAC.Stan.Param, FireDAC.Stan.Error,
  FireDAC.DatS, FireDAC.Phys.Intf, FireDAC.DApt.Intf, FireDAC.Stan.Async,
  FireDAC.DApt, FireDAC.Comp.DataSet, FireDAC.Comp.Client, System.Types,
  System.StrUtils;
...
type
  ...
  private
    ...
    function BuscarSeloGrupo(vpSeloLivroId: Integer): Integer;

    procedure ControlarSituacao;
  public
    { Public declarations }
  end;
...
function TfrmSeloRegistrar.BuscarSeloGrupo(vpSeloLivroId: Integer): Integer;
begin
  Result := dtmControles.GetInt(
    'SELECT G_SG.NUMERO ' +
    '  FROM G_SELO_LIVRO G_SL ' +
    ' INNER JOIN G_SELO_LOTE G_ST ON G_ST.SELO_LOTE_ID = G_SL.SELO_LOTE_ID ' +
    ' INNER JOIN G_SELO_GRUPO G_SG ON G_SG.SELO_GRUPO_ID = G_ST.SELO_GRUPO_ID ' +
    ' WHERE G_SL.SELO_LIVRO_ID = ' + IntToStr(vpSeloLivroId) +
    ' ORDER BY NUMERO   '
  );
end;

procedure TfrmSeloRegistrar.ControlarSituacao;
const
  ciInutilizado = 'Inutilizado';
var
  viListCodServentia: TStringDynArray;
  viSeloGrupoNumero: Integer;
  viCodServentia: string;
begin
  inherited;
  lcxSeloSituacao.Enabled := True;

  viSeloGrupoNumero := BuscarSeloGrupo(frmCadSeloLote.sqlSeloLivroSELO_LIVRO_ID.AsInteger);

  viListCodServentia := SplitString(vlCodServentiaSeloDigital, ';');

  for viCodServentia in viListCodServentia do
  begin
    if (viSeloGrupoNumero = StrToIntDef(viCodServentia, 0))
    and (Trim(frmCadSeloLote.sqlSeloSituacaoDESCRICAO.AsString) <> ciInutilizado) then
    begin
      lcxSeloSituacao.Enabled := False;
      lcxSeloSituacao.ItemIndex := 2; // Inutilizado
      Exit;
    end;
  end;
end;
