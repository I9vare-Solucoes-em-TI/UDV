...
uses
  ...
  private
    ...
    function GravarEstacaoHttp :Boolean;
...
begin
  ...
  try
    ...
    begin
      ...
      if (viVersaoSys < viVersaoDB) then
      begin
        ShowMessage('Sistema desatualizado nesta máquina, favor entrar em contato com o suporte!');
        Result := False;
      end
      else
        GravarEstacaoHttp;
...
function THttpVersao.GravarEstacaoHttp :Boolean;
var
  viURL : string;
begin
  Result := False;
  try
    viURL  := 'http://www.tri7.com.br/estacao.php?idcartorio=' + FIdCartorio +
                                                '&chave=' + HTTPEncode(vgKey) +
                                                '&ip=' + HTTPEncode(RetornaIP);
    Result := copy(RetornaNumerico(FIdHTTP.Get(viURL)), 1, 1) = '1';
  except
