...
type
  TfrmExportarSelo = class(TForm)
    ...
    BtnValidarTodos: TcxButton;
    sqlSelosNaoValidados: TI9Query;
    sqlSelosNaoValidadosSELO_LIVRO_ID: TBCDField;
    sqlSelosNaoValidadosNUMERO: TBCDField;
    sqlSelosNaoValidadosSIGLA: TStringField;
    sqlSelosNaoValidadosCALC_NUMERO_SELO: TStringField;
    sqlSelosNaoValidadosVALIDADO: TStringField;
    sqlSelosNaoValidadosQTDE: TIntegerField;
    ...
    procedure sqlSelosNaoValidadosCalcFields(DataSet: TDataSet);
    procedure FormShow(Sender: TObject);
  private
    { Private declarations }
    vlCodigoCartorio,
    vlChaveCartorio, vlPermissao_ControleMaster, vlPastaExportacao : string;
    vlCodigoExportacao : Integer;
    vlPesquisaCodigoExp : Boolean;
...
uses
  ...
  ADM.Controller.Factory.Entity.EnvioSelosTJ,
  ControleSelos,
...
begin
  ...
  begin
    ...
    if CharInSet(Valor[i], ['0'..'9', 'a'..'z', 'A'..'Z', '-', '/', '.', '_', ' ', 'à', 'â', 'ê', 'ô', 'û', 'ã', 'õ', 'á', 'é', 'í', 'ó' ,'ú', 'ç', 'ü', 'À', 'Â', 'Ê', 'Ô', 'Û', 'Ã', 'Õ', 'Á', 'É', 'Í', 'Ó', 'Ú', 'Ç', 'Ü']) then
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        ParamByName('CODIGO_EXPORTACAO').AsInteger := vlCodigoExportacao;
        ExecSQL;
      end;
      viSelos := '';
      viCont  := 0;
  ...
  begin
    try
      ...
      try
        ...
        viCont := 0;
        vlCodigoExportacao := dtmControles.GerarSequencia('SELO_CODIGO_EXPORTACAO');
        cdsSelos.First;
        while not cdsSelos.Eof do
        begin
  ...
  begin
    ...
    while viTentarEnvioXML do
      try
        ...
        if TMensagemUtils.ExibirMensagemPergunta('Deseja validar os Selos encaminhados?') then
        begin
          vlPesquisaCodigoExp := True;
          btnPesquisarClick(self);
          btnValidarClick(Self);
          vlPesquisaCodigoExp := False;
        end;
...
begin
  ...
  if chxFiltroSistema.Checked and (not vlPesquisaCodigoExp) then
  begin
  ...
  begin
    if (not vlPesquisaCodigoExp) then
    begin
    ...
    else viSql := viSql + ' AND SL.CODIGO_EXPORTACAO = '+vlCodigoEXPORTACAO.ToString;
...
procedure TfrmExportarSelo.BtnValidarTodosClick(Sender: TObject);
  ...
  begin
    ...
    btnXML.Enabled          := vpHabilitado;
    btnValidar.Enabled      := vpHabilitado;
    btnValidarTodos.Enabled := vpHabilitado;
    cxBtnFechar.Enabled     := vpHabilitado;
  end;

  procedure AplicarFiltrosRegistrosNaoValidados;
  begin
    rdbSelosNaoExp.Checked := False;
    rdbSelosTodos.Checked := True;
    chxSelosNaoValidados.Checked := True;
    chxListaSelos.Checked := False;
    rdbPeriodoQualquer.Checked := True;
    rdbPeriodoSelecionado.Checked := False;
    edtDataInicio.Clear;
    edtDataFinal.Clear;
    chxTribunal.Checked := False;
    chxFiltroSistema.Checked := False;
    icxTipoSistema.ItemIndex := 0;
    lcxTipoCodigo.Clear;
    lcxTipoSelo.Clear;
    lcxLote.Clear;
...
var
  ...
  viQryExecute: TI9Query;
begin
  Screen.Cursor := crHourGlass;
  sqlSelosNaoValidados.DisableControls;
  HabilitarBotoes(False);

  LblProgressBar.Visible := True;

  try
    sqlSelosNaoValidados.Close;
    sqlSelosNaoValidados.Open;

    sqlSelosNaoValidados.First;

    ProgressBar.Properties.Max := sqlSelosNaoValidadosQTDE.AsInteger;

    ProgressBar.Position := 0;
    ProgressBar.Properties.Text := IntToStr(Trunc(ProgressBar.Position / ProgressBar.Properties.Max * 100)) + '%';
    LblProgressBar.Caption := FloatToStr(ProgressBar.Position) + '/' + FloatToStr(ProgressBar.Properties.Max);

    viQryExecute := TI9Query.Create(nil);
    try
      viQryExecute.Close;
      viQryExecute.Connection := dtmControles.DB;
      viQryExecute.SQL.Clear;
      viQryExecute.SQL.Add(' UPDATE G_SELO_LIVRO ');
      viQryExecute.SQL.Add('    SET VALIDADO = ' + QuotedStr('S'));
      viQryExecute.SQL.Add('  WHERE SELO_LIVRO_ID = :SELO_LIVRO_ID ');

      while not sqlSelosNaoValidados.Eof do
      begin
        viSeloLivroId := sqlSelosNaoValidadosSELO_LIVRO_ID.AsInteger;

        if dtmControles.VerificarSeloTJ(sqlSelosNaoValidadosCALC_NUMERO_SELO.AsString) = 'S' then
        begin
          viQryExecute.Params.ParamByName('SELO_LIVRO_ID').AsInteger := viSeloLivroId;

          try
            viQryExecute.ExecSQL;
          except
            on e: Exception do
            begin
        ...
        ProgressBar.Properties.Text := IntToStr(Trunc(ProgressBar.Position / ProgressBar.Properties.Max * 100)) + '%';
        LblProgressBar.Caption := FloatToStr(ProgressBar.Position) + '/' + FloatToStr(ProgressBar.Properties.Max);
        sqlSelosNaoValidados.Next;
        Application.ProcessMessages;
      end;

    finally
      FreeAndNil(viQryExecute);
    end;

  finally
    AplicarFiltrosRegistrosNaoValidados;
    btnPesquisarClick(Sender);
    LblProgressBar.Visible := False;
    Screen.Cursor := crDefault;
    sqlSelosNaoValidados.EnableControls;
    HabilitarBotoes(True);
  end;
end;
...
begin
  ...
  chxVerificarSelos.Visible := (pos('2',viSistemas) > 0) or (pos('5',viSistemas) > 0);
...
procedure TfrmExportarSelo.FormShow(Sender: TObject);
begin
  BtnValidarTodos.Visible := vgLogin = 'TRI7';
end;
...
procedure TfrmExportarSelo.sqlSelosNaoValidadosCalcFields(DataSet: TDataSet);
begin
  sqlSelosNaoValidadosCALC_NUMERO_SELO.AsString := sqlSelosNaoValidadosSIGLA.AsString + RetZeros(sqlSelosNaoValidadosNUMERO.AsString, 6);
end;
