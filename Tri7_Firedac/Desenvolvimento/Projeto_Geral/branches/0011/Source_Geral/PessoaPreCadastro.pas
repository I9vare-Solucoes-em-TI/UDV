    ...
    CASE Controles.vgId OF
     ...
     2 : // Tabelionato de Notas
          viSql := ' SELECT FIRST 50 NOME, CPF_CNPJ, DOCUMENTO, PESSOA_TIPO, PESSOA_ID, CPF_TERCEIRO '+
                   ...
                   ' WHERE NOME LIKE '+ QuotedStr(edtNome.Text+'%');
     7 : // RTD
          viSql := ' SELECT FIRST 50 NOME, CPFCNPJ AS CPF_CNPJ, DOCUMENTO, '' '' PESSOA_TIPO, PESSOA_ID, '' '' CPF_TERCEIRO '+
                   ' FROM D_PESSOA '+
                   ' WHERE NOME LIKE '+ QuotedStr(edtNome.Text+'%');
     12 : // Registro Civil
          viSql := ' SELECT FIRST 50 NOME, CPF_CNPJ, DOCUMENTO, PESSOA_TIPO, PESSOA_ID, CPF_TERCEIRO '+
...
//      else viSql := viSql + ' AND (SEXO = '+ QuotedStr('M')+ ' OR SEXO IS NULL)';
      ...
      begin
        if Controles.vgId <> 7 then
          viSql := viSql + ' AND (PESSOA_TIPO = '+ QuotedStr(vgPessoa.PessoaTipo) + ' OR PESSOA_TIPO IS NULL)';
      end;
...
begin
  ...
  begin
    ...
    Caption := 'Pré-Cadastro Representante';
    edtNome.Left := 86;
    edtNome.Width := 592;
  end
  else
...
procedure TfrmPessoaPreCadastro.edtNomeKeyUp(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
