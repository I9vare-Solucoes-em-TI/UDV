...
type
  ...
  private
    ...
    procedure SaveTiffMultiPage(const vpImageEnMView: TImageEnMView);
    ...
    {$REGION 'Eventos para controle de digitalização em lote não usar os dois ao mesmo tempo'}
...
begin
  ...
  begin
    Application.MessageBox('Nenhum nome de arquivo foi informado!', 'Atenão', MB_OK + MB_ICONWARNING);
    Clear;
    Abort;
  end;
end;
...
begin
  ...
  viTextoIntervalo := InputBox('Informar o intevalo (Ex.: 1-3 ou 1;2;3;)','Páginas:', '');
  ...
  begin
    Application.MessageBox('Nenhum intevalo valido informado!', 'Atenção', MB_OK + MB_ICONWARNING);
    Exit;
  end;
...
begin
  ...
  begin
    ...
    if Application.MessageBox('Confirmar excluir arquivo selecionado?', 'Atenção', MB_YESNO + MB_ICONQUESTION + MB_DEFBUTTON2) = idNo then
      Exit;
...
begin
  DoBeforeDelete;
  if Application.MessageBox('Confirmar excluir página selecionada?', 'Atenção', MB_YESNO + MB_ICONQUESTION) = IDYES then
  begin
...
begin
  ...
  begin
    ...
    begin
      ...
      Screen.Cursor := crHourGlass;
      lblConvertendo.Caption := 'Excluindo página(s)...';
      lblConvertendo.Visible := ivAlways;
...
begin
  try
    ...
    FImageEnMView.MIO.DoPrintPreviewDialog('', False, 'Impressão');
  finally
    FImageEnMView.Clear;
  end;
...
begin
  if GetCount > 0 then
    if Application.MessageBox('Confirmar salvar imagem atual?', 'Atenção', MB_YESNO + MB_ICONQUESTION) = IDYES then
    begin
...
begin
  ...
  begin
    Application.MessageBox('Visualizar de imagem se encontra aberto, feche o mesmo para poder abrir em modo de edição!',
                           'Atenção',
...
begin
  ...
  begin
    ShowMessage('Esta versão requer o plug-in, v' + IEVC_EXPECTED_LIBRARY_VERSION);
    Abort;
  end;
...
begin
  ...
  begin
    Application.MessageBox('DPIX não pode estar vazio!', 'Atenção', MB_OK + MB_ICONWARNING);
    edtDPIX.SetFocus;
    Exit;
  end;
  ...
  begin
    Application.MessageBox('DPIY não pode estar vazio!', 'Atenção', MB_OK + MB_ICONWARNING);
    edtDPIY.SetFocus;
    Exit;
  end;
...
begin
  if GetCount = 0 then
  begin
    Application.MessageBox('Está rotina não pode ser executada, pois não existe nenhuma imagem selecionada para redigitalizar!', 'Atenção', MB_OK + MB_ICONWARNING);
    Exit;
  end;
...
procedure TfmeImagem.SaveTiffMultiPage(const vpImageEnMView: TImageEnMView);
var
 viCount,
 i: Integer;
 viImageEnVect : TImageEnVect;
            ...
            begin
              ...
              begin
                ...
                viFraseIdent := 'Documento assinado digitalmente.' +#13#10+
                                'Página: ' + IntToStr(i+1) + ' de ' + IntToStr(viPaginas-1) +#13#10+
                                'Data: ' + DateToStr(date);
                RotinasImagem.SetTextoRodapeImage(ImageEnViewPDF, viFraseIdent);
              end;
...
begin
  ...
  begin
    ...
    lblConvertendo.Caption   := 'Inciando Importação...';
    lblConvertendo.Visible   := ivAlways;
    Application.ProcessMessages;
      ...
      begin
        ...
        begin
          try
            ...
            imagem := RotinasImagem.GetPdfToTiff(fn);
          except
            on E: Exception do
            begin
              Messagedlg(E.Message, mtError, [mbOK], 0);
