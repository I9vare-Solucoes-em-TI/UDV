object frmExportarSelo: TfrmExportarSelo
  Left = 0
  Top = 0
  Caption = 'Exportar Selos'
  ClientHeight = 477
  ClientWidth = 1041
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  OnActivate = FormActivate
  OnClose = FormClose
  OnCreate = FormCreate
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object pnlConvenio: TPanel
    Left = 0
    Top = 0
    Width = 1041
    Height = 130
    Align = alTop
    BevelInner = bvLowered
    Color = 14807280
    ParentBackground = False
    TabOrder = 0
    object Panel4: TPanel
      Left = 969
      Top = 2
      Width = 70
      Height = 126
      Align = alRight
      BevelOuter = bvNone
      Color = 14807280
      ParentBackground = False
      TabOrder = 0
      object cxBtnFechar: TcxButton
        Left = 10
        Top = 7
        Width = 55
        Height = 45
        Cursor = crHandPoint
        Caption = 'Sair'
        Colors.Default = clWhite
        LookAndFeel.Kind = lfUltraFlat
        LookAndFeel.NativeStyle = False
        OptionsImage.Glyph.Data = {
          
          050505050505130D000000000000000000000000002409050505}
        OptionsImage.Layout = blGlyphTop
        ParentShowHint = False
        ShowHint = True
        TabOrder = 0
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
        OnClick = cxBtnFecharClick
      end
    end
    object gbxPeriodo: TcxPageControl
      Left = 304
      Top = 7
      Width = 225
      Height = 78
      TabOrder = 1
      Properties.ActivePage = tbcPeriodo
      Properties.CustomButtons.Buttons = <>
      Properties.Style = 1
      LookAndFeel.NativeStyle = True
      LookAndFeel.SkinName = 'Office2007Blue'
      ClientRectBottom = 74
      ClientRectLeft = 4
      ClientRectRight = 221
      ClientRectTop = 24
      object tbcPeriodo: TcxTabSheet
        Caption = 'Per'#237'odo'
        ImageIndex = 0
        object cxGroupBox5: TcxGroupBox
          Left = 4
          Top = 2
          ParentBackground = False
          ParentColor = False
          ParentFont = False
          Style.Color = clWhite
          Style.StyleController = dtmControles.cxEditStyleGroup
          Style.TextStyle = [fsBold]
          TabOrder = 0
          Height = 45
          Width = 209
          object edtDataInicio: TcxDateEdit
            Left = 10
            Top = 15
            ParentFont = False
            Style.StyleController = dtmControles.cxEsc_EditsNormal
            TabOrder = 0
            Width = 83
          end
          object edtDataFinal: TcxDateEdit
            Left = 114
            Top = 15
            ParentFont = False
            Style.StyleController = dtmControles.cxEsc_EditsNormal
            TabOrder = 1
            Width = 83
          end
          object cxLabel11: TcxLabel
            Left = 99
            Top = 16
            Caption = 'a'
            Transparent = True
          end
        end
      end
      object tbcListaSelos: TcxTabSheet
        Caption = 'Lista Selos'
        ImageIndex = 2
        object mmoLista: TcxMemo
          Left = 0
          Top = 0
          Align = alClient
          ParentFont = False
          Properties.ScrollBars = ssVertical
          Properties.WordWrap = False
          Style.StyleController = dtmControles.cxEsc_EditsNormal
          TabOrder = 0
          Height = 50
          Width = 217
        end
      end
    end
    object cxGroupBox3: TcxGroupBox
      Left = 7
      Top = 7
      Caption = ' Tipo da Pesquisa'
      Style.LookAndFeel.NativeStyle = False
      Style.LookAndFeel.SkinName = 'Black'
      Style.TextStyle = [fsBold]
      StyleDisabled.LookAndFeel.NativeStyle = False
      StyleDisabled.LookAndFeel.SkinName = 'Black'
      StyleFocused.LookAndFeel.NativeStyle = False
      StyleFocused.LookAndFeel.SkinName = 'Black'
      StyleHot.LookAndFeel.NativeStyle = False
      StyleHot.LookAndFeel.SkinName = 'Black'
      TabOrder = 2
      Transparent = True
      Height = 51
      Width = 293
      object rdbSelosNaoExp: TcxRadioButton
        Left = 12
        Top = 20
        Width = 125
        Height = 17
        Caption = 'Selos N'#227'o Exportados'
        Checked = True
        TabOrder = 0
        TabStop = True
        OnClick = rdbSelosNaoExpClick
        Transparent = True
      end
      object rdbSelosTodos: TcxRadioButton
        Left = 153
        Top = 20
        Width = 97
        Height = 17
        Caption = 'Todos os Selos'
        TabOrder = 1
        OnClick = rdbSelosTodosClick
        Transparent = True
      end
    end
    object gbxPesquisaFiltro: TcxGroupBox
      Left = 6
      Top = 86
      Caption = ' Filtro de Pesquisa'
      Style.BorderStyle = ebsNone
      Style.LookAndFeel.Kind = lfUltraFlat
      Style.LookAndFeel.NativeStyle = False
      Style.LookAndFeel.SkinName = ''
      Style.TextStyle = [fsBold]
      StyleDisabled.LookAndFeel.Kind = lfUltraFlat
      StyleDisabled.LookAndFeel.NativeStyle = False
      StyleDisabled.LookAndFeel.SkinName = ''
      StyleFocused.LookAndFeel.Kind = lfUltraFlat
      StyleFocused.LookAndFeel.NativeStyle = False
      StyleFocused.LookAndFeel.SkinName = ''
      StyleHot.LookAndFeel.Kind = lfUltraFlat
      StyleHot.LookAndFeel.NativeStyle = False
      StyleHot.LookAndFeel.SkinName = ''
      TabOrder = 3
      Transparent = True
      Height = 43
      Width = 361
      object rdbPeriodoSelecionado: TcxRadioButton
        Left = 154
        Top = 21
        Width = 125
        Height = 17
        Caption = 'Per'#237'odo Selecionado'
        TabOrder = 0
        OnClick = rdbPeriodoSelecionadoClick
        Transparent = True
      end
      object rdbPeriodoQualquer: TcxRadioButton
        Left = 11
        Top = 21
        Width = 117
        Height = 17
        Caption = 'Qualquer Per'#237'odo'
        Checked = True
        TabOrder = 1
        TabStop = True
        OnClick = rdbPeriodoQualquerClick
        Transparent = True
      end
    end
    object btnMarcar: TcxButton
      Left = 307
      Top = 103
      Width = 110
      Height = 19
      Cursor = crHandPoint
      Caption = 'Marcar Todos'
      Colors.Default = clBlue
      Colors.Normal = 15513994
      Colors.Hot = 8421440
      Colors.Pressed = clGreen
      Colors.Disabled = clBtnFace
      Enabled = False
      LookAndFeel.NativeStyle = False
      LookAndFeel.SkinName = 'Office2007Green'
      TabOrder = 4
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      OnClick = btnMarcarClick
    end
    object btnDesmarcar: TcxButton
      Left = 423
      Top = 103
      Width = 110
      Height = 19
      Cursor = crHandPoint
      Caption = 'Desmarcar Todos'
      Colors.Default = clBlue
      Colors.Normal = 15513994
      Colors.Hot = 8421440
      Colors.Pressed = clGreen
      Colors.Disabled = clBtnFace
      Enabled = False
      LookAndFeel.NativeStyle = False
      LookAndFeel.SkinName = 'Office2007Pink'
      TabOrder = 5
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      OnClick = btnDesmarcarClick
    end
    object btnPesquisar: TcxButton
      Left = 532
      Top = 47
      Width = 106
      Height = 25
      Caption = 'Pesquisar'
      Colors.Default = clWhite
      LookAndFeel.Kind = lfUltraFlat
      LookAndFeel.NativeStyle = False
      LookAndFeel.SkinName = ''
      OptionsImage.Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFF}
      TabOrder = 6
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      OnClick = btnPesquisarClick
    end
    object btnXML: TcxButton
      Left = 642
      Top = 47
      Width = 106
      Height = 25
      Caption = 'Gerar XML'
      Colors.Default = clWhite
      LookAndFeel.Kind = lfUltraFlat
      LookAndFeel.NativeStyle = False
      LookAndFeel.SkinName = ''
      OptionsImage.Glyph.Data = {
        
        DBFFFFFFFFFFFFFFFFFF}
      TabOrder = 7
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      OnClick = btnXMLClick
    end
    object ProgressBar: TcxProgressBar
      Left = 539
      Top = 103
      ParentFont = False
      Properties.AnimationRestartDelay = 1
      Properties.AnimationSpeed = 1
      Properties.Max = 1000.000000000000000000
      Properties.PeakSize = 1
      Properties.ShowTextStyle = cxtsText
      Style.StyleController = dtmControles.cxEsc_EditsNormal
      TabOrder = 8
      Width = 429
    end
    object chxTribunal: TcxCheckBox
      Left = 471
      Top = 8
      Caption = 'Tribunal'
      ParentFont = False
      Style.StyleController = dtmControles.cxEsc_EditsNormal
      TabOrder = 9
      Visible = False
      Width = 62
    end
    object btnValidar: TcxButton
      Left = 752
      Top = 47
      Width = 106
      Height = 25
      Caption = 'Validar TJ'
      Colors.Default = clWhite
      LookAndFeel.Kind = lfUltraFlat
      LookAndFeel.NativeStyle = False
      LookAndFeel.SkinName = ''
      OptionsImage.Glyph.Data = {
        
        2319080808080808080808080808080808080808080808080808}
      TabOrder = 10
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      OnClick = btnValidarClick
    end
    object chxListaSelos: TcxCheckBox
      Left = 154
      Top = 60
      Caption = 'Pesquisar lista de selos'
      ParentFont = False
      Style.StyleController = dtmControles.cxEsc_EditsNormal
      TabOrder = 11
      Transparent = True
      OnClick = chxListaSelosClick
      Width = 133
    end
    object chxSelosNaoValidados: TcxCheckBox
      Left = 12
      Top = 59
      Caption = 'Somente N'#227'o Validados'
      ParentFont = False
      Style.StyleController = dtmControles.cxEsc_EditsNormal
      TabOrder = 12
      Transparent = True
      Width = 137
    end
    object chxVerificarSelos: TcxCheckBox
      Left = 529
      Top = 26
      Caption = 'Verificar Data de Selos Anteriores a Tabela Vigente'
      ParentFont = False
      Style.StyleController = dtmControles.cxEsc_EditsNormal
      Style.TextColor = clRed
      StyleHot.TextColor = clRed
      TabOrder = 13
      Transparent = True
      Visible = False
      OnClick = chxVerificarSelosClick
      Width = 270
    end
    object BtnValidarTodos: TcxButton
      Left = 862
      Top = 47
      Width = 113
      Height = 25
      Caption = 'Validar Todos TJ'
      Colors.Default = clWhite
      LookAndFeel.Kind = lfUltraFlat
      LookAndFeel.NativeStyle = False
      LookAndFeel.SkinName = ''
      OptionsImage.Glyph.Data = {
        
        2319080808080808080808080808080808080808080808080808}
      TabOrder = 14
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      OnClick = BtnValidarTodosClick
    end
    object LblProgressBar: TcxLabel
      Left = 543
      Top = 103
      Caption = '0/0'
      ParentFont = False
      Style.StyleController = dtmControles.cxStyleLabel
      Style.TextStyle = []
      Transparent = True
      Visible = False
    end
  end
  object Panel11: TPanel
    Left = 0
    Top = 212
    Width = 1041
    Height = 24
    Align = alTop
    BevelOuter = bvNone
    Color = 14807280
    Ctl3D = True
    ParentBackground = False
    ParentCtl3D = False
    TabOrder = 1
    object Image1: TImage
      Left = 11
      Top = 3
      Width = 20
      Height = 20
      Picture.Data = {
        
        FF00}
      Transparent = True
    end
    object Image2: TImage
      Left = 103
      Top = 4
      Width = 20
      Height = 20
      Picture.Data = {
        
        FF00}
      Transparent = True
    end
    object cxLabel10: TcxLabel
      Left = 36
      Top = 3
      Caption = 'Exportado'
      Style.TextColor = clGreen
      Transparent = True
    end
    object cxLabel5: TcxLabel
      Left = 125
      Top = 3
      Caption = 'Aguardando'
      Style.TextColor = clRed
      Transparent = True
    end
  end
  object pnlPesquisaSistema: TPanel
    Left = 0
    Top = 130
    Width = 1041
    Height = 54
    Align = alTop
    BevelOuter = bvLowered
    Color = 13034466
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clRed
    Font.Height = -16
    Font.Name = 'Times New Roman'
    Font.Style = [fsBold]
    ParentBackground = False
    ParentFont = False
    TabOrder = 2
    object chxFiltroSistema: TCheckBox
      Left = 21
      Top = 21
      Width = 119
      Height = 17
      Caption = 'Filtro por Sistema'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = 4210688
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
      OnClick = chxFiltroSistemaClick
    end
    object cxGroupBox1: TcxGroupBox
      Left = 333
      Top = 2
      Caption = 'Selo Lote'
      ParentFont = False
      Style.Font.Charset = DEFAULT_CHARSET
      Style.Font.Color = clRed
      Style.Font.Height = -11
      Style.Font.Name = 'Tahoma'
      Style.Font.Style = [fsBold]
      Style.IsFontAssigned = True
      TabOrder = 1
      Height = 48
      Width = 131
      object SpeedButton1: TSpeedButton
        Left = 93
        Top = 17
        Width = 24
        Height = 22
        Cursor = crHandPoint
        Flat = True
        Glyph.Data = {
          
          FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
        OnClick = SpeedButton1Click
      end
      object lcxTipoCodigo: TcxLookupComboBox
        Left = 6
        Top = 18
        Enabled = False
        ParentFont = False
        Properties.DropDownListStyle = lsFixedList
        Properties.GridMode = True
        Properties.KeyFieldNames = 'SELO_GRUPO_ID'
        Properties.ListColumns = <
          item
            FieldName = 'NUMERO'
          end>
        Properties.ListOptions.AnsiSort = True
        Properties.ListOptions.ShowHeader = False
        Properties.ListSource = dtsTipoCodigo
        Properties.OnEditValueChanged = lcxTipoCodigoPropertiesEditValueChanged
        Style.Font.Charset = DEFAULT_CHARSET
        Style.Font.Color = clWindowText
        Style.Font.Height = -13
        Style.Font.Name = 'Times New Roman'
        Style.Font.Style = [fsBold]
        Style.StyleController = dtmControles.cxEsc_EditsNormal
        Style.TextColor = clWindowText
        Style.TextStyle = []
        Style.IsFontAssigned = True
        TabOrder = 0
        OnEnter = lcxTipoCodigoEnter
        Width = 84
      end
    end
    object cxLabel1: TcxLabel
      Left = 144
      Top = 2
      Caption = 'Serventia'
      ParentFont = False
      Style.StyleController = dtmControles.cxStyleLabel
      Style.TextStyle = [fsUnderline]
      Transparent = True
    end
    object icxTipoSistema: TcxImageComboBox
      Left = 141
      Top = 20
      ParentFont = False
      Properties.Items = <>
      Properties.OnEditValueChanged = icxTipoSistemaPropertiesEditValueChanged
      Style.Font.Charset = DEFAULT_CHARSET
      Style.Font.Color = clWindowText
      Style.Font.Height = -13
      Style.Font.Name = 'Times New Roman'
      Style.Font.Style = []
      Style.StyleController = dtmControles.cxEsc_EditsNormal
      Style.IsFontAssigned = True
      TabOrder = 3
      Width = 186
    end
    object cxGroupBox4: TcxGroupBox
      Left = 470
      Top = 2
      Caption = 'Selo Grupo         Nota Fiscal'
      ParentFont = False
      Style.Font.Charset = DEFAULT_CHARSET
      Style.Font.Color = clRed
      Style.Font.Height = -11
      Style.Font.Name = 'Tahoma'
      Style.Font.Style = [fsBold]
      Style.IsFontAssigned = True
      TabOrder = 4
      Height = 48
      Width = 217
      object SpeedButton2: TSpeedButton
        Left = 185
        Top = 17
        Width = 24
        Height = 22
        Cursor = crHandPoint
        Flat = True
        Glyph.Data = {
          
          FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
        OnClick = SpeedButton2Click
      end
      object lcxTipoSelo: TcxLookupComboBox
        Left = 5
        Top = 18
        Enabled = False
        ParentFont = False
        Properties.DropDownListStyle = lsEditList
        Properties.GridMode = True
        Properties.KeyFieldNames = 'SELO_GRUPO_ID'
        Properties.ListColumns = <
          item
            FieldName = 'NUMERO'
          end>
        Properties.ListOptions.ShowHeader = False
        Properties.ListSource = dtsTipoSelo
        Properties.OnEditValueChanged = lcxTipoSeloPropertiesEditValueChanged
        Style.Font.Charset = DEFAULT_CHARSET
        Style.Font.Color = clWindowText
        Style.Font.Height = -13
        Style.Font.Name = 'Times New Roman'
        Style.Font.Style = [fsBold]
        Style.StyleController = dtmControles.cxEsc_EditsNormal
        Style.TextColor = clWindowText
        Style.TextStyle = []
        Style.IsFontAssigned = True
        TabOrder = 0
        OnEnter = lcxTipoSeloEnter
        Width = 84
      end
      object lcxLote: TcxLookupComboBox
        Left = 95
        Top = 18
        Enabled = False
        ParentFont = False
        Properties.DropDownListStyle = lsFixedList
        Properties.GridMode = True
        Properties.KeyFieldNames = 'SELO_LOTE_ID'
        Properties.ListColumns = <
          item
            FieldName = 'NOTA_FISCAL'
          end>
        Properties.ListOptions.ShowHeader = False
        Properties.ListSource = dtsSeloLote
        Properties.OnEditValueChanged = lcxTipoSeloPropertiesEditValueChanged
        Style.Font.Charset = DEFAULT_CHARSET
        Style.Font.Color = clWindowText
        Style.Font.Height = -13
        Style.Font.Name = 'Times New Roman'
        Style.Font.Style = [fsBold]
        Style.StyleController = dtmControles.cxEsc_EditsNormal
        Style.TextColor = clWindowText
        Style.TextStyle = []
        Style.IsFontAssigned = True
        TabOrder = 1
        Width = 84
      end
    end
  end
  object cxPageControl1: TcxPageControl
    Left = 0
    Top = 236
    Width = 1041
    Height = 241
    Align = alClient
    TabOrder = 3
    Properties.ActivePage = tabSelosPesquisados
    Properties.CustomButtons.Buttons = <>
    Properties.Style = 6
    LookAndFeel.NativeStyle = False
    LookAndFeel.SkinName = ''
    ClientRectBottom = 241
    ClientRectRight = 1041
    ClientRectTop = 24
    object tabSelosPesquisados: TcxTabSheet
      Caption = 'Selos Pesquisados'
      ImageIndex = 0
      object gridExportar: TcxGrid
        Left = 0
        Top = 0
        Width = 1041
        Height = 217
        Align = alClient
        PopupMenu = PopupMenu1
        TabOrder = 0
        LookAndFeel.Kind = lfUltraFlat
        LookAndFeel.NativeStyle = False
        object gridSelos: TcxGridDBTableView
          Navigator.Buttons.CustomButtons = <>
          Navigator.Buttons.PriorPage.Visible = False
          Navigator.Buttons.NextPage.Visible = False
          Navigator.Buttons.Insert.Visible = False
          Navigator.Buttons.Delete.Visible = False
          Navigator.Buttons.Edit.Visible = False
          Navigator.Buttons.Post.Visible = False
          Navigator.Buttons.Cancel.Visible = False
          Navigator.Buttons.Refresh.Visible = False
          Navigator.Buttons.SaveBookmark.Visible = False
          Navigator.Buttons.GotoBookmark.Visible = False
          Navigator.Buttons.Filter.Visible = False
          Navigator.Visible = True
          OnCustomDrawCell = gridSelosCustomDrawCell
          DataController.DataSource = dtsSelos
          DataController.Summary.DefaultGroupSummaryItems = <>
          DataController.Summary.FooterSummaryItems = <
            item
              Format = 'Total: 0'
              Kind = skCount
              FieldName = 'NOTA_FISCAL'
              Column = gridSelosNOTA_FISCAL
            end
            item
              Format = ',#0.00'
              Kind = skSum
              FieldName = 'VALOR_EMOLUMENTO'
              Column = gridSelosVALOR_EMOLUMENTO
            end>
          DataController.Summary.SummaryGroups = <>
          OptionsBehavior.ColumnHeaderHints = False
          OptionsCustomize.ColumnFiltering = False
          OptionsData.Deleting = False
          OptionsData.Inserting = False
          OptionsView.Footer = True
          OptionsView.GridLines = glVertical
          OptionsView.GroupByBox = False
          Preview.Column = gridSelosColumn1
          Preview.Visible = True
          Styles.StyleSheet = dtmControles.GridTableViewStyleSheetDevExpress
          object gridSelosColumn1: TcxGridDBColumn
            DataBinding.FieldName = 'CALC_CORRIGIDO'
          end
          object gridSelosEXPORTAR: TcxGridDBColumn
            Caption = 'Exportar'
            DataBinding.FieldName = 'CALC_SELECIONADO'
            PropertiesClassName = 'TcxCheckBoxProperties'
            Width = 60
          end
          object gridSelosExportado: TcxGridDBColumn
            Caption = 'Exportado'
            DataBinding.FieldName = 'CALC_EXPORTADO'
            PropertiesClassName = 'TcxImageComboBoxProperties'
            Properties.Images = dtmControles.imgCadBasicos
            Properties.Items = <
              item
                ImageIndex = 6
                Value = True
              end
              item
                ImageIndex = 8
                Value = False
              end>
            Options.Editing = False
            Options.Focusing = False
            Width = 64
          end
          object gridSelosColumn3: TcxGridDBColumn
            Caption = 'Validado TJ'
            DataBinding.FieldName = 'VALIDADO_TJ'
            PropertiesClassName = 'TcxImageComboBoxProperties'
            Properties.Images = dtmControles.imgCadBasicos
            Properties.Items = <
              item
                Description = 'Validado'
                ImageIndex = 6
                Value = 'S'
              end
              item
                ImageIndex = 7
              end
              item
                ImageIndex = 7
                Value = 'N'
              end>
            Options.Editing = False
            Options.Focusing = False
            Width = 81
          end
          object gridSelosNOTA_FISCAL: TcxGridDBColumn
            Caption = 'Id Pedido'
            DataBinding.FieldName = 'NOTA_FISCAL'
            Options.Editing = False
            Options.Focusing = False
            Width = 80
          end
          object gridSelosNUMERO_AGRUPADOR: TcxGridDBColumn
            Caption = 'C'#243'digo Selo (Agrupador)'
            DataBinding.FieldName = 'NUMERO_AGRUPADOR'
            Options.Editing = False
            Options.Focusing = False
            Width = 150
          end
          object gridSelosColumn2: TcxGridDBColumn
            Caption = 'Ordem Agrupador'
            DataBinding.FieldName = 'ORDEM'
            Options.Editing = False
            Options.Focusing = False
            Width = 104
          end
          object gridSelosNUMERO_SELO: TcxGridDBColumn
            Caption = 'C'#243'digo Ato'
            DataBinding.FieldName = 'cal_numero_selo'
            Options.Editing = False
            Options.Focusing = False
            Width = 150
          end
          object gridSelosTIPO_ATO: TcxGridDBColumn
            Caption = 'Tipo Ato'
            DataBinding.FieldName = 'TIPO_ATO'
            PropertiesClassName = 'TcxLabelProperties'
            Options.Editing = False
            Options.Focusing = False
            Width = 60
          end
          object gridSelosDATA: TcxGridDBColumn
            Caption = 'Data/Hora'
            DataBinding.FieldName = 'DATA'
            Options.Editing = False
            Options.Focusing = False
            Width = 120
          end
          object gridSelosAPRESENTANTE: TcxGridDBColumn
            Caption = 'Nome Civil'
            DataBinding.FieldName = 'APRESENTANTE'
            Options.Editing = False
            Options.Focusing = False
            Width = 162
          end
          object gridSelosNOME_COMPLETO: TcxGridDBColumn
            Caption = 'Nome Serventu'#225'rio'
            DataBinding.FieldName = 'NOME_COMPLETO'
            Options.Editing = False
            Options.Focusing = False
            Width = 250
          end
          object gridSelosIP_MAQUINA: TcxGridDBColumn
            Caption = 'IP M'#225'quina'
            DataBinding.FieldName = 'IP_MAQUINA'
            Options.Editing = False
            Options.Focusing = False
            Width = 80
          end
          object gridSelosVALOR_EMOLUMENTO: TcxGridDBColumn
            Caption = 'Valor Emolumento'
            DataBinding.FieldName = 'VALOR_EMOLUMENTO'
            Options.Editing = False
            Options.Focusing = False
            Width = 120
          end
          object gridSelosVALOR_TAXA_JUDICIARIA: TcxGridDBColumn
            Caption = 'Valor Tx. Judici'#225'ria'
            DataBinding.FieldName = 'VALOR_TAXA_JUDICIARIA'
            Options.Editing = False
            Options.Focusing = False
            Width = 120
          end
          object gridSelosVALOR_FUNDESP: TcxGridDBColumn
            Caption = 'Valor Fundos'
            DataBinding.FieldName = 'VALOR_FUNDESP'
            Options.Editing = False
            Options.Focusing = False
            Width = 120
          end
          object gridSelosVALOR_TOTAL: TcxGridDBColumn
            Caption = 'Valor Total'
            DataBinding.FieldName = 'VALOR_TOTAL'
            Options.Editing = False
            Options.Focusing = False
          end
        end
        object gridExportarLevel1: TcxGridLevel
          GridView = gridSelos
        end
      end
    end
    object tabSelosEstoque: TcxTabSheet
      Caption = 'ATEN'#199#195'O: Selos no estoque m'#237'nimo (Pressione para verificar)'
      Highlighted = True
      ImageIndex = 1
      TabVisible = False
      ExplicitTop = 0
      ExplicitWidth = 0
      ExplicitHeight = 0
      object grdBasica: TcxGrid
        Left = 0
        Top = 0
        Width = 1041
        Height = 217
        Align = alClient
        TabOrder = 0
        LookAndFeel.NativeStyle = False
        object grdBasicaDBTableView1: TcxGridDBTableView
          Navigator.Buttons.CustomButtons = <>
          Navigator.Buttons.PriorPage.Visible = False
          Navigator.Buttons.NextPage.Visible = False
          Navigator.Buttons.Insert.Visible = False
          Navigator.Buttons.Delete.Visible = False
          Navigator.Buttons.Edit.Visible = False
          Navigator.Buttons.Post.Visible = False
          Navigator.Buttons.Cancel.Visible = False
          Navigator.Buttons.Refresh.Visible = False
          Navigator.Buttons.SaveBookmark.Visible = False
          Navigator.Buttons.GotoBookmark.Visible = False
          Navigator.Buttons.Filter.Visible = False
          Navigator.Visible = True
          DataController.Summary.DefaultGroupSummaryItems = <>
          DataController.Summary.FooterSummaryItems = <
            item
              Format = 'Total:0'
              Kind = skCount
              FieldName = 'NATUREZA_ID'
            end>
          DataController.Summary.SummaryGroups = <>
          OptionsBehavior.ColumnHeaderHints = False
          OptionsCustomize.ColumnFiltering = False
          OptionsData.CancelOnExit = False
          OptionsData.Deleting = False
          OptionsData.DeletingConfirmation = False
          OptionsData.Editing = False
          OptionsData.Inserting = False
          OptionsSelection.CellSelect = False
          OptionsView.ColumnAutoWidth = True
          OptionsView.GroupByBox = False
          Styles.StyleSheet = dtmControles.GridTableViewStyleSheetDevExpress
          object cxGridBasicaDDGrupo: TcxGridDBColumn
            Caption = 'Grupo'
            DataBinding.FieldName = 'SELO_GRUPO_ID'
            PropertiesClassName = 'TcxLookupComboBoxProperties'
            Properties.KeyFieldNames = 'SELO_GRUPO_ID'
            Properties.ListColumns = <
              item
                FieldName = 'DESCRICAO_COMPLETA'
              end>
            RepositoryItem = dtmLookup.Lista_SeloGrupo
            Width = 214
          end
          object grdBasicaDBTableView1SELO_GRUPO_ID: TcxGridDBColumn
            Caption = 'Codigo'
            DataBinding.FieldName = 'SELO_GRUPO_ID'
            PropertiesClassName = 'TcxLookupComboBoxProperties'
            Properties.Alignment.Horz = taCenter
            Properties.KeyFieldNames = 'SELO_GRUPO_ID'
            Properties.ListColumns = <
              item
                FieldName = 'NUMERO'
              end>
            Width = 43
          end
          object grdBasicaDBTableView1TIPO_CARTORIO: TcxGridDBColumn
            Caption = 'Tipo Cart'#243'rio'
            DataBinding.FieldName = 'SELO_GRUPO_ID'
            PropertiesClassName = 'TcxLookupComboBoxProperties'
            Properties.KeyFieldNames = 'SELO_GRUPO_ID'
            Properties.ListColumns = <
              item
                FieldName = 'DESCRICAO_TIPO_CARTORIO'
              end>
            Width = 149
          end
          object grdBasicaDBTableView1DATA_LOTE: TcxGridDBColumn
            Caption = 'Data'
            DataBinding.FieldName = 'DATA_LOTE'
            PropertiesClassName = 'TcxDateEditProperties'
            Width = 55
          end
          object grdBasicaDBTableView1SIGLA: TcxGridDBColumn
            Caption = 'Sigla'
            DataBinding.FieldName = 'SIGLA'
            Width = 64
          end
          object grdBasicaDBTableView1CALC_NUMERO_INI: TcxGridDBColumn
            Caption = 'N'#186' Inicial'
            DataBinding.FieldName = 'CALC_NUMERO_INI'
            Width = 65
          end
          object grdBasicaDBTableView1CALC_NUMERO_FIM: TcxGridDBColumn
            Caption = 'N'#186' Final'
            DataBinding.FieldName = 'CALC_NUMERO_FIM'
            Width = 66
          end
          object cxGridBasicaDBSituacao: TcxGridDBColumn
            Caption = 'Situa'#231#227'o'
            DataBinding.FieldName = 'SITUACAO'
            PropertiesClassName = 'TcxImageComboBoxProperties'
            Properties.Images = dtmControles.imgCadBasicos
            Properties.Items = <
              item
                Description = 'Aguardando Uso'
                ImageIndex = 8
                Value = 'A'
              end
              item
                Description = 'Fechado'
                ImageIndex = 9
                Value = 'F'
              end
              item
                Description = 'Ativado'
                ImageIndex = 47
                Value = 'I'
              end>
            Width = 66
          end
          object grdBasicaDBTableView1NOTA_FISCAL: TcxGridDBColumn
            Caption = 'Nota Fiscal'
            DataBinding.FieldName = 'NOTA_FISCAL'
            Width = 62
          end
        end
        object grdBasicaLevel1: TcxGridLevel
          GridView = grdBasicaDBTableView1
        end
      end
    end
  end
  object pnlImpressaoTexto: TPanel
    Left = 0
    Top = 184
    Width = 1041
    Height = 28
    Align = alTop
    BevelOuter = bvLowered
    Caption = 'Registros n'#227'o validados. Verifique!!!'
    Color = 13034466
    Enabled = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clRed
    Font.Height = -16
    Font.Name = 'Times New Roman'
    Font.Style = [fsBold]
    ParentBackground = False
    ParentFont = False
    TabOrder = 4
    Visible = False
  end
  object sqlSelos: TI9Query
    OnCalcFields = sqlSelosCalcFields
    Connection = dtmControles.DB
    FetchOptions.AssignedValues = [evMode]
    FetchOptions.Mode = fmAll
    SQL.Strings = (
        'SELECT SO.NOTA_FISCAL,'#13#10'       SL.SELO_LIVRO_ID, '#13#10'       SL.COD' +
        'IGO_EXPORTACAO,'#13#10'       SL.NUMERO_AGRUPADOR,'#13#10'       SL.SIGLA, S' +
        'L.NUMERO,'#13#10'       SG.NUMERO AS TIPO_ATO,'#13#10'       SL.APRESENTANTE' +
        ','#13#10'       U.NOME_COMPLETO,'#13#10'       SL.DATA,'#13#10'       SL.IP_MAQUIN' +
        'A,'#13#10'       SL.VALOR_TOTAL,'#13#10'       SL.VALOR_EMOLUMENTO,'#13#10'       ' +
        'SL.VALOR_TAXA_JUDICIARIA,'#13#10'       SL.VALOR_FUNDESP,'#13#10'       SL.N' +
        'UMERO_AGRUPADOR AS AGRUPADOR,'#13#10'       SL.CODIGO_GRATUITO,'#13#10'     ' +
        '  SL.VALIDADO,'#13#10'       SL.PROTOCOLO,'#13#10'       SG.SISTEMA_ID'#13#10'FROM' +
        ' G_SELO_LIVRO SL'#13#10'LEFT JOIN G_SELO_LOTE SO ON'#13#10'  SL.SELO_LOTE_ID' +
        ' = SO.SELO_LOTE_ID'#13#10'LEFT JOIN G_SELO_GRUPO SG ON'#13#10'  SO.SELO_GRUP' +
        'O_ID = SG.SELO_GRUPO_ID'#13#10'LEFT JOIN G_USUARIO U ON'#13#10'  SL.USUARIO_' +
        'ID = U.USUARIO_ID'#13#10'WHERE SL.CAMPO_ID = 4120')
    Left = 11
    Top = 271
    object sqlSelosNOTA_FISCAL: TStringField
      FieldName = 'NOTA_FISCAL'
      Size = 30
    end
    object sqlSelosNUMERO_AGRUPADOR: TStringField
      FieldName = 'NUMERO_AGRUPADOR'
      Size = 30
    end
    object sqlSelosSIGLA: TStringField
      FieldName = 'SIGLA'
      Size = 30
    end
    object sqlSelosNUMERO: TBCDField
      FieldName = 'NUMERO'
      Precision = 20
      Size = 2
    end
    object sqlSelosTIPO_ATO: TBCDField
      FieldName = 'TIPO_ATO'
      Precision = 20
      Size = 2
    end
    object sqlSelosAPRESENTANTE: TStringField
      FieldName = 'APRESENTANTE'
      Size = 120
    end
    object sqlSelosNOME_COMPLETO: TStringField
      FieldName = 'NOME_COMPLETO'
      Size = 120
    end
    object sqlSelosDATA: TSQLTimeStampField
      FieldName = 'DATA'
    end
    object sqlSelosIP_MAQUINA: TStringField
      FieldName = 'IP_MAQUINA'
      Size = 30
    end
    object sqlSelosVALOR_TOTAL: TBCDField
      FieldName = 'VALOR_TOTAL'
      currency = True
      Precision = 20
      Size = 3
    end
    object sqlSelosVALOR_EMOLUMENTO: TBCDField
      FieldName = 'VALOR_EMOLUMENTO'
      currency = True
      Precision = 20
      Size = 3
    end
    object sqlSelosVALOR_TAXA_JUDICIARIA: TBCDField
      FieldName = 'VALOR_TAXA_JUDICIARIA'
      currency = True
      Precision = 20
      Size = 3
    end
    object sqlSelosVALOR_FUNDESP: TBCDField
      FieldName = 'VALOR_FUNDESP'
      currency = True
      Precision = 20
      Size = 3
    end
    object sqlSelosCODIGO_GRATUITO: TBCDField
      FieldName = 'CODIGO_GRATUITO'
      Precision = 20
      Size = 2
    end
    object sqlSelosPROTOCOLO: TBCDField
      FieldName = 'PROTOCOLO'
      Precision = 20
      Size = 2
    end
    object sqlSeloscal_numero_selo: TStringField
      DisplayWidth = 30
      FieldKind = fkCalculated
      FieldName = 'cal_numero_selo'
      Size = 30
      Calculated = True
    end
    object sqlSelosCALC_SELECIONADO: TBooleanField
      FieldKind = fkInternalCalc
      FieldName = 'CALC_SELECIONADO'
    end
    object sqlSelosSELO_LIVRO_ID: TBCDField
      FieldName = 'SELO_LIVRO_ID'
      Precision = 20
      Size = 2
    end
    object sqlSelosCALC_EXPORTADO: TBooleanField
      FieldKind = fkCalculated
      FieldName = 'CALC_EXPORTADO'
      Calculated = True
    end
    object sqlSelosCODIGO_EXPORTACAO: TBCDField
      FieldName = 'CODIGO_EXPORTACAO'
      Precision = 20
      Size = 2
    end
    object sqlSelosVALIDADO: TStringField
      FieldName = 'VALIDADO'
      Size = 1
    end
    object sqlSelosSISTEMA_ID: TBCDField
      FieldName = 'SISTEMA_ID'
      Precision = 20
      Size = 2
    end
  end
  object dtsSelos: TDataSource
    DataSet = cdsSelos
    Left = 100
    Top = 272
  end
  object SaveDialog1: TSaveDialog
    DefaultExt = '.xml'
    Filter = 'XML|*.XML'
    Left = 284
    Top = 264
  end
  object dtsDataExportacao: TDataSource
    Left = 35
    Top = 383
  end
  object PopupMenu1: TPopupMenu
    Left = 144
    Top = 272
    object actCorrigirNome: TMenuItem
      Caption = 'Corrigir Nome Civil'
      OnClick = actCorrigirNomeClick
    end
    object CorrigirTodosPesquisados1: TMenuItem
      Caption = 'Corrigir Todos Pesquisados'
      OnClick = CorrigirTodosPesquisados1Click
    end
    object N1: TMenuItem
      Caption = '-'
    end
    object CorrigirServenturio1: TMenuItem
      Caption = 'Corrigir Serventu'#225'rio'
      OnClick = CorrigirServenturio1Click
    end
    object N2: TMenuItem
      Caption = '-'
    end
    object ExportarparaExcel1: TMenuItem
      Caption = 'Exportar para Excel'
      OnClick = ExportarparaExcel1Click
    end
  end
  object sqlTipoSelo: TI9Query
    Connection = dtmControles.DB
    FetchOptions.AssignedValues = [evMode]
    FetchOptions.Mode = fmAll
    SQL.Strings = (
        'SELECT SELO_GRUPO_ID, DESCRICAO_COMPLETA, NUMERO'#13#10'FROM G_SELO_GR' +
        'UPO'#13#10'WHERE TIPO_CARTORIO = :TIPO_CARTORIO'#13#10'     AND SITUACAO = '#39 +
        'A'#39#13#10'ORDER BY NUMERO')
    Left = 134
    Top = 323
    ParamData = <
      item
        Name = 'TIPO_CARTORIO'
        DataType = ftBCD
        ParamType = ptInput
      end>
    object sqlTipoSeloSELO_GRUPO_ID: TBCDField
      FieldName = 'SELO_GRUPO_ID'
      Precision = 20
      Size = 2
    end
    object sqlTipoSeloDESCRICAO_COMPLETA: TStringField
      FieldName = 'DESCRICAO_COMPLETA'
      Size = 260
    end
    object sqlTipoSeloNUMERO: TBCDField
      FieldName = 'NUMERO'
      Precision = 20
      Size = 2
    end
  end
  object dtsTipoSelo: TDataSource
    DataSet = sqlTipoSelo
    Left = 135
    Top = 396
  end
  object sqlSeloLote: TI9Query
    IndexFieldNames = 'SELO_GRUPO_ID'
    MasterSource = dtsTipoSelo
    MasterFields = 'SELO_GRUPO_ID'
    Connection = dtmControles.DB
    FetchOptions.AssignedValues = [evMode]
    FetchOptions.Mode = fmAll
    SQL.Strings = (
        'SELECT SELO_LOTE_ID, NOTA_FISCAL, SELO_GRUPO_ID'#13#10'FROM G_SELO_LOT' +
        'E'#13#10'WHERE SELO_GRUPO_ID = :SELO_GRUPO_ID'#13#10'ORDER BY NOTA_FISCAL')
    Left = 169
    Top = 324
    ParamData = <
      item
        Name = 'SELO_GRUPO_ID'
        DataType = ftBCD
        ParamType = ptInput
      end>
    object sqlSeloLoteSELO_LOTE_ID: TBCDField
      FieldName = 'SELO_LOTE_ID'
      Precision = 20
      Size = 2
    end
    object sqlSeloLoteNOTA_FISCAL: TStringField
      FieldName = 'NOTA_FISCAL'
      Size = 30
    end
    object sqlSeloLoteSELO_GRUPO_ID: TBCDField
      FieldName = 'SELO_GRUPO_ID'
      Precision = 20
      Size = 2
    end
  end
  object dtsSeloLote: TDataSource
    DataSet = sqlSeloLote
    Left = 169
    Top = 397
  end
  object cdsSelos: TClientDataSet
    PersistDataPacket.Data = {
      
      000200010001000D44454641554C545F4F524445520200820000000000}
    Active = True
    Aggregates = <>
    FieldDefs = <
      item
        Name = 'NOTA_FISCAL'
        DataType = ftString
        Size = 30
      end
      item
        Name = 'NUMERO_AGRUPADOR'
        DataType = ftString
        Size = 30
      end
      item
        Name = 'SIGLA'
        DataType = ftString
        Size = 30
      end
      item
        Name = 'NUMERO'
        DataType = ftBCD
        Precision = 20
        Size = 2
      end
      item
        Name = 'TIPO_ATO'
        DataType = ftBCD
        Precision = 20
        Size = 2
      end
      item
        Name = 'APRESENTANTE'
        DataType = ftString
        Size = 120
      end
      item
        Name = 'NOME_COMPLETO'
        DataType = ftString
        Size = 120
      end
      item
        Name = 'DATA'
        DataType = ftTimeStamp
      end
      item
        Name = 'IP_MAQUINA'
        DataType = ftString
        Size = 30
      end
      item
        Name = 'VALOR_TOTAL'
        DataType = ftBCD
        Precision = 20
        Size = 3
      end
      item
        Name = 'VALOR_EMOLUMENTO'
        DataType = ftBCD
        Precision = 20
        Size = 3
      end
      item
        Name = 'VALOR_TAXA_JUDICIARIA'
        DataType = ftBCD
        Precision = 20
        Size = 3
      end
      item
        Name = 'VALOR_FUNDESP'
        DataType = ftBCD
        Precision = 20
        Size = 3
      end
      item
        Name = 'CODIGO_GRATUITO'
        DataType = ftInteger
      end
      item
        Name = 'PROTOCOLO'
        DataType = ftInteger
      end
      item
        Name = 'cal_numero_selo'
        DataType = ftString
        Size = 30
      end
      item
        Name = 'CALC_SELECIONADO'
        DataType = ftBoolean
      end
      item
        Name = 'SELO_LIVRO_ID'
        DataType = ftBCD
        Precision = 20
        Size = 2
      end
      item
        Name = 'CALC_EXPORTADO'
        DataType = ftBoolean
      end
      item
        Name = 'CODIGO_EXPORTACAO'
        DataType = ftBCD
        Precision = 20
        Size = 2
      end
      item
        Name = 'VALIDADO_TJ'
        DataType = ftString
        Size = 1
      end
      item
        Name = 'SISTEMA_ID'
        DataType = ftBCD
        Precision = 20
        Size = 2
      end
      item
        Name = 'NOME_CORRIGIDO'
        DataType = ftString
        Size = 120
      end
      item
        Name = 'ORDEM'
        DataType = ftInteger
      end
      item
        Name = 'VALIDADO'
        DataType = ftString
        Size = 1
      end>
    IndexDefs = <
      item
        Name = 'DEFAULT_ORDER'
      end
      item
        Name = 'CHANGEINDEX'
      end>
    IndexFieldNames = 'ORDEM'
    Params = <>
    StoreDefs = True
    Left = 56
    Top = 273
    object cdsSelosNOTA_FISCAL: TStringField
      FieldName = 'NOTA_FISCAL'
      Size = 30
    end
    object cdsSelosNUMERO_AGRUPADOR: TStringField
      FieldName = 'NUMERO_AGRUPADOR'
      Size = 30
    end
    object cdsSelosSIGLA: TStringField
      FieldName = 'SIGLA'
      Size = 30
    end
    object cdsSelosNUMERO: TBCDField
      FieldName = 'NUMERO'
      Precision = 20
      Size = 2
    end
    object cdsSelosTIPO_ATO: TBCDField
      FieldName = 'TIPO_ATO'
      Precision = 20
      Size = 2
    end
    object cdsSelosAPRESENTANTE: TStringField
      FieldName = 'APRESENTANTE'
      Size = 120
    end
    object cdsSelosNOME_COMPLETO: TStringField
      FieldName = 'NOME_COMPLETO'
      Size = 120
    end
    object cdsSelosDATA: TSQLTimeStampField
      FieldName = 'DATA'
    end
    object cdsSelosIP_MAQUINA: TStringField
      FieldName = 'IP_MAQUINA'
      Size = 30
    end
    object cdsSelosVALOR_TOTAL: TBCDField
      FieldName = 'VALOR_TOTAL'
      currency = True
      Precision = 20
      Size = 3
    end
    object cdsSelosVALOR_EMOLUMENTO: TBCDField
      FieldName = 'VALOR_EMOLUMENTO'
      currency = True
      Precision = 20
      Size = 3
    end
    object cdsSelosVALOR_TAXA_JUDICIARIA: TBCDField
      FieldName = 'VALOR_TAXA_JUDICIARIA'
      currency = True
      Precision = 20
      Size = 3
    end
    object cdsSelosVALOR_FUNDESP: TBCDField
      FieldName = 'VALOR_FUNDESP'
      currency = True
      Precision = 20
      Size = 3
    end
    object cdsSelosCODIGO_GRATUITO: TIntegerField
      FieldName = 'CODIGO_GRATUITO'
    end
    object cdsSelosPROTOCOLO: TIntegerField
      FieldName = 'PROTOCOLO'
    end
    object cdsSeloscal_numero_selo: TStringField
      DisplayWidth = 30
      FieldName = 'cal_numero_selo'
      Size = 30
    end
    object cdsSelosCALC_SELECIONADO: TBooleanField
      FieldName = 'CALC_SELECIONADO'
    end
    object cdsSelosSELO_LIVRO_ID: TBCDField
      FieldName = 'SELO_LIVRO_ID'
      Precision = 20
      Size = 2
    end
    object cdsSelosCALC_EXPORTADO: TBooleanField
      FieldName = 'CALC_EXPORTADO'
    end
    object cdsSelosCODIGO_EXPORTACAO: TBCDField
      FieldName = 'CODIGO_EXPORTACAO'
      Precision = 20
      Size = 2
    end
    object cdsSelosVALIDADO_TJ: TStringField
      FieldName = 'VALIDADO_TJ'
      Size = 1
    end
    object cdsSelosSISTEMA_ID: TBCDField
      FieldName = 'SISTEMA_ID'
      Precision = 20
      Size = 2
    end
    object cdsSelosNOME_CORRIGIDO: TStringField
      FieldName = 'NOME_CORRIGIDO'
      Size = 120
    end
    object cdsSelosORDEM: TIntegerField
      FieldName = 'ORDEM'
    end
    object cdsSelosVALIDADO: TStringField
      FieldName = 'VALIDADO'
      Size = 1
    end
  end
  object sqlTipoCodigo: TI9Query
    Connection = dtmControles.DB
    FetchOptions.AssignedValues = [evMode]
    FetchOptions.Mode = fmAll
    SQL.Strings = (
        'SELECT SELO_GRUPO_ID, NUMERO'#13#10'FROM G_SELO_GRUPO'#13#10'WHERE TIPO_CART' +
        'ORIO = :TIPO_CARTORIO'#13#10'     AND SITUACAO = '#39'A'#39#13#10'ORDER BY NUMERO')
    Left = 254
    Top = 308
    ParamData = <
      item
        Name = 'TIPO_CARTORIO'
        DataType = ftBCD
        ParamType = ptInput
      end>
    object sqlTipoCodigoSELO_GRUPO_ID: TBCDField
      FieldName = 'SELO_GRUPO_ID'
      Precision = 20
      Size = 2
    end
    object sqlTipoCodigoNUMERO: TBCDField
      FieldName = 'NUMERO'
      Precision = 20
      Size = 2
    end
  end
  object dtsTipoCodigo: TDataSource
    DataSet = sqlTipoCodigo
    Left = 255
    Top = 340
  end
  object sqlSelosNaoValidados: TI9Query
    OnCalcFields = sqlSelosNaoValidadosCalcFields
    Connection = dtmControles.DB
    FetchOptions.AssignedValues = [evMode]
    FetchOptions.Mode = fmAll
    SQL.Strings = (
      'SELECT'
      '       SL.selo_livro_id,'
      '       SL.numero,'
      '       SL.sigla,'
      '       SL.validado,'
      '       QTD.QTDE'
      ''
      '  FROM g_selo_livro SL'
      ' INNER JOIN ('
      '              SELECT COUNT(SL.selo_livro_id) QTDE'
      '                FROM g_selo_livro SL'
      '               WHERE COALESCE(SL.validado, '#39'N'#39') <> '#39'S'#39
      '                 AND SL.selo_situacao_id IN (2, 4)'
      '             ) QTD ON QTD.QTDE > 0'
      ''
      ' WHERE COALESCE(SL.validado, '#39'N'#39') <> '#39'S'#39
      '   AND SL.selo_situacao_id IN (2, 4)')
    Left = 384
    Top = 315
    object sqlSelosNaoValidadosSELO_LIVRO_ID: TBCDField
      FieldName = 'SELO_LIVRO_ID'
      Origin = 'SELO_LIVRO_ID'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
      Precision = 18
      Size = 2
    end
    object sqlSelosNaoValidadosNUMERO: TBCDField
      FieldName = 'NUMERO'
      Origin = 'NUMERO'
      Precision = 18
      Size = 2
    end
    object sqlSelosNaoValidadosSIGLA: TStringField
      FieldName = 'SIGLA'
      Origin = 'SIGLA'
      Size = 30
    end
    object sqlSelosNaoValidadosCALC_NUMERO_SELO: TStringField
      DisplayWidth = 30
      FieldKind = fkCalculated
      FieldName = 'CALC_NUMERO_SELO'
      Size = 30
      Calculated = True
    end
    object sqlSelosNaoValidadosVALIDADO: TStringField
      FieldName = 'VALIDADO'
      Origin = 'VALIDADO'
      Size = 1
    end
    object sqlSelosNaoValidadosQTDE: TIntegerField
      AutoGenerateValue = arDefault
      FieldName = 'QTDE'
      Origin = 'QTDE'
      ProviderFlags = []
      ReadOnly = True
    end
  end
end
