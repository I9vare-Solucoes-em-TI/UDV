...
type
  ...
  TfrmCadastroRapidoTarefa = class(TForm)
    ...
    sqlTarefaOBSERVACAO: TBlobField;
    sqlTarefaSTATUS: TStringField;
    LblTarefaDesativada: TcxLabel;
...
var
  ...
  viTarefaId: Integer;
begin
  if sqlTarefaSTATUS.AsString = 'I' then
  begin
    TMensagemUtils.ExibirMensagemAtencao(
      'Esta tarefa foi desativada e não é permitido alterá-la!');
    Exit;
  end;
  ...
  begin
    if not TTarefaProcessos.PrecisaReprocessar(
    ...
    if not TMensagemUtils.ExibirMensagemPergunta(
      'Esta gravação irá recriar todas os itens desta tarefa!' + sLineBreak +
  ...
  if rgpTermino.ItemIndex < 0 then
  begin
    TMensagemUtils.ExibirMensagemAtencao(
      'Campo "Repetição termina em" não foi definido corretamente!');
    rgpTermino.SetFocus;
    Abort;
  end;
  ...
  if not TTarefaProcessos.ProcessarTarefa(
    ...
    sqlTarefaOBSERVACAO.AsString,
    sqlTarefaSTATUS.AsString,
    viListaUsuarioId,
    viDescMensagem) then
  begin
...
procedure TfrmCadastroRapidoTarefa.FormShow(
  Sender: TObject);
var
  viExisteSubTarefaRealizada: Boolean;
begin
  inherited;
  sqlTarefa.Close;
  ...
  if sqlTarefaTAREFA_ID.AsInteger > 0 then
  begin
    viExisteSubTarefaRealizada := TTarefaProcessos.ExisteSubTarefaRealizada(
      sqlTarefaTAREFA_ID.AsInteger);

    icxServentia.Enabled := not viExisteSubTarefaRealizada;
    lcxTarefaGrupo.Enabled := not viExisteSubTarefaRealizada;
    edtAssunto.Enabled := not viExisteSubTarefaRealizada;
  end;
  ...
  LblTarefaDesativada.Visible := sqlTarefaSTATUS.AsString = 'I';

  PreencherResponsaveis(vlTarefaId);

  if icxServentia.CanFocus then
    icxServentia.SetFocus
  else
    rdtMensagem.SetFocus;
end;
