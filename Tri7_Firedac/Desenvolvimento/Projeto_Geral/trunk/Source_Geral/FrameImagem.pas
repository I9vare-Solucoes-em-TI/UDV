  ...
  begin
    ...
    try
      ...
      if TwainPRO.ErrorCode >= 1000 then
        Application.MessageBox(PChar('Scanner não pode ser inicializado! Erro: ' + IntToStr(TwainPRO.ErrorCode)), 'Atenção', MB_OK + MB_ICONERROR);
    end;
  end
  else
  begin
    ...
    else ShowMessage('Twain não instalado!')
  end;
end;
...
procedure TfmeImagem.TwainPROPostScan(Sender: TObject;
  var Cancel: WordBool);
begin
  {Carrega imagem do scanner para o componente de exibição}
  if not cbxScanImageFloating.Checked then
  begin
    //Controla Nº paginas digitalizadas
    if vgControleNumeroPagina and (Xpress1.SaveMultiPage = True) then
      if vg_NumeroPaginas <= Xpress1.Pages then
      begin
        Cancel := True;
        Application.MessageBox('Número de páginas digitalizadas maior que número de paginas informadas!', 'Atenção', MB_OK + MB_ICONWARNING);
        Exit;
      end;
...
begin
  ...
  //Determina a posição central para ser inserida a imagem de carimbo
...
begin
  ...
  if Xpress1.Pages = 1 then
  begin
    Application.MessageBox('Adicione mais imagens para realizar essa função!',
      'Atenção', MB_OK + MB_ICONWARNING);
    Abort;
  end;

  if Application.MessageBox('Confirma juntar essas imagens?', 'Atenção', MB_YESNO + MB_ICONQUESTION) = ID_YES then
    btnBrandClick(nil);
end;
...
begin
  if Xpress1.Pages = 0 then
  begin
    Application.MessageBox('Esta rotina não pode ser executada, pois não existe nenhuma imagem selecionada!', 'Atenção', MB_OK + MB_ICONWARNING);
    Exit;
  end;

  if Application.MessageBox(PWideChar('ESTÁ ROTINA EXCLUÍRA O ARQUIVO DE IMAGEM POR COMPLETO!'+ #13#10 +
                                      'Deseja continuar com está operação?'), 'Atenção',
                                      MB_YESNO + MB_ICONWARNING + MB_DEFBUTTON2) = IDNO then
    Exit;

  if Application.MessageBox('Confirma exclusão deste arquivo de imagem?', 'Atenção', MB_YESNO + MB_ICONQUESTION + MB_DEFBUTTON2) = ID_NO then
    Exit;
  ...
  begin
      ...
      begin
        Application.MessageBox('Não fui possivel excluir este arquivo!', 'Erro',MB_OK + MB_ICONERROR);
        Exit;
      end;
    ...
    Application.MessageBox('Não fui possivel salvar o histórico deste arquivo!', 'Erro',MB_OK + MB_ICONERROR);
end;

procedure TfmeImagem.btnExcluirPagImagemClick(Sender: TObject);
begin
  ValidaExisteImagem;
  if Application.MessageBox('Confirma exclusão desta página da imagem?', 'Atenção', MB_YESNO + MB_ICONQUESTION + MB_DEFBUTTON2) = ID_YES then
  begin
    ImagemHistorico;
    ...
    begin
      ...
      if not CopyFile(Pchar(vg_DiretorioAuxiliar + '\IMA.TIF'), Pchar(vg_ArquivoImagem), FALSE) then
        ShowMessage('Erro, não foi possivel copiar a imagem!')
      else
        GravarHistorico(True);
      //CarregarNumeroImagens;//Carrega o número de imagens para o combobox
...
begin
  ValidaExisteImagem;
  if Application.MessageBox('Confirma gravação desta imagem?', 'Atenção', MB_YESNO + MB_ICONQUESTION) = ID_YES then
    GravarImagem;
end;
...
begin
  ...
  if not CopyFile(Pchar(viArquivo), Pchar(vg_ArquivoImagem), False) then
    ShowMessage('Erro, não foi possivel salvar a imagem!');
...
begin
  ...
  begin
    Application.MessageBox('Não existe histórico para esta imagem!', 'Atenção', MB_OK + MB_ICONWARNING);
    Exit;
  end;
  ...
  frmMostrarImagem.Caption := 'Histórico das Imagens';
...
begin
  if Xpress1.Pages <> 0 then
  begin
    if Application.MessageBox('Deseja gerar imagem nova no mesmo tamanho da imagem atual?', 'Atenção', MB_ICONQUESTION + MB_YESNO) = IDYES then
    begin
      w := Xpress1.IWidth;
      h := Xpress1.IHeight;
    end
...
begin
  {Habilita a inserção de carimbo na imagem}
  if not vg_hbltImagem then
  begin
    spbPreviewWidthClick(nil);//Volta a imagem para o tamanho da tela de exibição
    vg_hbltImagem  := True;
    vg_hbltCarimbo := True;
    if (curLayer = 0) then
    begin
...
var
  e, X, Y : Integer;
begin
  {Insere a imagem de carimbo na posição indicada pelo cursor do mouse}
  if vg_hbltImagem then
  begin
    if (lcbCarimbo.EditValue = null) and pnlCarimbo.Visible then
    begin
      Application.MessageBox('Nenhum carimbo foi selecionado!', 'Atenção', MB_OK +  MB_ICONWARNING);
      Exit;
    end;
  ...
  begin
    ...
    begin
      Application.MessageBox(PChar('Não há texto para este lembrete!' + #13#10 + 'Preencha o campo lembrete antes de tentar inserer.'), 'Atenção', MB_OK + MB_ICONWARNING);
...
begin
  ...
  if vgControleNumeroPagina then
    if vg_NumeroPaginas <= Xpress1.Pages then
    begin
      Application.MessageBox('Número de páginas digitalizadas igual ou maior que número de paginas informadas!', 'Atenção', MB_OK + MB_ICONWARNING);
      Exit;
    end;
...
begin
  if Xpress1.Pages = 0 then
  begin
    Application.MessageBox('Esta rotina não pode ser executada, pois não existe nenhuma imagem selecionada para redigitalizar!', 'Atenção', MB_OK + MB_ICONWARNING);
    Exit;
  end;

  if Application.MessageBox(PWideChar('Está rotina excluíra apenas a página atual e substituirá pela nova digitalização.' + #13#10 +
                                      'Deseja continuar com está operação?'), 'Atenção',
...
begin
  ...
  begin
    ...
    if viNumero <> '' then
      ...
      begin
        Application.MessageBox('Atenção nome do arquivo fora do padrão, entrar em contato com o suporte!', 'Atenção', MB_OK + MB_ICONERROR);
        vg_NomeArquivo := '';
      end;
  end;
...
begin
  if Xpress1.Pages = 0 then
  begin
    Application.MessageBox('Não existe imagem!', 'Atenção', MB_OK + MB_ICONWARNING);
    Abort;
  end;
end;
...
begin
  ...
  begin
    Application.MessageBox('Grupo deve ser informado!', 'Atenção', MB_OK + MB_ICONWARNING);
    icxGrupo.SetFocus;
    Abort;
  end;
  ...
  begin
    Application.MessageBox('Tipo deve ser informado!', 'Atenção', MB_OK + MB_ICONWARNING);
    lcxTipo.SetFocus;
    Abort;
  end;
  ...
  begin
    Application.MessageBox('Número deve ser informado!', 'Atenção', MB_OK + MB_ICONWARNING);
    edtNumero.SetFocus;
    Exit;
  end;
end;
...
var i,
  ...
  {Adiciona o cabeçalho a imagem a ser impressa}
  procedure ImpCabecalho;
  begin
    //Montar Cabeçalho para impressão
  ...
  {Adiciona o rodapé a imagem a ser impressa}
  procedure ImpRodape;
  begin
    //Montar Cabeçalho para impressão
  ...
  begin
    Result := False;
    case vfTipo of
      1: Result := True;  //todas as páginas
      2: Result := (i mod 2 = 0); //apenas nas páginas pares
      3: Result := (i mod 2 <> 0);//apenas nas páginas Impares
      4: Result := i = 1; //apenas nas primeira páginas
      5: Result := (i = NPagina);//apenas nas primeira páginas
    end;
  end;
  ...
  begin
    ...
    //Cria uma nova página para impressão
...
begin
  ...
  ICabecalho := vg_ImprimirCabecalho; //Controla impressão de Cabeçalhos
  IRodape    := vg_ImprimirRodape; //Controla impressão de Cabeçalhos
  vg_WPagina := Round(PrintPRO1.ScaleWidth); //Largura da página
  vg_HPagina := Round(PrintPRO1.ScaleHeight); //Altura da página
  NPagina    := Xpress1.Pages; //Número de Paginas

  if vg_SolicitarPrint then
    PrintPRO1.PrintDialog; //Escolher a Impressoara

  {Inicio da montagem do arquivo para impressão}
  try
    If PrintPRO1.PrintError <> 0 Then
      Exit;
  except
    Application.MessageBox('Erro de impressora, fovor verificar as configurações da mesma!', 'Atenção', MB_ICONERROR + MB_OK);
    Exit;
  end;
...
begin
  ValidaExisteImagem;
  Posicao := 0;
  valor := InputBox('Mover imagem para', 'Posição:', '');
  if Trim(Valor) <> '' then
  begin
     try
  ...
  begin
    Application.MessageBox('Posição a ser movida não é valida!', 'Atenção', MB_OK + MB_ICONWARNING);
    Abort;
  end;
  ...
  //Inseri a imagem na posição indicada
  ...
  PegaImagem;
  vg_Imagem_Alterada := True;
  Application.MessageBox('Posição modifica com sucesso!', 'Atenção', MB_OK + MB_ICONWARNING);
end;
...
begin
  ...
  with CreateMessageDialog('Páginas a serem geradas em PDF?', mtConfirmation, [mbYes, MbNo, mbCancel]) do
  begin
    vgPrintPDF := True;
    Caption := 'Impressão';
...
begin
  ...
  viWPagina := Round(PrintPRO1.ScaleWidth); //Largura da página
  viHPagina := Round(PrintPRO1.ScaleHeight); //Altura da página
...
begin
  try
    ...
    if not DirectoryExists(vg_CaminhoDiretorio) then
      ...
      Application.MessageBox(PChar('Caminho da imagem não existe : ' + vg_CaminhoDiretorio), 'Atenção', MB_OK + MB_ICONERROR);
      Abort;
    end;
    ...
    if not FileExists(PChar(vg_NomeArquivo))  then
      ShowMessage('Erro, imagem não salva!')
    else
    ...
    on E: Exception do
    begin
      //Application.MessageBox('ERRO! Imagem não digitalizada! Verifique!',  'Atenção', MB_OK  + MB_ICONEXCLAMATION);

      TMensagemUtils.ExibirMensagemErro(
        string.Format(
          'Não foi possível gravar a imagem devido ao ' +
          'motivo abaixo:%s%s',
          [DupeString(
            sLineBreak,
            2),
...
begin
  try
    ...
    begin
      Application.MessageBox(PChar('ERRO! Você está sem permissão de gravação no SERVIDOR DE IMAGENS.'+#13+'Entre em contato com o SUPORTE TÉCNICO!'), 'Atenção', MB_OK + MB_ICONERROR);
    end
    else
    begin
      if vIndexar = 'S' then
