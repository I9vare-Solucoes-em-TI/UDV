...
{$ENDREGION}
  ...
  function MontarClausulaSeloLivroId(const vpSeloGrupoList: IList<ISeloGrupo>): string;
  var
    i: Integer;
    viSQLSeloGrupoIds: TStringList;
  begin
    Result := '';

    viSQLSeloGrupoIds := TStringList.Create;
    try
      viSQLClausula := '';

      for i := 1 to vpSeloGrupoList.Count do
      begin
        if viSQLClausula = '' then
          viSQLClausula := IntToStr(vpSeloGrupoList.Items[i - 1].SeloGrupoID)
        else
          viSQLClausula := viSQLClausula + ', ' + IntToStr(vpSeloGrupoList.Items[i - 1].SeloGrupoID);

        if (i mod 200 = 0) or (i = vpSeloGrupoList.Count) then
        begin
          viSQLSeloGrupoIds.Add(viSQLClausula);
          viSQLClausula := '';
        end;
      end;

      for i := 0 to viSQLSeloGrupoIds.Count - 1 do
      begin
        if i = 0 then
          viSQLSeloGrupoIds[i] := ' AND ( G_SLO.SELO_GRUPO_ID IN ( ' + viSQLSeloGrupoIds[i] + ' ) '
        else
          viSQLSeloGrupoIds[i] := ' OR G_SLO.SELO_GRUPO_ID IN ( ' + viSQLSeloGrupoIds[i] + ' ) ';

        if i = viSQLSeloGrupoIds.Count - 1 then
          viSQLSeloGrupoIds[i] := viSQLSeloGrupoIds[i] + ' ) ';
      end;

      Result := viSQLSeloGrupoIds.Text;

    finally
      FreeAndNil(viSQLSeloGrupoIds);
    end;
  end;
...
begin
  ...
  viSQLClausula := MontarClausulaSeloLivroId(vpSeloGrupoList);
  ...
  viSQL := {$REGION 'Comando SQL SELECT'}
    ...
    {$REGION 'Cláusula WHERE e Parâmetros'}
    'WHERE ' +
    'G_SL.SELO_SITUACAO_ID = :P_SELO_SITUACAO_ID ' + viSQLClausula +
    ' AND G_SL.DATA <= :P_DATA AND ' +
      ...
      try
        ...
        while viDataSet.Eof.&Not do
          try
            viSeloLivro := Get(viDataSet);
            viSeloLivro.SeloSituacao := vpSeloSituacao;
            viSeloLivro.SeloLote.SeloGrupo := vpSeloGrupoList.First(
              function(const vpValue: ISeloGrupo): Boolean
              begin
                Result := vpValue.SeloGrupoID = viDataSet.FieldByName(
                  'G_SLO_SELO_GRUPO_ID').AsInteger;
              end);
            Result.Add(viSeloLivro);
          finally
            viDataSet.Next;
...
function TSeloLivroDAO.GetList(
  ...
  const vpMenorIgualData: TDateTime;
  const vpCampoOrdenacaoList: TCampoOrdenacaoList;
  const vpValidado: Boolean): TSeloLivroList;
{$REGION 'Constantes'}
const
  CI_LOAD_SELO_SITUACAO: Boolean = False;
  CI_LOAD_SELO_LOTE: Boolean = True;
      ...
      try
        ...
        while viDataSet.Eof.&Not do
          try
            viSeloLivro := Get(viDataSet);
            viSeloLivro.SeloSituacao := vpSeloSituacao;
            viSeloLivro.SeloLote.SeloGrupo := nil;
