...
var
  viValor : String;
begin
  if Application.MessageBox('Confirmar Período Ativo como Padrão para todos os Sistemas?','Pergunta', mb_IconQuestion + mb_YesNo) = idNo then
    exit
  else
    if Application.MessageBox('TEM CERTEZA QUE DESEJA FAZER ESTA ALTERAÇÃO?','Pergunta', mb_IconQuestion + mb_YesNo) = idNo then
      exit;
  ...
  Application.MessageBox('Período Padrão alterado com sucesso!!!','TRI7 INFORMA!!!', MB_ICONINFORMATION + MB_OK);
