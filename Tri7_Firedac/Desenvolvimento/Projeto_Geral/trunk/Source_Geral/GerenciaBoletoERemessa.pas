...
type
  TCorrigirRemessa = reference to procedure(
    const vpBoletoID: Integer;
    const vpValidacao: string);
  ...
  TfrmGerenciaBoletoRemessa = class(TForm)
    ...
    cxButton2: TcxButton;
    btnSairRemessa: TcxButton;
    cxButton4: TcxButton;
    cxButton5: TcxButton;
    icbRemessaGerada: TcxImageComboBox;
    cxLabel7: TcxLabel;
    chbLista: TcxCheckBox;
    pgcGerarRemessa: TcxPageControl;
    tbsBoletosRemessa: TcxTabSheet;
    tbsBoletoValidacao: TcxTabSheet;
    grdBoleto: TcxGrid;
    ...
    tblValidarRemessa: TI9MemTable;
    dtsValidarRemessa: TDataSource;
    cxGrid1: TcxGrid;
    cxGridDBTableView4: TcxGridDBTableView;
    cxGridBOLETO_ID: TcxGridDBColumn;
    cxGridNOSSO_NUMERO: TcxGridDBColumn;
    cxGridNOME: TcxGridDBColumn;
    cxGridCPFCNPJ: TcxGridDBColumn;
    cxGridCIDADE: TcxGridDBColumn;
    cxGridUF: TcxGridDBColumn;
    cxGridLevel8: TcxGridLevel;
    cxGroupBox7: TcxGroupBox;
    mmoValidacao: TcxDBMemo;
    tblValidarRemessaBOLETO_ID: TBCDField;
    tblValidarRemessaNOSSO_NUMERO: TStringField;
    tblValidarRemessaNOME: TStringField;
    tblValidarRemessaCPFCNPJ: TStringField;
    tblValidarRemessaLOGRADOURO: TStringField;
    tblValidarRemessaBAIRRO: TStringField;
    tblValidarRemessaCIDADE: TStringField;
    tblValidarRemessaUF: TStringField;
    tblValidarRemessaCEP: TStringField;
    tblValidarRemessaTEXTO: TMemoField;
    cxGridDBTableView4LOGRADOURO: TcxGridDBColumn;
    cxGridDBTableView4CEP: TcxGridDBColumn;
    ppmCorrigirRemessa: TPopupMenu;
    CorrigirRemessa1: TMenuItem;
    cxLabel8: TcxLabel;
    ...
    procedure btnValidarRemessaClick(Sender: TObject);
    procedure CorrigirRemessa1Click(Sender: TObject);
    procedure ppmCorrigirRemessaPopup(Sender: TObject);

  private
    FCorrigirRemessa : TCorrigirRemessa;
    ...
    function ValidarRemessa: Boolean;

    const C_ARQUIVO_EXPORTAR = 'C:\Temp\RelTemp.csv';

  public
    procedure RegisterCorrigirRemessa(
      const vpCorrigirRemessa: TCorrigirRemessa);
  end;
...
procedure TfrmGerenciaBoletoRemessa.CorrigirRemessa1Click(Sender: TObject);
begin
  DoCorrigirRemessa;
end;
...
var
  viDataSet: TDataSet;
begin
  pgcGerarRemessa.ActivePage := tbsBoletosRemessa;
  ...
  btnValidarRemessa.Enabled := not tblRemessa.IsEmpty;
end;
...
procedure TfrmGerenciaBoletoRemessa.btnValidarRemessaClick(Sender: TObject);
begin
  btnGerarRemessa.Enabled := ValidarRemessa;
  tbsBoletoValidacao.TabVisible := not btnGerarRemessa.Enabled;

  if tbsBoletoValidacao.TabVisible then
    pgcGerarRemessa.ActivePage := tbsBoletoValidacao;

  btnValidarRemessa.Enabled := False;
end;
...
procedure TfrmGerenciaBoletoRemessa.ppmCorrigirRemessaPopup(Sender: TObject);
begin
  if not Assigned(FCorrigirRemessa) then
    Abort;
end;

procedure TfrmGerenciaBoletoRemessa.RegisterCorrigirRemessa(
  const vpCorrigirRemessa: TCorrigirRemessa);
begin
  FCorrigirRemessa := vpCorrigirRemessa;
end;
...
begin
  ...
  tbsBoletoValidacao.TabVisible := False;
end;
...
procedure TfrmGerenciaBoletoRemessa.DoCorrigirRemessa;
begin
  if Assigned(
    FCorrigirRemessa) then
     FCorrigirRemessa(
      tblValidarRemessaBOLETO_ID.AsInteger,
      tblValidarRemessaTEXTO.AsString);
end;
...
begin
  tblRemessa.Close;
  tblRemessa.Open;
  tblValidarRemessa.Close;
  tblValidarRemessa.Open;
end;
...
function TfrmGerenciaBoletoRemessa.ValidarRemessa: Boolean;
  {$REGION 'ValidarInformacoesBoleto'}
  function ValidarInformacoesBoleto: string;
  begin
    Result := EmptyStr;

    if Trim(tblRemessaCPFCNPJ.AsString) = '' then
      Result := 'Por favor, Informar CPF/CNPJ para continuar!';

    if Trim(tblRemessaLOGRADOURO.AsString) = '' then
      Result := Result + 'Por favor, Informar o logradouro para continuar!' + #13#10;

    if Trim(tblRemessaCIDADE.AsString) = ''  then
      Result := Result +  'Por favor, Informar a CIDADE para continuar!' + #13#10;

    if Trim(tblRemessaUF.AsString) = '' then
      Result := Result + 'Por favor, Informar o UF para continuar!' + #13#10;

    if Trim(tblRemessaCEP.AsString) = '' then
      Result := Result + 'Por favor, Informar o CEP para continuar!' + #13#10;
  end;
  {$ENDREGION}
...
begin
  ...
  try
    ...
    while not tblRemessa.Eof do
    begin
      viTexto := Trim(ValidarInformacoesBoleto);
      if not viTexto.IsEmpty then
      begin
        tblValidarRemessa.Append;
        tblValidarRemessaBOLETO_ID.AsInteger := tblRemessaBOLETO_ID.AsInteger;
        tblValidarRemessaNOSSO_NUMERO.AsString := tblRemessaBOLETO_ID.AsString;
        tblValidarRemessaNOME.AsString := tblRemessaNOME.AsString;
        tblValidarRemessaCPFCNPJ.AsString := tblRemessaCPFCNPJ.AsString;
        tblValidarRemessaLOGRADOURO.AsString := tblRemessaLOGRADOURO.AsString;
        tblValidarRemessaBAIRRO.AsString := tblRemessaBAIRRO.AsString;
        tblValidarRemessaCIDADE.AsString := tblRemessaCIDADE.AsString;
        tblValidarRemessaUF.AsString := tblRemessaUF.AsString;
        tblValidarRemessaCEP.AsString := tblRemessaCEP.AsString;
        tblValidarRemessaTEXTO.AsString := viTexto;
        tblValidarRemessa.Post;
  ...
  Result := tblValidarRemessa.IsEmpty;
end;
