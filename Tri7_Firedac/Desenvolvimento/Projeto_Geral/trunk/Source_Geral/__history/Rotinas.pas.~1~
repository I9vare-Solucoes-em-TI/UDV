...
Uses windows, SysUtils, Types, Classes, Graphics, Controls, Forms, Dialogs,
  ...
  cxCurrencyEdit, cxCalendar, Consts, Tlhelp32, WPRTEDefsConsts,
  IniFiles, ShellAPI, IdBaseComponent, IdComponent, IdIPWatch,
  System.Generics.Collections, cxProgressBar, QrCode, System.MaskUtils,
  IdHashMessageDigest, idHash, cxGridDBTableView;
...
const
     ...
     procedure ExibirFormulario(ClasseForm   : TComponentClass;
                                var Reference;
                                FormModal    : Boolean = False;
                                vpTop        : Boolean = False;
                                vpMaximixar  : Boolean = False); overload;

     procedure ExibirFormulario(vpForm       : TForm;
                                ClasseForm   : TComponentClass;
                                var Reference;
                                FormModal    : Boolean = False;
                                vpTop        : Boolean = False;
                                vpMaximixar  : Boolean = False); overload;
     ...
     function NumeroExtenso(vpValor: Integer) : String;
    ...
    function SplitStringFixedLength(
      const vpValue: string;
      const vpFixedLength: Integer): TStrings;
    ...
    function LiberarTextoAll(const vpTexto: string): string;
    function TextoProtegido(wptAuxiliar: TWPRichText): Boolean;
    ...
    function BuscarMesExtenso(vpMesNumero : Integer):String;
    ...
    function TrocaCaracterEspecial(vpTexto : string; vpLimExt : boolean) : string;
    function RetorneSoapDateTime(vpDataHora: String): TDateTime;
    ...
    function PreencherString(vpTexto: string; vpCaracter: String; vpTamanho: integer; vpDirecao: string = 'E'): String;

    function MensagemPersonalizada(Mensagem, Titulo, Butao_Sim, Butao_Nao: String): TModalResult;
    function GetHashMD5(const fileName : string) : string;
    ...
    function formatarDtmSQL(valorDoCampo: TDateTime; strHoraInicialFinal: String = ''): String;
    function formatarDtHrSQL(valorDoCampo: TDateTime): String;
    function formatarHrSQL(valorDoCampo: TDateTime): String;
    function formatarDatSQL(valorDoCampo : TDateTime): String;
    function formatarCurSQL(valorDoCampo : Currency): String;
    function formatarIntSQL(valorDoCampo: Integer; bolAceitaNulo: Boolean = False): String;
    function formatarStrSQL(valorDoCampo : String; intTamanhoDoCampo: Integer = -1): String;
    function formatarBloSQL(strNomeDoCampo: String; valorDoCampo: TStream): String;

    Procedure AtualizaArquivosSistema(Origem : String);

    procedure ResizeBitmap(Bitmap: TBitmap; const NewWidth, NewHeight: integer);

    procedure LockFormUpdate(vpForm: TWinControl);
    procedure UnlockFormUpdate(vpForm: TWinControl);

    function GetFileSize(const aFilename: String): Int64;

    procedure CentralizarControle(
      const vpControle: TControl;
      const vpContainer: TControl);

    procedure GridCopyClipboard(const vpGrig: TcxGridDBTableView);

    function FormatarTelefone(Telefone : String):String;
    function ArquivoEmUso(const NomeDoArquivo: string): boolean;

    function ValidarAtualizacaoArquivo(const vpArquivoOrigem, vpArquivoDestino: string): Boolean;
    ...
    function CondicaoList(
      const vpNumeroList: TList<string>;
      const vpCampo: string): string;overload;

    function GetListIDs(const vpDataSet: TDataSet;
      const vpCampoID: string): TList<Integer>;

    function CondicaoListIN(
      const vpIDsList: TList<Integer>;
...
begin
  ...
  z      := 0;
  Result := False;
  for i := 1 to Length(ip) do if CharInSet(ip[i], ziff) then
  else
  begin
    if ip[i] = '.' then
    begin
...
begin
  ...
  if vpValue.IsEmpty then
    Exit;

  Result := TStringList.Create;
  viLength := vpValue.Length;
  viCapacity := (viLength div vpFixedLength) + 1;
  Result.Capacity := viCapacity;
  viIndex := 0;

  while viIndex <= (viLength - 1) do
  begin
    viValue := vpValue.Substring(
      viIndex,
      vpFixedLength);
...
begin
  ...
  if valorDoCampo = 0 then
    strValor := 'NULL'
  else
    strValor := QuotedStr(FormatDateTime('yyyy/MM/dd ' + Trim(strHoraInicialFinal), valorDoCampo));
...
begin
  ...
  if valorDoCampo = 0 then
    ...
    strValor := QuotedStr(FormatDateTime('yyyy/MM/dd hh:mm:ss', valorDoCampo));
...
begin
  ...
  if valorDoCampo = 0 then
    ...
    strValor := QuotedStr(FormatDateTime('hh:mm:ss', valorDoCampo));
...
begin
  if valorDoCampo = 0 then
    ...
    strValor := QuotedStr(FormatDateTime('yyyy/MM/dd', valorDoCampo));
...
var
  strValor : String;
begin
  strValor := StringReplace(FloatToStr(valorDoCampo), ',', '.', [rfReplaceAll, rfIgnoreCase]);
...
var
  strValor : String;
begin
  if bolAceitaNulo then
  begin
    if valorDoCampo > 0 then
      strValor := IntToStr(valorDoCampo)
    else
      strValor := 'NULL';
  end
  else
...
var
  strValor : String;
begin
  if intTamanhoDoCampo >= 0 then
    strValor := Copy(strValor, 1, intTamanhoDoCampo);

  strValor := QuotedStr(valorDoCampo);

  //strValor := AnsiUpperCase(strValor);  Estava ocasionando erro nos arquivos xml (Avisar o Bruno)
...
var
  strValor : String;
begin
  strValor := StringReplace(strNomeDoCampo, ':', '', [rfReplaceAll]);

  if valorDoCampo <> nil then
    strValor := ':' + strValor
  else
    strValor := 'NULL';
...
begin
  ...
  Case vpTipo of
    ...
    7   : begin
            if vlQtdAdicional <> 200 then
              vlQtdAdicional := 1;
            Result := (10*vlQtdAdicional)+vpData;
            if (StrToInt(copy(DateToStr(Result),1,2)) > 21) then
            begin
              viMes := copy(DateToStr(Result),4,2);
              viAno := StrToInt(copy(DateToStr(Result),7,4));
              if viMes = '12' then
              begin
                viMes := '01';
                inc(viAno);
              end
              else viMes := IntToStr(StrToInt(viMes)+1);
              Result := StrToDate('01/'+viMes+'/'+IntToStr(viAno));
            end
          end;
  end;
end;
...
begin
  ...
  repeat
    ...
    begin
      ...
      vwptTexto.SelectionAsString := vwptTextoSubstituir.AsString;
    end
    else
      vSair := True;
  until
...
begin
  ...
  try
    ...
    wptMemoria.SelectAll;
    wptAuxiliar.SelectionAsString := wptMemoria.AsString;
...
begin
  ...
  try
    viwptTexto.Hide;
    viwptTexto.AsString := vpTexto;
    RetirarProtecaoDoTexto(viwptTexto, True);
    Result := viwptTexto.AsString;
  finally
...
begin
  ...
  if not wptAuxiliar.IsSelected then
    if wptAuxiliar.CPAttr.HasStyle(afsProtected) then
       Exit(True);

  i := wptAuxiliar.SelLength;
  wptAuxiliar.CPPosition := wptAuxiliar.SelStart;
  ...
  begin
    ...
    if not wptAuxiliar.CPMoveNext then
      Break;
...
begin
  ...
  begin
    ...
    //Define o tamanho do QrCode com base no valor entre "|"
    //Deve ser especificado sempre no Inicio
    //Exemplo: |100|texto qrcode
    if not Trim(viValor).IsEmpty then
      if viValor[1] = '|' then
      begin
        Delete(viValor, 1, 1);
        if Pos('|', viValor) > 0 then
        begin
          viArrayValor := viValor.Split(['|']);
          try
            viTamanho := StrToIntDef(viArrayValor[0], 0);
            viValor   := viArrayValor[1];
          finally
            Finalize(viArrayValor);
          end;
        end;
      end;
    ...
    Try
      //Altera o tamanho do qrCode
      if viTamanho > 0 then
        ResizeBitmap(viImg, viTamanho, viTamanho);
  ...
  try
    ...
    begin
      if not DirectoryExists(viPasta+'PDF') then
        MkDir(viPasta+'PDF');
      viPdf.Filename := IncludeTrailingPathDelimiter(viPasta+'PDF')+ vpArquivoDestino+'.pdf';
    end
    else viPdf.Filename := IncludeTrailingPathDelimiter(viPastaDestino) + vpArquivoDestino+'.pdf';
...
procedure LockFormUpdate(vpForm: TWinControl);
begin
  vpForm.Enabled := False;
  Application.ProcessMessages;

  if vlLockFormUpdatePile = 0 then
    vpform.Perform(WM_SETREDRAW, 0, 0);

  inc(vlLockFormUpdatePile);
end;
...
begin
  ...
  dec(vlLockFormUpdatePile);

  if vlLockFormUpdatePile = 0 then
  begin
    vpForm.Perform(WM_SetRedraw, 1, 0);
    RedrawWindow(vpForm.Handle, nil, 0, RDW_FRAME + RDW_INVALIDATE + RDW_ALLCHILDREN + RDW_NOINTERNALPAINT);
  end;
end;
...
procedure ExibirFormulario(vpForm       : TForm;
                           ...
                           vpMaximixar  : Boolean = False);
begin
  if ClasseForm.InheritsFrom(TForm) then
    vgFormClass := TFormClass(ClasseForm);

  with Application do
  begin
    if not FormModal then
    begin
      LockFormUpdate(vpForm);
      try
        if TComponent(Reference) = nil then
        begin
          CreateForm(ClasseForm, Reference);
        ...
        UnlockFormUpdate(vpForm);
      end;
      Application.ProcessMessages;
      TForm(Reference).OnActivate(nil);
    end
    else
    begin
      if TComponent(Reference) = nil then
...
begin
  Result := '';
  for i := 1 to Length(Valor) do
    if CharInSet(Valor[i], ['0'..'9', 'a'..'z', 'A'..'Z', ' ']) then
       Result := Result + Valor[i];
end;
...
begin
  Result    := '';
  ComoTexto := FloatToStrF(Abs(vpValor), ffFixed, 18, 2);
...
Procedure AtualizaArquivosSistema(Origem : String);
const
  arquivos : array[1..23] of Pchar =('fbclient.dll',
                                     'libmysql.dll',
                                     'borlndmm.dll',
                                     'libeay32.dll',
                                     'MSVCR100.DLL',
                                     ...
                                     'ievision.dll',
                                     'VRS.ini',
                                     'Login_Administrativo.jpg',
                                     'Login_Caixa.jpg',
                                     'Login_FinanceiroTri7.jpg',
                                     'Login_Protesto.jpg',
                                     'Login_RegistroCivil.jpg',
                                     'Login_RegistroImoveis.jpg',
                                     'Login_RTD.jpg',
                                     'Login_TabelionatoNotas.jpg',
                                     'tidy.exe',
                                     'LOGO_ASS.JPEG');
var
  viFile: string;
  i : Integer;
begin
  if not Origem.IsEmpty then
    for i := 1 to Length(arquivos) do
    begin
      viFile := IncludeTrailingPathDelimiter(Origem) + arquivos[i];
      if FileExists(viFile) then
        if TamanhoArquivo(viFile) > 512 then
          if ValidarAtualizacaoArquivo(viFile, arquivos[i]) then
            if not ArquivoEmUso(viFile) then
              CopyFile(PChar(viFile), PChar(arquivos[i]), false);
    end;
end;
...
var
  viData: TXSDateTime;
begin
  viData := TXSDateTime.Create;
  try
    viData.XSToNative(vpDataHora);
    Result := viData.AsDateTime;
  finally
    FreeAndNil(viData);
  end;
end;
...
begin
  ...
  for i := 1 to 38 do
    viTexto := StringReplace(viTexto, xCarEsp[i], xCarTro[i], [rfreplaceall]);

  // De acordo com o parâmetro aLimExt, elimina caracteres extras.
  if vpLimExt then
    for i := 1 to 50 do
      viTexto := StringReplace(viTexto, xCarExt[i], '', [rfreplaceall]);
...
begin
  ...
  f:= createmessagedialog(Mensagem, mtconfirmation,[mbyes,mbno, mbCancel]);
  try
...
procedure CentralizarControle(
  ...
  const vpContainer: TControl);
begin
  vpControle.Left := (vpContainer.ClientWidth - vpControle.ClientWidth) div 2;
  vpControle.Top := (vpContainer.ClientHeight - vpControle.ClientHeight) div 2;
end;

procedure GridCopyClipboard(const vpGrig: TcxGridDBTableView);
begin
  with vpGrig do
    if OptionsSelection.CellSelect then
      Clipboard.AsText := Controller.FocusedRow.DisplayTexts[Controller.FocusedColumn.Index];
end;
...
begin
   if FileExists(NomeDoArquivo) then
     try
       StreamArquivo := TFileStream.Create(NomeDoArquivo,
...
var
  vVersaoOrigem,
  vVersaoDestino : TDateTime;
begin
  Result := True;

  if not FileExists(vpArquivoDestino) then
    Exit;

  vVersaoOrigem  := FileDateToDateTime(FileAge(vpArquivoOrigem));
  vVersaoDestino := FileDateToDateTime(FileAge(vpArquivoDestino));

  Result := vVersaoOrigem > vVersaoDestino; //Existe uma nova versão
end;
...
begin
  ...
  for viBoletoId in vpIDsList do
  begin
...
begin
  viCondicaoList := TListFactory<string>.New.GetInstance;
  for viNumero in vpNumeroList do
  begin
    viCondicao := string.Format(
      '%s = %s',
      [vpCampo, QuotedStr(Trim(viNumero))]);
...
begin
  ...
  try
    ...
    begin
      Result.Add(vpDataSet.FieldByName(vpCampoID).AsInteger);
      vpDataSet.Next;
    end;
  finally
...
{$ENDREGION}
begin
  if vpIDsList.Count > 1500 then
  begin
    Application.MessageBox('Máximo de 1500 IDs por pesquisa!', 'Atenção', MB_OK + MB_ICONERROR);
    Abort;
  end;
  ...
  Result := ' ' + vpCampo +  ' IN (' + string.join(',', viCondicaoList.ToArray) + ') ';
end;
