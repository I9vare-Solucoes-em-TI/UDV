...
begin
  ...
  if DirectoryExists(vlPasta + '\'+ cbxNaturezaGeral.Text) then
  begin
...
begin
  CarregarINI;
  LockWindowUpdate(Handle);
  if DirectoryExists(vlPasta + '\'+ cbxNaturezaPesquisa.Text) then
  begin
...
procedure TfrmPesquisaImagem.rdbDigitalizacaoColoridaClick(Sender: TObject);
begin
  fmeImagem1.ImagemColorida(True);
end;

procedure TfrmPesquisaImagem.rdbDigitalizacaoPretoBrancoClick(Sender: TObject);
begin
