...
var
  ...
  var
    ...
    i : Integer;
  begin
    if CharInSet(vlNivelSegurancaSenha[1], ['2', '3']) then
    begin
      viNivel1 := False; viNivel2 := False; viNivel3 := False;
      for i := 1 to Length(edtNovaSenha.Text) do
      begin
        if CharInSet(edtNovaSenha.Text[i], ['A'..'Z']) then
          viNivel1 := True
        else
          if CharInSet(edtNovaSenha.Text[i], ['0'..'9']) then
            viNivel2 := True;

        if (vlNivelSegurancaSenha = '3') and (CharInSet(edtNovaSenha.Text[i], ['a'..'z'])) then
          viNivel3 := True;
      end;
