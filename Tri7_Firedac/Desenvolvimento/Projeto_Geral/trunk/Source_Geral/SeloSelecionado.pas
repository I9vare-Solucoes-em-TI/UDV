...
type
  ...
  private
    ...
    procedure VerificarSolicitacaoSelo(
      const vpSeloGrupoId: Integer;
...
uses
  ...
  Geral.Controller.SeloLivro,
  Geral.View.Show.Message,
...
begin
  ...
  begin
    viDataBase := dtmControles.DataHoraBanco(
      3,
      False);

    VerificarSolicitacaoSelo(
      vgDadosSelo.SeloGrupoId,
      viDataBase,
      vgDadosSelo.Qtd);
...
procedure TfrmSeloSelecionado.VerificarSolicitacaoSelo(
  const vpSeloGrupoId: Integer;
  const vpDataBase: TDateTime;
  const vpQuantidadeSolicitada: Integer);
var
  viListaErroValidacao: TObjectList<TErroValidacao>;
  viSolicitacaoSeloValida: Boolean;
  viListaMensagem: TStrings;
  viErroValidacao: TErroValidacao;
  viPossuiErroNaoPermiteAutorizacao: Boolean;
  viTexto: string;
  viQuebraLinhaDupla: string;
  viContinuar: Boolean;
  viPermissao: string;
  viOperacao: Integer;
begin
  ...
  try
    viSolicitacaoSeloValida := TSeloLivroController.ValidarSolicitacaoSelo(
      vpSeloGrupoId,
      vpDataBase,
      vpQuantidadeSolicitada,
      viListaErroValidacao);

    if viSolicitacaoSeloValida then
      Exit;

    if not Assigned(viListaErroValidacao) then
      Abort;

    if viListaErroValidacao.Count <= 0 then
      Abort;

    viListaMensagem := TStringList.Create;
    viPossuiErroNaoPermiteAutorizacao := False;

    for viErroValidacao in viListaErroValidacao do
    begin
      viListaMensagem.Add(viErroValidacao.Mensagem);

      if not viErroValidacao.PermiteAutorizacao then
        viPossuiErroNaoPermiteAutorizacao := True;
    end;

    viTexto := viListaMensagem.Text;

    if viPossuiErroNaoPermiteAutorizacao then
    begin
      TfrmShowMessage.ShowMessageDlg(
        viTexto,
        TTipoMessageDlg.tmAlert);

      Abort;
    end;

    viQuebraLinhaDupla := DupeString(
      sLineBreak,
      2);

    viTexto := string.Format(
      '%s%sDeseja continuar mesmo assim?',
      [viTexto,
      viQuebraLinhaDupla]);

    viContinuar := TfrmShowMessage.ShowMessageDlg(
      viTexto,
      TTipoMessageDlg.tmQuestion);
    ...
    for viErroValidacao in viListaErroValidacao do
    begin
      viOperacao := TPermissaoUtil.BuscarValorTipoOperacao(viErroValidacao.TipoOperacao);

      viPermissao := LiberarPermissaoUsuario(
        viErroValidacao.ChaveRotina,
        viOperacao,
        viErroValidacao.SistemaId);

      if viPermissao[viOperacao] <> '1' then
        Abort;
    end;
  finally
    FreeAndNil(viListaErroValidacao);
    FreeAndNil(viListaMensagem);
  end;
end;
