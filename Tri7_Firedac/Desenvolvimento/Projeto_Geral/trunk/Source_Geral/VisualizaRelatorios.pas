...
begin
  if ID.IsEmpty then
  begin
    Application.MessageBox(PWideChar('Nenhum modelo de relatório foi configurado para está opção.' + #13#10 +
                                     'Favor entrar em contato com o suporte.'), 'Atenção', MB_OK + MB_ICONINFORMATION);
    Abort;
  end;
  ...
  begin
    ...
    try
      ...
      sqlAuxiliar.Active := False;
      frxReport.OnDblClickObject := nil;
      frxReport.Clear;
...
procedure TfrmVisualizaRelatorios.frxPreviewPageChanged(Sender: TfrxPreview;
  PageNo: Integer);
begin
  StatusBar.Panels[0].Text := 'Páginas : ' + IntToStr(frxPreview.PageNo) +
  ...
  try
    ...
    begin
      ...
      begin
        ...
        begin
          if Application.MessageBox('Deseja gravar essa alteração?', 'Atenção',
          ...
          begin
            ...
            begin
              if Application.MessageBox
                ('Deseja gravar o histórico dessa alteração?', 'Atenção',
                MB_YESNO + MB_ICONQUESTION) = IDYES then
              begin
...
procedure CriarFuncoesRelatorio(frRelatorio: TfrxReport);
begin
  frRelatorio.AddFunction('function BuscarConfig(vpGrupo, vpSecao, vpNome : string; vpTipoValor: char): Variant', 'TRI7', 'Retorna o valor dos parâmetros de configuração do sistema');
  frRelatorio.AddFunction('function ExecSQL(Qry: string):Boolean', 'TRI7', 'Executa o SQL passado como parâmetro');
  ...
  frRelatorio.AddFunction('function NumeroExtenso(vpValor: Integer) : String', 'TRI7', 'Retorna o número por extenso');
  frRelatorio.AddFunction('function ValorExtenso(Valor: Double; TipoExtenso : Integer = 3): String', 'TRI7', 'Retorna o valor por extenso');
  frRelatorio.AddFunction('procedure RetornaValores(Valor: String)', 'TRI7', 'Retorna valores internos para a aplicação');
  ...
  frRelatorio.AddFunction('function BuscarMesExtenso(Valor: Integer): String', 'TRI7', 'Retorna o mês por extenso');
  frRelatorio.AddFunction('function RetornaCPFCNPJFormatado(Valor : String) : String', 'TRI7', 'Retorna CPF/CNPJ formatados');
  frRelatorio.AddFunction('function FormatarTelefone(Telefone : String):String', 'TRI7', 'Formata número de telefone');
end;
...
begin
  ...
  else if AnsiSameText(NameFuncao, 'FormatarTelefone') then
    Result := FormatarTelefone(Parametros[0])
end;

procedure TfrmVisualizaRelatorios.btnFecharClick(Sender: TObject);
begin
  if vlExportarArquivo then
    vgCancelado := False
  else
    vgCancelado := True;
