...
begin
  ...
  try
    ...
    try
      ...
      viSeloGrupoFilho.Connection := dtmControles.DB;
      ...
      while not viSeloGrupo.Eof do
        try
          {$REGION 'Preencher valores dos parâmetros'}
          viSeloGrupoFilhoID := dtmControles.GerarSequencia(
            'G_SELO_GRUPO_FILHO');
...
begin
  ...
  qrySeloGrupo.Connection := dtmControles.DB;
  qrySeloGrupoFilho.Connection := dtmControles.DB;
