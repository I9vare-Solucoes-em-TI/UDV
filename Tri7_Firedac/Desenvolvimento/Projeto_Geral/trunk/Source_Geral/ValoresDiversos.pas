...
uses
  ...
  cxTLdxBarBuiltInMenu, DbxDevartInterBase, FireDAC.Stan.Intf,
...
type
  ...
  private
    FTexto: string;
    function InserirValor: string;
  public
    { Public declarations }

    class function ValoresDiversos: string;
  end;
...
class function TfrmValoresDiversos.ValoresDiversos: string;
var
  viFrmValoresDiversos: TfrmValoresDiversos;
begin
  Result := string.Empty;

  Application.CreateForm(
    Self,
    viFrmValoresDiversos);

  try
    if viFrmValoresDiversos.ShowModal = mrOk  then
       Result := viFrmValoresDiversos.FTexto;
  finally
    viFrmValoresDiversos.Free;
  end;
...
begin
  inherited;
  if pgvValores.ActivePageIndex = 0 then
    FTexto := InserirValor;
end;
...
function TfrmValoresDiversos.InserirValor: string;
var
