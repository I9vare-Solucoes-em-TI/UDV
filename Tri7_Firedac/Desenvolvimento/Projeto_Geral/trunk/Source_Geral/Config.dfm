inherited frmConfig: TfrmConfig
  Left = 291
  Top = 120
  Caption = 'Configura'#231#227'o do Sistema'
  ClientHeight = 587
  ClientWidth = 972
  OldCreateOrder = True
  OnCloseQuery = FormCloseQuery
  ExplicitWidth = 988
  ExplicitHeight = 626
  PixelsPerInch = 96
  TextHeight = 13
  inherited PanelBotoes: TPanel
    Width = 972
    ExplicitWidth = 972
    inherited PanelBotoesBasicos: TPanel
      Width = 766
      ExplicitWidth = 890
      inherited PanelBasicoBotoes: TPanel
        inherited cxBtnPesquisar: TcxButton
          Left = 261
          TabOrder = 4
          Visible = False
          ExplicitLeft = 261
        end
        inherited cxBtnCancelar: TcxButton
          Left = 197
          TabOrder = 3
          ExplicitLeft = 197
        end
        inherited cxBtnGravar: TcxButton
          Left = 133
          TabOrder = 2
          ExplicitLeft = 133
        end
        inherited cxBtnExcluir: TcxButton
          Left = 69
          TabOrder = 1
          ExplicitLeft = 69
        end
        inherited cxBtnAlterar: TcxButton
          Left = 5
          TabOrder = 0
          ExplicitLeft = 5
        end
        inherited cxBtnIncluir: TcxButton
          Left = 325
          TabOrder = 5
          Visible = False
          ExplicitLeft = 325
        end
      end
    end
    inherited PanelBotaoFechar: TPanel
      Left = 768
      Width = 202
      ExplicitLeft = 768
      ExplicitWidth = 202
      inherited cxBtnFechar: TcxButton
        Left = 132
        ExplicitLeft = 132
      end
      object btnExportar: TcxButton
        Left = 4
        Top = 4
        Width = 60
        Height = 45
        Caption = 'Exportar'
        Colors.Default = clWhite
        LookAndFeel.Kind = lfUltraFlat
        LookAndFeel.NativeStyle = False
        LookAndFeel.SkinName = ''
        OptionsImage.Glyph.Data = {
          
          401D0404040404040404}
        OptionsImage.Layout = blGlyphTop
        TabOrder = 1
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        OnClick = btnExportarClick
      end
      object btnImportar: TcxButton
        Left = 68
        Top = 4
        Width = 60
        Height = 45
        Caption = 'Atualizar'
        Colors.Default = clWhite
        LookAndFeel.Kind = lfUltraFlat
        LookAndFeel.NativeStyle = False
        LookAndFeel.SkinName = ''
        OptionsImage.Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFF}
        OptionsImage.Layout = blGlyphTop
        TabOrder = 2
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        OnClick = btnImportarClick
      end
    end
  end
  inherited StatusBar: TStatusBar
    Top = 567
    Width = 972
    ExplicitTop = 567
    ExplicitWidth = 972
  end
  inherited PanelGeral: TPanel
    Top = 209
    Width = 972
    Height = 358
    TabOrder = 3
    ExplicitTop = 209
    ExplicitWidth = 972
    ExplicitHeight = 358
    inherited PanelData: TPanel
      Width = 970
      Height = 144
      Color = clWindow
      Enabled = True
      ExplicitWidth = 970
      ExplicitHeight = 144
      object cxGridGrupo: TcxGrid
        Left = 1
        Top = 1
        Width = 359
        Height = 142
        Align = alLeft
        TabOrder = 0
        LookAndFeel.Kind = lfUltraFlat
        LookAndFeel.NativeStyle = False
        RootLevelStyles.TabsBackground = dtmControles.cxStyle1
        object cxGridDBTableView1: TcxGridDBTableView
          Navigator.Buttons.CustomButtons = <>
          Navigator.Buttons.PriorPage.Visible = False
          Navigator.Buttons.NextPage.Visible = False
          Navigator.Buttons.Insert.Visible = False
          Navigator.Buttons.Delete.Visible = False
          Navigator.Buttons.Edit.Visible = False
          Navigator.Buttons.Post.Visible = False
          Navigator.Buttons.Cancel.Visible = False
          Navigator.Buttons.Refresh.Visible = False
          Navigator.Buttons.SaveBookmark.Visible = False
          Navigator.Buttons.GotoBookmark.Visible = False
          Navigator.Buttons.Filter.Visible = False
          Navigator.Visible = True
          DataController.DataSource = dtsConfigGrupo
          DataController.Summary.DefaultGroupSummaryItems = <>
          DataController.Summary.FooterSummaryItems = <>
          DataController.Summary.SummaryGroups = <>
          OptionsBehavior.ColumnHeaderHints = False
          OptionsData.CancelOnExit = False
          OptionsData.Deleting = False
          OptionsData.DeletingConfirmation = False
          OptionsData.Editing = False
          OptionsData.Inserting = False
          OptionsSelection.CellSelect = False
          OptionsView.ColumnAutoWidth = True
          OptionsView.GridLines = glVertical
          OptionsView.GroupByBox = False
          Styles.StyleSheet = dtmControles.GridTableViewStyleSheetDevExpress
          object cxGridDBTableView1SISTEMA_ID: TcxGridDBColumn
            Caption = 'Sistema'
            DataBinding.FieldName = 'SISTEMA_ID'
            Visible = False
            OnGetDisplayText = cxGridDBTableView1SISTEMA_IDGetDisplayText
          end
          object cxGridDBColumn1: TcxGridDBColumn
            Caption = 'Grupos'
            DataBinding.FieldName = 'DESCRICAO'
            Width = 331
          end
        end
        object cxGridLevel1: TcxGridLevel
          GridView = cxGridDBTableView1
        end
      end
      object pgcValores: TPageControl
        Left = 360
        Top = 1
        Width = 609
        Height = 142
        ActivePage = tabValorTabela
        Align = alClient
        TabOrder = 1
        object tabDescritivo: TTabSheet
          Caption = 'Comum(Descritivo)'
          ImageIndex = 2
          object edtValorComum: TcxTextEdit
            Left = 12
            Top = 55
            ParentFont = False
            Style.StyleController = dtmControles.cxEsc_EditsObrigatorio
            TabOrder = 3
            Text = 'edtValorComum'
            Width = 381
          end
          object cxLabel4: TcxLabel
            Left = 11
            Top = 40
            Caption = 'Valor:'
            ParentFont = False
            Style.Font.Charset = DEFAULT_CHARSET
            Style.Font.Color = clNavy
            Style.Font.Height = -11
            Style.Font.Name = 'MS Sans Serif'
            Style.Font.Style = []
            Style.IsFontAssigned = True
          end
          object cxLabel9: TcxLabel
            Left = 10
            Top = 6
            Caption = 'Descri'#231#227'o:'
            ParentFont = False
            Style.Font.Charset = DEFAULT_CHARSET
            Style.Font.Color = clNavy
            Style.Font.Height = -11
            Style.Font.Name = 'MS Sans Serif'
            Style.Font.Style = []
            Style.IsFontAssigned = True
          end
          object cxDBTextEdit1: TcxDBTextEdit
            Left = 13
            Top = 22
            DataBinding.DataField = 'DESCRICAO'
            DataBinding.DataSource = SourceAncestral
            ParentFont = False
            Style.StyleController = dtmControles.cxEsc_EditsNormal
            TabOrder = 0
            Width = 379
          end
        end
        object tabImpressora: TTabSheet
          Caption = 'Impressora'
          object cxLabel2: TcxLabel
            Left = 11
            Top = 40
            Caption = 'Impressora:'
            ParentFont = False
            Style.Font.Charset = DEFAULT_CHARSET
            Style.Font.Color = clNavy
            Style.Font.Height = -11
            Style.Font.Name = 'MS Sans Serif'
            Style.Font.Style = []
            Style.IsFontAssigned = True
          end
          object cbxValorImpressora: TcxComboBox
            Left = 11
            Top = 55
            ParentFont = False
            Properties.DropDownListStyle = lsEditFixedList
            Style.StyleController = dtmControles.cxEsc_EditsObrigatorio
            TabOrder = 3
            Width = 382
          end
          object cxLabel10: TcxLabel
            Left = 10
            Top = 6
            Caption = 'Descri'#231#227'o:'
            ParentFont = False
            Style.Font.Charset = DEFAULT_CHARSET
            Style.Font.Color = clNavy
            Style.Font.Height = -11
            Style.Font.Name = 'MS Sans Serif'
            Style.Font.Style = []
            Style.IsFontAssigned = True
          end
          object cxDBTextEdit2: TcxDBTextEdit
            Left = 13
            Top = 22
            DataBinding.DataField = 'DESCRICAO'
            DataBinding.DataSource = SourceAncestral
            ParentFont = False
            Style.StyleController = dtmControles.cxEsc_EditsNormal
            TabOrder = 1
            Width = 379
          end
        end
        object tabValorPadrao: TTabSheet
          Caption = 'Valores Padr'#227'o'
          ImageIndex = 1
          object cxLabel1: TcxLabel
            Left = 11
            Top = 40
            Caption = 'Valor:'
            ParentFont = False
            Style.Font.Charset = DEFAULT_CHARSET
            Style.Font.Color = clNavy
            Style.Font.Height = -11
            Style.Font.Name = 'MS Sans Serif'
            Style.Font.Style = []
            Style.IsFontAssigned = True
          end
          object lcbValoresPadrao: TcxLookupComboBox
            Left = 12
            Top = 55
            ParentFont = False
            Properties.DropDownListStyle = lsFixedList
            Properties.KeyFieldNames = 'VALOR'
            Properties.ListColumns = <
              item
                FieldName = 'DESCRICAO'
              end>
            Properties.ListSource = dtsValoresPadrao
            Style.StyleController = dtmControles.cxEsc_EditsObrigatorio
            TabOrder = 3
            Width = 381
          end
          object cxLabel11: TcxLabel
            Left = 10
            Top = 6
            Caption = 'Descri'#231#227'o:'
            ParentFont = False
            Style.Font.Charset = DEFAULT_CHARSET
            Style.Font.Color = clNavy
            Style.Font.Height = -11
            Style.Font.Name = 'MS Sans Serif'
            Style.Font.Style = []
            Style.IsFontAssigned = True
          end
          object cxDBTextEdit3: TcxDBTextEdit
            Left = 13
            Top = 22
            DataBinding.DataField = 'DESCRICAO'
            DataBinding.DataSource = SourceAncestral
            ParentFont = False
            Style.StyleController = dtmControles.cxEsc_EditsNormal
            TabOrder = 1
            Width = 379
          end
        end
        object tabValorTabela: TTabSheet
          Caption = 'Valor de Tabela'
          ImageIndex = 3
          object cxLabel3: TcxLabel
            Left = 11
            Top = 40
            Caption = 'Valor:'
            ParentFont = False
            Style.Font.Charset = DEFAULT_CHARSET
            Style.Font.Color = clNavy
            Style.Font.Height = -11
            Style.Font.Name = 'MS Sans Serif'
            Style.Font.Style = []
            Style.IsFontAssigned = True
          end
          object lcbValoresTabela: TcxLookupComboBox
            Left = 12
            Top = 55
            ParentFont = False
            Properties.DropDownListStyle = lsFixedList
            Properties.ListColumns = <>
            Properties.ListSource = dtsValoresTabela
            Style.StyleController = dtmControles.cxEsc_EditsObrigatorio
            TabOrder = 3
            Width = 381
          end
          object cxLabel12: TcxLabel
            Left = 10
            Top = 6
            Caption = 'Descri'#231#227'o:'
            ParentFont = False
            Style.Font.Charset = DEFAULT_CHARSET
            Style.Font.Color = clNavy
            Style.Font.Height = -11
            Style.Font.Name = 'MS Sans Serif'
            Style.Font.Style = []
            Style.IsFontAssigned = True
          end
          object cxDBTextEdit4: TcxDBTextEdit
            Left = 13
            Top = 22
            DataBinding.DataField = 'DESCRICAO'
            DataBinding.DataSource = SourceAncestral
            ParentFont = False
            Style.StyleController = dtmControles.cxEsc_EditsNormal
            TabOrder = 1
            Width = 379
          end
        end
        object tabCor: TTabSheet
          Caption = 'Cor'
          ImageIndex = 4
          object cxLabel6: TcxLabel
            Left = 11
            Top = 40
            Caption = 'Cor:'
            ParentFont = False
            Style.Font.Charset = DEFAULT_CHARSET
            Style.Font.Color = clNavy
            Style.Font.Height = -11
            Style.Font.Name = 'MS Sans Serif'
            Style.Font.Style = []
            Style.IsFontAssigned = True
          end
          object cxButton1: TcxButton
            Left = 263
            Top = 53
            Width = 130
            Height = 25
            Hint = 'Personalizar Cor'
            Colors.Default = clBlue
            Colors.Normal = 15513994
            Colors.Hot = 8421440
            Colors.Pressed = clGreen
            Colors.Disabled = clBtnFace
            OptionsImage.Glyph.Data = {
              
              A5FFE7C6A5FFE7C6A5FFE7C6A5FFE7C6A5FFE7C6A5FFE7C6A5FF}
            ParentShowHint = False
            ShowHint = True
            TabOrder = 2
            OnClick = cxButton1Click
          end
          object cbxValorCor: TcxColorComboBox
            Left = 12
            Top = 55
            Properties.CustomColors = <>
            TabOrder = 4
            Width = 247
          end
          object cxLabel13: TcxLabel
            Left = 10
            Top = 6
            Caption = 'Descri'#231#227'o:'
            ParentFont = False
            Style.Font.Charset = DEFAULT_CHARSET
            Style.Font.Color = clNavy
            Style.Font.Height = -11
            Style.Font.Name = 'MS Sans Serif'
            Style.Font.Style = []
            Style.IsFontAssigned = True
          end
          object cxDBTextEdit5: TcxDBTextEdit
            Left = 13
            Top = 22
            DataBinding.DataField = 'DESCRICAO'
            DataBinding.DataSource = SourceAncestral
            ParentFont = False
            Style.StyleController = dtmControles.cxEsc_EditsNormal
            TabOrder = 1
            Width = 379
          end
        end
        object tabFonte: TTabSheet
          Caption = 'Fonte'
          ImageIndex = 5
          object cxLabel5: TcxLabel
            Left = 11
            Top = 40
            Caption = 'Fonte:'
            ParentFont = False
            Style.Font.Charset = DEFAULT_CHARSET
            Style.Font.Color = clNavy
            Style.Font.Height = -11
            Style.Font.Name = 'MS Sans Serif'
            Style.Font.Style = []
            Style.IsFontAssigned = True
          end
          object cbxValorFonte: TcxFontNameComboBox
            Left = 12
            Top = 55
            ParentFont = False
            Properties.FontPreview.ShowButtons = False
            Properties.FontTypes = [cxftTTF, cxftRaster, cxftDevice, cxftFixed]
            Properties.ShowFontTypeIcon = []
            Style.StyleController = dtmControles.cxEsc_EditsObrigatorio
            TabOrder = 3
            Width = 381
          end
          object cxLabel14: TcxLabel
            Left = 10
            Top = 6
            Caption = 'Descri'#231#227'o:'
            ParentFont = False
            Style.Font.Charset = DEFAULT_CHARSET
            Style.Font.Color = clNavy
            Style.Font.Height = -11
            Style.Font.Name = 'MS Sans Serif'
            Style.Font.Style = []
            Style.IsFontAssigned = True
          end
          object cxDBTextEdit6: TcxDBTextEdit
            Left = 13
            Top = 22
            DataBinding.DataField = 'DESCRICAO'
            DataBinding.DataSource = SourceAncestral
            ParentFont = False
            Style.StyleController = dtmControles.cxEsc_EditsNormal
            TabOrder = 1
            Width = 379
          end
        end
      end
    end
    object cxSplitter1: TcxSplitter
      Left = 1
      Top = 145
      Width = 970
      Height = 8
      HotZoneClassName = 'TcxMediaPlayer9Style'
      HotZone.SizePercent = 43
      HotZone.BorderColor = 14807280
      AlignSplitter = salTop
      Control = PanelData
      Color = 13034466
      ParentColor = False
    end
    object PanelGrids: TPanel
      Left = 1
      Top = 153
      Width = 970
      Height = 204
      Align = alClient
      BevelOuter = bvNone
      Color = 13034466
      ParentBackground = False
      TabOrder = 2
      object pgcPesquisa: TPageControl
        Left = 0
        Top = 0
        Width = 970
        Height = 204
        ActivePage = tabDados
        Align = alClient
        TabOrder = 0
        object tabDados: TTabSheet
          Caption = 'Parametros'
          object cxGridBasica: TcxGrid
            Left = 0
            Top = 0
            Width = 962
            Height = 176
            Align = alClient
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            ParentFont = False
            PopupMenu = popDuplicar
            TabOrder = 0
            LookAndFeel.Kind = lfUltraFlat
            LookAndFeel.NativeStyle = False
            RootLevelStyles.TabsBackground = dtmControles.cxStyle1
            object cxGridBasicaDBTableView1: TcxGridDBTableView
              Navigator.Buttons.CustomButtons = <>
              Navigator.Buttons.PriorPage.Visible = False
              Navigator.Buttons.NextPage.Visible = False
              Navigator.Buttons.Insert.Visible = False
              Navigator.Buttons.Delete.Visible = False
              Navigator.Buttons.Edit.Visible = False
              Navigator.Buttons.Post.Visible = False
              Navigator.Buttons.Cancel.Visible = False
              Navigator.Buttons.Refresh.Visible = True
              Navigator.Buttons.SaveBookmark.Visible = False
              Navigator.Buttons.GotoBookmark.Visible = False
              Navigator.Buttons.Filter.Visible = False
              Navigator.Visible = True
              OnCustomDrawCell = cxGridBasicaDBTableView1CustomDrawCell
              DataController.DataSource = SourceAncestral
              DataController.Summary.DefaultGroupSummaryItems = <>
              DataController.Summary.FooterSummaryItems = <>
              DataController.Summary.SummaryGroups = <>
              OptionsBehavior.ColumnHeaderHints = False
              OptionsData.CancelOnExit = False
              OptionsData.Deleting = False
              OptionsData.DeletingConfirmation = False
              OptionsData.Editing = False
              OptionsData.Inserting = False
              OptionsSelection.CellSelect = False
              OptionsView.ColumnAutoWidth = True
              OptionsView.GridLines = glVertical
              OptionsView.GroupByBox = False
              Styles.StyleSheet = dtmControles.GridTableViewStyleSheetDevExpress
              object cxGridBasicaDBSecao: TcxGridDBColumn
                Caption = 'Se'#231#227'o'
                DataBinding.FieldName = 'SECAO'
                Visible = False
                GroupIndex = 0
                Width = 68
              end
              object cxGridBasicaDBNome: TcxGridDBColumn
                Caption = 'Par'#226'metro'
                DataBinding.FieldName = 'NOME'
                Width = 132
              end
              object cxGridBasicaDBValor: TcxGridDBColumn
                Caption = 'Valor'
                DataBinding.FieldName = 'calc_Valor'
                Width = 158
              end
              object cxGridBasicaDBConfiguracao: TcxGridDBColumn
                Caption = 'Descri'#231#227'o'
                DataBinding.FieldName = 'DESCRICAO'
                Width = 377
              end
              object cxGridBasicaDBTerminal: TcxGridDBColumn
                Caption = 'Terminal'
                DataBinding.FieldName = 'TERMINAL'
                Width = 76
              end
              object cxGridBasicaDBAtualizado: TcxGridDBColumn
                DataBinding.FieldName = 'ATUALIZADO'
                Visible = False
                Options.Filtering = False
              end
              object cxGridBasicaDBTableView1OBRIGATORIO: TcxGridDBColumn
                Caption = 'Obrigat'#243'rio'
                DataBinding.FieldName = 'OBRIGATORIO'
                RepositoryItem = dtmLookup.Combo_SimNao
                Width = 68
              end
            end
            object cxGridBasicaLevel1: TcxGridLevel
              GridView = cxGridBasicaDBTableView1
            end
          end
        end
        object tabTexto: TTabSheet
          Caption = 'Texto'
          ImageIndex = 1
          inline fmeEditor: TfmeEditor
            Left = 0
            Top = 0
            Width = 962
            Height = 176
            Align = alClient
            TabOrder = 0
            ExplicitWidth = 962
            ExplicitHeight = 176
            inherited wptFerramentas: TWPToolbar
              Width = 962
              Hint = 'Bot'#227'o direiro do Mouse + op'#231#245'es'
              ParentShowHint = False
              ShowHint = True
              PopupMenu = TextFields
              sel_EditIcons = [SelCopy, SelCut, SelPaste, SelSelAll, SelHideSel, SelFind, SelReplace, SelSpellCheck]
              FontSizeFrom = 3
              ExplicitWidth = 962
              inherited btnGravacaoAutomatica: TSpeedButton
                Left = 243
                ExplicitLeft = 243
              end
              inherited WPToolPanel1: TWPToolPanel
                Left = 264
                ExplicitLeft = 264
                inherited btnSaveImagem: TWPToolButton
                  OnClick = fmeEditorbtnSaveImagemClick
                end
                inherited btnInserirImagem: TWPToolButton
                  OnClick = fmeEditorbtnInserirImagemClick
                end
                inherited btnSalvarConfiguracao: TWPToolButton
                  OnClick = fmeEditorbtnSalvarConfiguracaoClick
                end
              end
              inherited cxLabel7: TcxLabel
                Left = 354
                Top = 29
                Visible = True
                ExplicitLeft = 354
                ExplicitTop = 29
              end
              inherited speNumColunas: TcxSpinEdit
                Left = 391
                Top = 29
                Visible = True
                ExplicitLeft = 391
                ExplicitTop = 29
                ExplicitWidth = 42
                ExplicitHeight = 19
                Width = 42
              end
            end
            inherited WPReguaHorizontal: TWPRuler
              Width = 962
              ExplicitWidth = 962
            end
            inherited PanelMarcacao: TPanel
              Width = 962
              Visible = True
              ExplicitWidth = 962
              inherited lblInfoMarcacaoInterna: TcxLabel
                AnchorX = 657
                AnchorY = 14
              end
              inherited btnDicionarioGramatical: TcxButton
                LookAndFeel.SkinName = ''
              end
              inherited btnMarcacaoAutomatica: TcxButton
                LookAndFeel.SkinName = ''
              end
              inherited btnMarcacaoManual: TcxButton
                LookAndFeel.SkinName = ''
              end
              inherited btnVerificarMarcacao: TcxButton
                LookAndFeel.SkinName = ''
              end
            end
            inherited wptTexto: TWPRichText
              Width = 936
              Height = 48
              EditOptionsEx = [wpAllowDrawDropBetweenTextBlocks]
              OnKeyPress = fmeEditorwptTextoKeyPress
              ExplicitWidth = 936
              ExplicitHeight = 48
            end
            inherited WPVertRuler1: TWPVertRuler
              Height = 48
              ExplicitHeight = 48
            end
            inherited StatusBar: TStatusBar
              Top = 157
              Width = 962
              ExplicitTop = 157
              ExplicitWidth = 962
            end
          end
        end
      end
    end
  end
  object pnlDuplicar: TPanel [3]
    Left = 0
    Top = 56
    Width = 972
    Height = 110
    Align = alTop
    BevelInner = bvLowered
    BevelWidth = 2
    BorderStyle = bsSingle
    Color = 16247513
    ParentBackground = False
    TabOrder = 1
    Visible = False
    object Shape1: TShape
      Left = 4
      Top = 36
      Width = 960
      Height = 66
      Align = alClient
      Brush.Color = clWindow
      Pen.Color = 4210688
      Pen.Width = 2
      ExplicitWidth = 961
      ExplicitHeight = 67
    end
    object rdbSelecionado: TRadioButton
      Left = 22
      Top = 45
      Width = 113
      Height = 17
      Caption = 'Valor Selecionado'
      Checked = True
      Color = clCream
      Font.Charset = DEFAULT_CHARSET
      Font.Color = 4210688
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      TabOrder = 4
      TabStop = True
    end
    object cxLabel7: TcxLabel
      Left = 22
      Top = 73
      Caption = 'Duplicar para o Terminal:'
      ParentFont = False
      Style.StyleController = dtmControles.cxStyleLabel
      Transparent = True
    end
    object rdbTodaSecao: TRadioButton
      Left = 142
      Top = 45
      Width = 91
      Height = 17
      Caption = 'Toda Se'#231#227'o'
      Color = clCream
      Font.Charset = DEFAULT_CHARSET
      Font.Color = 4210688
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      TabOrder = 5
    end
    object Panel2: TPanel
      Left = 4
      Top = 4
      Width = 960
      Height = 32
      Align = alTop
      BevelInner = bvLowered
      Color = 16247513
      ParentBackground = False
      TabOrder = 0
      object cxLabel8: TcxLabel
        Left = 2
        Top = 2
        Align = alClient
        Caption = 'Duplicar Parametro(s) / Se'#231#227'o'
        ParentColor = False
        ParentFont = False
        Style.BorderStyle = ebsOffice11
        Style.Color = 13034466
        Style.Font.Charset = DEFAULT_CHARSET
        Style.Font.Color = clBlue
        Style.Font.Height = -16
        Style.Font.Name = 'Times New Roman'
        Style.Font.Style = []
        Style.Shadow = True
        Style.TextColor = 4210688
        Style.TextStyle = [fsBold]
        Style.IsFontAssigned = True
        Properties.Alignment.Horz = taCenter
        Properties.Alignment.Vert = taVCenter
        AnchorX = 480
        AnchorY = 16
      end
    end
    object cbxTerminais: TcxComboBox
      Left = 144
      Top = 71
      ParentFont = False
      Style.StyleController = dtmControles.cxEsc_EditsNormal
      TabOrder = 6
      Width = 234
    end
    object btnCancelar: TcxButton
      Left = 670
      Top = 43
      Width = 106
      Height = 25
      Caption = 'Cancelar'
      Colors.Default = clWhite
      LookAndFeel.Kind = lfUltraFlat
      LookAndFeel.NativeStyle = False
      LookAndFeel.SkinName = ''
      OptionsImage.Glyph.Data = {
        
        34447CC3ECFDFFFFFFFF}
      TabOrder = 3
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      OnClick = btnCancelarClick
    end
    object btnDuplicar: TcxButton
      Left = 561
      Top = 43
      Width = 106
      Height = 25
      Caption = 'Duplicar'
      Colors.Default = clWhite
      LookAndFeel.Kind = lfUltraFlat
      LookAndFeel.NativeStyle = False
      LookAndFeel.SkinName = ''
      OptionsImage.Glyph.Data = {
        
        05050505050505050505}
      TabOrder = 2
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      OnClick = btnDuplicarClick
    end
    object btnTerminal: TcxButton
      Left = 272
      Top = 43
      Width = 106
      Height = 25
      Caption = 'Terminais'
      Colors.Default = clWhite
      LookAndFeel.Kind = lfUltraFlat
      LookAndFeel.NativeStyle = False
      LookAndFeel.SkinName = ''
      OptionsImage.Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFF}
      TabOrder = 1
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      OnClick = btnTerminalClick
    end
  end
  object gbxBuscarParametro: TcxGroupBox [4]
    Left = 0
    Top = 166
    Align = alTop
    Caption = 'Buscar Parametro'
    TabOrder = 2
    Height = 43
    Width = 972
    object btnBuscarParametro: TcxButton
      Left = 384
      Top = 12
      Width = 106
      Height = 25
      HelpType = htKeyword
      Caption = 'Buscar'
      Colors.Default = clWhite
      LookAndFeel.Kind = lfUltraFlat
      LookAndFeel.NativeStyle = False
      LookAndFeel.SkinName = ''
      OptionsImage.Glyph.Data = {
        
        FFFFFFFFF5F5F5EFF0EF}
      TabOrder = 0
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      OnClick = btnBuscarParametroClick
    end
    object EdtBuscar: TcxTextEdit
      Left = 14
      Top = 15
      ParentFont = False
      Properties.CharCase = ecUpperCase
      Style.StyleController = dtmControles.cxEsc_EditsNormal
      TabOrder = 1
      OnKeyDown = EdtBuscarKeyDown
      Width = 366
    end
  end
  inherited ActionList1: TActionList
    Left = 621
    Top = 282
  end
  inherited DataSetAncestral: TI9Query
    SQL.Strings = (
      'SELECT *'#13#10'FROM G_CONFIG'#13#10'ORDER BY SECAO, TERMINAL, NOME')
    Left = 651
    Top = 282
  end
  inherited ProviderAncestral: TDataSetProvider
    Left = 683
    Top = 282
  end
  inherited ClientAncestral: TClientDataSet
    BeforePost = ClientAncestralBeforePost
    AfterPost = ClientAncestralAfterPost
    AfterScroll = ClientAncestralAfterScroll
    OnCalcFields = ClientAncestralCalcFields
    Left = 715
    Top = 282
    object ClientAncestralCONFIG_ID: TBCDField
      FieldName = 'CONFIG_ID'
      Origin = 'CONFIG_ID'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
      Precision = 18
      Size = 2
    end
    object ClientAncestralCONFIG_GRUPO_ID: TBCDField
      FieldName = 'CONFIG_GRUPO_ID'
      Origin = 'CONFIG_GRUPO_ID'
      Precision = 18
      Size = 2
    end
    object ClientAncestralCONFIG_PADRAO_ID: TBCDField
      FieldName = 'CONFIG_PADRAO_ID'
      Origin = 'CONFIG_PADRAO_ID'
      Precision = 18
      Size = 2
    end
    object ClientAncestralSECAO: TStringField
      FieldName = 'SECAO'
      Origin = 'SECAO'
      Size = 60
    end
    object ClientAncestralNOME: TStringField
      FieldName = 'NOME'
      Origin = 'NOME'
      Size = 120
    end
    object ClientAncestralTEXTO: TBlobField
      FieldName = 'TEXTO'
      Origin = 'TEXTO'
    end
    object ClientAncestralTERMINAL: TStringField
      FieldName = 'TERMINAL'
      Origin = 'TERMINAL'
      Size = 30
    end
    object ClientAncestralTIPO_VALOR: TStringField
      FieldName = 'TIPO_VALOR'
      Origin = 'TIPO_VALOR'
      Size = 1
    end
    object ClientAncestralcalc_Valor: TStringField
      FieldKind = fkCalculated
      FieldName = 'calc_Valor'
      Size = 120
      Calculated = True
    end
    object ClientAncestralATUALIZADO: TStringField
      FieldName = 'ATUALIZADO'
      Origin = 'ATUALIZADO'
      Size = 1
    end
    object ClientAncestralDESCRICAO: TStringField
      DisplayWidth = 120
      FieldName = 'DESCRICAO'
      Origin = 'DESCRICAO'
      Size = 120
    end
    object ClientAncestralVALOR: TStringField
      FieldName = 'VALOR'
      Size = 520
    end
    object ClientAncestralOBRIGATORIO: TStringField
      FieldName = 'OBRIGATORIO'
      Origin = 'OBRIGATORIO'
      Size = 1
    end
  end
  inherited SourceAncestral: TDataSource
    Left = 747
    Top = 282
  end
  object sqlConfigGrupo: TI9Query
    AfterScroll = sqlConfigGrupoAfterScroll
    Connection = dtmControles.DB
    FetchOptions.AssignedValues = [evMode]
    FetchOptions.Mode = fmAll
    SQL.Strings = (
        'SELECT * '#13#10'FROM G_CONFIG_GRUPO'#13#10'WHERE SISTEMA_ID = :SISTEMA_ID'#13#10 +
        'ORDER BY DESCRICAO')
    Left = 714
    Top = 317
    ParamData = <
      item
        Name = 'SISTEMA_ID'
        DataType = ftBCD
        ParamType = ptInput
      end>
    object sqlConfigGrupoDESCRICAO: TStringField
      FieldName = 'DESCRICAO'
      Size = 60
    end
    object sqlConfigGrupoCONFIG_GRUPO_ID: TBCDField
      FieldName = 'CONFIG_GRUPO_ID'
      Origin = 'CONFIG_GRUPO_ID'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
      Precision = 18
      Size = 2
    end
    object sqlConfigGrupoSISTEMA_ID: TBCDField
      FieldName = 'SISTEMA_ID'
      Origin = 'SISTEMA_ID'
      Precision = 18
      Size = 2
    end
  end
  object dtsConfigGrupo: TDataSource
    DataSet = sqlConfigGrupo
    Left = 746
    Top = 317
  end
  object sqlValoresPadrao: TI9Query
    Connection = dtmControles.DB
    FetchOptions.AssignedValues = [evMode]
    FetchOptions.Mode = fmAll
    SQL.Strings = (
        'SELECT CV.*'#13#10'FROM G_CONFIG_PADRAO_VALOR CV'#13#10'WHERE CV.CONFIG_PADR' +
        'AO_ID = :CONFIG_PADRAO_ID'#13#10'ORDER BY CV.VALOR')
    Left = 530
    Top = 266
    ParamData = <
      item
        Name = 'CONFIG_PADRAO_ID'
        DataType = ftInteger
        ParamType = ptInput
        Value = '0'
      end>
    object sqlValoresPadraoVALOR: TStringField
      FieldName = 'VALOR'
      Size = 30
    end
    object sqlValoresPadraoDESCRICAO: TStringField
      FieldName = 'DESCRICAO'
      Size = 30
    end
    object sqlValoresPadraoCONFIG_PADRAO_VALOR_ID: TBCDField
      FieldName = 'CONFIG_PADRAO_VALOR_ID'
      Origin = 'CONFIG_PADRAO_VALOR_ID'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
      Precision = 18
      Size = 2
    end
    object sqlValoresPadraoCONFIG_PADRAO_ID: TBCDField
      FieldName = 'CONFIG_PADRAO_ID'
      Origin = 'CONFIG_PADRAO_ID'
      Precision = 18
      Size = 2
    end
  end
  object dtsValoresPadrao: TDataSource
    DataSet = sqlValoresPadrao
    Left = 562
    Top = 266
  end
  object sqlValoresTabela: TI9Query
    Connection = dtmControles.DB
    FetchOptions.AssignedValues = [evMode]
    FetchOptions.Mode = fmAll
    Left = 531
    Top = 299
  end
  object dtsValoresTabela: TDataSource
    DataSet = sqlValoresTabela
    Left = 563
    Top = 299
  end
  object ColorDialog: TColorDialog
    Left = 463
    Top = 268
  end
  object popDuplicar: TPopupMenu
    Left = 497
    Top = 267
    object mniDuplicarParametro: TMenuItem
      Caption = 'Duplicar Parametro Terminal'
      OnClick = mniDuplicarParametroClick
    end
    object mniExcluirParametro: TMenuItem
      Caption = 'Excluir Parametro Terminal'
      OnClick = mniExcluirParametroClick
    end
  end
  object TextFields: TPopupMenu
    Left = 744
    Top = 356
    object PageNumber1: TMenuItem
      Caption = 'N'#250'mero da P'#225'gina'
      OnClick = Time1Click
    end
    object NumberofnextPage1: TMenuItem
      Tag = 1
      Caption = 'N'#250'mero da Pr'#243'xima P'#225'gina'
      OnClick = Time1Click
    end
    object NumberofpreviousPage1: TMenuItem
      Tag = 2
      Caption = 'N'#250'mero da P'#225'gina Anterior'
      OnClick = Time1Click
    end
    object PageCount1: TMenuItem
      Tag = 3
      Caption = 'Quantidade de P'#225'ginas'
      OnClick = Time1Click
    end
    object Date1: TMenuItem
      Tag = 4
      Caption = 'Data'
      OnClick = Time1Click
    end
    object Time1: TMenuItem
      Tag = 5
      Caption = 'Hora'
      OnClick = Time1Click
    end
  end
  object ClientDataExportar: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'ProviderAncestral'
    BeforePost = ClientAncestralBeforePost
    AfterPost = ClientAncestralAfterPost
    AfterScroll = ClientAncestralAfterScroll
    OnCalcFields = ClientAncestralCalcFields
    OnReconcileError = ClientAncestralReconcileError
    Left = 843
    Top = 274
    object ClientDataExportarCONFIG_ID: TBCDField
      FieldName = 'CONFIG_ID'
      Origin = 'CONFIG_ID'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
      Precision = 18
      Size = 2
    end
    object ClientDataExportarCONFIG_GRUPO_ID: TBCDField
      FieldName = 'CONFIG_GRUPO_ID'
      Origin = 'CONFIG_GRUPO_ID'
      Precision = 18
      Size = 2
    end
    object ClientDataExportarCONFIG_PADRAO_ID: TBCDField
      FieldName = 'CONFIG_PADRAO_ID'
      Origin = 'CONFIG_PADRAO_ID'
      Precision = 18
      Size = 2
    end
    object ClientDataExportarSECAO: TStringField
      FieldName = 'SECAO'
      Origin = 'SECAO'
      Size = 60
    end
    object ClientDataExportarNOME: TStringField
      FieldName = 'NOME'
      Origin = 'NOME'
      Size = 120
    end
    object ClientDataExportarTEXTO: TBlobField
      FieldName = 'TEXTO'
      Origin = 'TEXTO'
    end
    object ClientDataExportarTERMINAL: TStringField
      FieldName = 'TERMINAL'
      Origin = 'TERMINAL'
      Size = 30
    end
    object ClientDataExportarTIPO_VALOR: TStringField
      FieldName = 'TIPO_VALOR'
      Origin = 'TIPO_VALOR'
      Size = 1
    end
    object ClientDataExportarATUALIZADO: TStringField
      FieldName = 'ATUALIZADO'
      Origin = 'ATUALIZADO'
      Size = 1
    end
    object ClientDataExportarDESCRICAO: TStringField
      DisplayWidth = 120
      FieldName = 'DESCRICAO'
      Origin = 'DESCRICAO'
      Size = 120
    end
    object ClientDataExportarVALOR: TStringField
      FieldName = 'VALOR'
      Size = 520
    end
    object ClientDataExportarOBRIGATORIO: TStringField
      FieldName = 'OBRIGATORIO'
      Size = 1
    end
  end
  object dtsXmlAtualizador: TDataSource
    DataSet = sqlXmlAtualizador
    Left = 491
    Top = 461
  end
  object sqlXmlAtualizador: TClientDataSet
    Aggregates = <>
    FieldDefs = <
      item
        Name = 'CONFIG_ID'
        Attributes = [faRequired]
        DataType = ftBCD
        Precision = 18
        Size = 2
      end
      item
        Name = 'CONFIG_GRUPO_ID'
        DataType = ftBCD
        Precision = 18
        Size = 2
      end
      item
        Name = 'CONFIG_PADRAO_ID'
        DataType = ftBCD
        Precision = 18
        Size = 2
      end
      item
        Name = 'SECAO'
        DataType = ftString
        Size = 60
      end
      item
        Name = 'NOME'
        DataType = ftString
        Size = 120
      end
      item
        Name = 'VALOR'
        DataType = ftString
        Size = 520
      end
      item
        Name = 'DESCRICAO'
        DataType = ftString
        Size = 260
      end
      item
        Name = 'TEXTO'
        DataType = ftBlob
      end
      item
        Name = 'TERMINAL'
        DataType = ftString
        Size = 30
      end
      item
        Name = 'TIPO_VALOR'
        DataType = ftString
        Size = 1
      end
      item
        Name = 'ATUALIZADO'
        DataType = ftString
        Size = 1
      end
      item
        Name = 'OBRIGATORIO'
        DataType = ftString
        Size = 1
      end>
    IndexDefs = <>
    Params = <>
    ProviderName = 'ProviderAncestral'
    StoreDefs = True
    Left = 579
    Top = 461
    object sqlXmlAtualizadorCONFIG_ID: TBCDField
      FieldName = 'CONFIG_ID'
      Required = True
      Precision = 18
      Size = 2
    end
    object sqlXmlAtualizadorCONFIG_GRUPO_ID: TBCDField
      FieldName = 'CONFIG_GRUPO_ID'
      Precision = 18
      Size = 2
    end
    object sqlXmlAtualizadorCONFIG_PADRAO_ID: TBCDField
      FieldName = 'CONFIG_PADRAO_ID'
      Precision = 18
      Size = 2
    end
    object sqlXmlAtualizadorSECAO: TStringField
      FieldName = 'SECAO'
      Size = 60
    end
    object sqlXmlAtualizadorNOME: TStringField
      FieldName = 'NOME'
      Size = 120
    end
    object sqlXmlAtualizadorVALOR: TStringField
      FieldName = 'VALOR'
      Size = 520
    end
    object sqlXmlAtualizadorDESCRICAO: TStringField
      FieldName = 'DESCRICAO'
      Size = 260
    end
    object sqlXmlAtualizadorTEXTO: TBlobField
      FieldName = 'TEXTO'
    end
    object sqlXmlAtualizadorTERMINAL: TStringField
      FieldName = 'TERMINAL'
      Size = 30
    end
    object sqlXmlAtualizadorTIPO_VALOR: TStringField
      FieldName = 'TIPO_VALOR'
      Size = 1
    end
    object sqlXmlAtualizadorATUALIZADO: TStringField
      FieldName = 'ATUALIZADO'
      Size = 1
    end
    object sqlXmlAtualizadorOBRIGATORIO: TStringField
      FieldName = 'OBRIGATORIO'
      Size = 1
    end
  end
end
